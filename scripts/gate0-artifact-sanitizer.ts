...
export async function sanitizeGate0ArtifactTree(root: string, signal?: AbortSignal): Promise<Gate0ArtifactSanitization> {
  ...
  for (const filePath of listRegularFiles(treeRoot)) {
    ...
    if (path.extname(filePath).toLowerCase() === ".zip") {
      ...
      const changed = await sanitizeZip(filePath, relativePath, report, signal);
      if (changed) report.filesChanged += 1;
      continue;
    }
    const original = readBuffer(filePath);
    const sanitized = sanitizeBuffer(original, relativePath);
...
export async function scanGate0ArtifactTree(root: string, signal?: AbortSignal): Promise<Gate0ArtifactScan> {
  const treeRoot = requireSafeTreeRoot(root);
  ...
  for (const filePath of listRegularFiles(treeRoot)) {
    ...
    scan.filesScanned += 1;
    if (path.extname(filePath).toLowerCase() === ".zip") {
      recordViolations(scan, relativePath, Buffer.alloc(0));
      scan.archivesScanned += 1;
      await scanZip(filePath, relativePath, scan, signal);
      continue;
    }
    recordViolations(scan, relativePath, readBuffer(filePath));
  }
...
async function sanitizeZip(
  ...
  for (const entry of Object.values(zip.files)) {
    ...
    const sanitized = sanitizeBuffer(original, `${relativePath}!/${entry.name}`);
    report.replacements += sanitized.replacements;
    if (sanitized.replacements > 0) {
...
async function scanZip(archivePath: string, relativePath: string, scan: Gate0ArtifactScan, signal?: AbortSignal) {
  const zip = await JSZip.loadAsync(readBuffer(archivePath));
  for (const entry of Object.values(zip.files)) {
    throwIfAborted(signal);
    assertSafeZipEntry(entry);
    ...
    if (entry.dir) {
      recordViolations(scan, artifactPath, Buffer.alloc(0));
      continue;
    }
    scan.filesScanned += 1;
    recordViolations(scan, artifactPath, await entry.async("nodebuffer"));
  }
}

function sanitizeBuffer(value: Buffer, artifactPath: string) {
  const textValue = value.toString("utf8");
  const detected = uniqueCategories(secretCategories(artifactPath), secretCategories(textValue));
  ...
  const sanitized = sanitizeGate0DiagnosticText(textValue);
  return { value: Buffer.from(sanitized.value, "utf8"), replacements: sanitized.replacements };
}

export function sanitizeGate0DiagnosticText(value: string) {
  let text = value;
  let replacements = 0;
...
function recordViolations(scan: Gate0ArtifactScan, artifactPath: string, value: Buffer) {
  const text = value.toString("utf8");
  const categories = uniqueCategories(secretCategories(artifactPath), secretCategories(text));
...
function secretCategories(text: string) {
