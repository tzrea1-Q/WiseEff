...
import { runtimeTopologyRelocationRecordPath } from "./parameter-catalog-allowlist/runtimeTopologyRelocation";
...
describe("parameter catalog boundary checker", () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(new Set(report.relocations.map((entry) => entry.id)).size).toBe(39);
      expect(new Set(report.relocations.map((entry) => entry.observed.id)).size).toBe(39);
      expect(new Set(report.relocations.flatMap((entry) => [entry.id, entry.observed.id])).size).toBe(78);
