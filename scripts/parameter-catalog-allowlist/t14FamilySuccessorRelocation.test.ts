...
describe("T1.4 family historical record subset", () => {
  it("keeps all 224 unchanged-file destinations active", async () => {
    ...
    expect(result.relocations).toHaveLength(224);
    expect(new Set(result.relocations.map((entry) => entry.id)).size).toBe(224);
    expect(new Set(result.relocations.map((entry) => entry.observed.id)).size).toBe(224);
