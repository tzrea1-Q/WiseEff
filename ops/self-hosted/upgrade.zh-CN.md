...
编译器必须打印 `releaseId=crel_vendor_catalog_1` 且 digest 为 `sha256:3d5c70fb5e0aad4bb7c3c063ca3283a81fb39ec25c471669a0500dd410a48f97`。digest 不同即停止：厂商 YAML 或编译器已漂移，禁止安装。
...
同一命令因丢失响应而重试时返回 `already-current`。当前指针不是带该 digest 的 `crel_acme_1` 时拒绝。重启后 Catalog 应列出 116 条定义，并仍包含原来的 `iin_max` 以及受审的 `charging_core` NodeType 属性。

该 advance 同时**退役 acme 样例**：`csub_acme_power` 与别名 `cali_acme_power_v1` 携带 tombstone 进入 `crel_vendor_catalog_1`（`reason=acme-sample-retired`，且不含后继主体，因为 acme 从未演化为同名真实厂商主体），而 `crel_acme_1` 中两者仍为 `active`。advance 之后，运行时解析 `acme,power` 返回 `retired` 而非实时匹配。这是预期行为，也是本 runbook 的 pin digest 从早先的 `sha256:efc5336e…` 变更的原因。编译器计数为 49 subjects / 1 alias / 116 definitions：被退役的 acme 成员仍被保留，T1.2 增加受审的 `charging_core` NodeType 及其两个嵌套数组属性。
