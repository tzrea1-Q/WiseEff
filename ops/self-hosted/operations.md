...
| Adopt a verified preexisting current Catalog | From repo root: `npx tsx scripts/catalog-publication-ops.ts adopt --check|--execute ...` with `WISEEFF_PUBLICATION_MANAGER_DATABASE_URL` | Writes a Receipt only. Does not advance the pointer or restore data. See [catalog-publication.md](catalog-publication.md). |
| Grant or revoke publisher capabilities | `npx tsx scripts/catalog-publication-ops.ts capabilities grant\|revoke\|status ...` | Real `user_role_bindings`, not `WISEEFF_CATALOG_TEST_CAPABILITIES`. |
| Isolated publication enable/disable (lab) | `npx tsx scripts/catalog-publication-ops.ts policy enable\|disable --actor <id> --confirmation ephemeral-test-only [--low-risk-single-actor]` | Ephemeral database names only. Low-risk single-actor is a separate flag. |
| Managed-instance publication enable/disable | `policy status` then `policy check enable` then `policy enable` with expected OID/id/digest/revision/freeze/adopted pins | Identity pins required. Enable does not clear freeze or require a prior online publish. See [catalog-publication.md](catalog-publication.md). |
| Online Catalog publication | CP-07 HTTP publish routes after policy enable, adoption, and freeze is clear | Distinct from inspect, adopt, and restore. Default `publication_enabled` remains false. |
...
# Recent logs
./scripts/compose --env-file .env logs --tail=200 api worker publication-manager proxy
...
For an actionable diagnosis, read `failed_phase`, `failure_service`, `failure_code`, `failure_summary`, `recovery_started`, `recovery_verified`, `recovery_failure_summary`, and `next_action` from `./scripts/upgrade.sh status --run-id <run-id> --json`. Both summaries are bounded and redacted; a failed recovery action—including stop, pause, or queue resume—has its sanitized diagnostic printed, journaled, and exposed as `recovery_failure_summary`. Public probes explicitly use `curl --noproxy '*'`. After proxy recreation they retry both live and ready endpoints within the normal health budget, so a transient Caddy startup refusal is not treated as recovery failure; exhaustion still fails closed. A Vite 403 caused by `Host: web:5173` is a Host allowlist result, so use loopback or the configured allowed hostname when checking TCP reachability.
