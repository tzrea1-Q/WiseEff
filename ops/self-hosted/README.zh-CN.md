...
- 已运行的 checkout 使用 [升级入口](upgrade.zh-CN.md)：先执行 `./scripts/upgrade.sh plan --ref <release-tag>`，确认后执行 `./scripts/upgrade.sh apply --ref <release-tag>`。`setup.sh` 仍只负责首次配置、启动和 provision，不是升级入口。
