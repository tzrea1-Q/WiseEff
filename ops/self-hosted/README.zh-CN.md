...
- 已收编、仍有旧示例参数且规范绑定为零的实例，使用经过审核的[示例参数重建指引](seed-rebuild.zh-CN.md)。该流程创建新恢复点并归档 Atlas/Aurora/Nebula 后再重建；不要在该实例上运行旧 M1 种子。
- 自托管 runtime 镜像通过 Alpine `dtc` 包内置 Device Tree Compiler，并在镜像构建时执行 `dtc --version`。旧版内部演示初始化脚本 `./scripts/seed-demo-data.sh` 的 M1 阶段会在容器内真实编译三项目 overlay，不依赖宿主机安装。
