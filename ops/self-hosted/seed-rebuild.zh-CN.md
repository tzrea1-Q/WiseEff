...
计划记录运行中的数据清单供审核，不作为停服后的保留校验基线。`begin` 在隔离写入服务、验证新备份之后，单独采集完整保留基线，并绑定本轮 run ID、未修改的已确认计划和恢复清单。计划到停服之间的正常会话与 Bridge 活动会保留在这份基线中；此后的变化仍会校验失败，包括会话时间戳和 Bridge 心跳。`resume-maintenance` 成功重试备份后也会采集同样的证据，再允许发布。不支持手工刷新基线。

旧版工具创建的运行若缺少维护基线，不允许继续发布或重建。未完成的旧运行应先使用原版本恢复，再升级并使用新的 run ID；不要修改计划或日志来续用旧运行。
...
控制器先完整归档并验证三个项目，再重建。对各项目重建使用的默认配置集，它解除已归档且不在受审来源中的旧成员，并将保留成员的角色和顺序更新为受审值。解除成员关系不会删除文件、版本或来源字节；这些数据仍在新数据验证通过后才清理。因此，即使部署中残留旧 `aurora-board.dts` 成员，当前配置集也会与新的 DTS/JSON 修订保持一致。它逐项比对已审核的参数身份集合、来源固定信息及保留数据，全部通过后才清理已归档的旧残留。目前预期共 372 条绑定，但仅数量相同不能通过验证。完成后的重复验证不会再次重建。

既有修订保留策略不变。已保留修订的成员所引用的文件，即使不再属于当前配置集，也须保留其版本和来源字节；不再被引用的已归档残留仍在验证后清理。

种子 DTS 导入保留模块发现、来源出现记录、节点连续性和校验，但不再生成或更新旧规格、旧绑定及旧审查任务。文件上传时自动触发的导入和最终混合来源版本均采用此规则。正式定义来自两个已发布的 Catalog 阶段，原有旧规格记录仍由维护基线严格保护。普通上传行为保持不变。

物化前，操作者流程确保存在一个以 `seed-placement-capacity:driver-group` 标识的专用额外驱动组位置。暂时空闲的原生驱动模块不能替代它，因为来源主体注册仍需要这些模块。重复准备会复用该专用位置，也兼容早期没有来源键的 `Seed placement capacity` 模块；类型、来源类别或来源标识冲突时拒绝继续，不修改已有记录。ConfigurationSchema 仍使用可用的业务模块位置。物化器本身不会创建这类结构，容量不足时仍会在同步参数值之前阻塞。
