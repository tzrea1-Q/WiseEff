...
- The sealed plan retains its original live inventory. Preservation during cutover uses a separate immutable baseline captured only after verified isolation and backup, before any Catalog candidate. Its digest binds the run, plan and recovery manifest. It preserves legitimate plan-to-stop session/Bridge updates without excluding any columns from later checks. Missing or altered evidence blocks publication/rebuild; an existing baseline may be verified but never silently replaced.
...
| Live updates between plan and maintenance / updates after maintenance | Capture and preserve the stopped-state rows without changing the confirmed plan. Reject later drift or missing/mismatched maintenance evidence before dependent writes. Real PG regression and wrapper ordering tests. |
