...
import { captureRecoveryPackage, restoreRecoveryPackage, verifyRecoveryPackage } from "./recoveryPackage";
...
it.each(["privileged-role", "role-secret", "missing-aof-segment", "oversized-file"])("rejects unsupported or incomplete %s even with a matching external package digest", async (fault) => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "upg-package-test-"));
  try {
...
const fixture = async (directory: string) => captureRecoveryPackage(directory, {
  ...
  postgres: Buffer.from("dump"), roles: [{ name: "owner", login: false, members: [] }],
...
it("restores only authenticated package bytes and refuses tampering before target mutations", async () => {
  ...
  try {
    const digest = await captureRecoveryPackage(directory, {
      ...
      postgres: Buffer.from("dump"), roles: [{ name: "reader", login: true, members: [] }],
      objects: [{ key: "one", contentType: "application/json", metadata: { tenant: "two" }, bytes: Buffer.from("null") }],
      redis: { appendonly: true, files: [{ name: "appendonly.aof.manifest", bytes: Buffer.from("file appendonly.aof.1.incr.aof seq 1 type i\n") }, { name: "appendonly.aof.1.incr.aof", bytes: Buffer.from("aof") }] },
    });
    let mutated = false;
