...
仅验收通过且已确认自有资源清理完成的用例删除合成包目录。失败、超时或清理失败
保留私有包与追加式 journal，并写入 0600 的 `retained-evidence.json` 标记。诊断只
记录相对开发宿主临时目录的定位符，不打印宿主路径；保留包不得进入普通 artifacts。
测试专用留证 fixture 自行创建 0700 目录，不接受任意清理路径，且拒绝再次结算来
删除已经保留的证据。
结算前通过 `lstat` 重新核对目录的设备、inode、owner 和 0700 权限；符号链接、
目录替换或权限漂移均拒绝清理及标记写入，不沿替换路径查找或删除证据。
