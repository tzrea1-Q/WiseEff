...
Checkout 在同步 pool 回调中登记实际 client 及持续 error/end 监听，然后才 resolve 初始化等待。两个永久 EventEmitter 反例在回调刚返回时立即发出 error/end；旧 await checkout 两项都失败。修复后九项 source 加原十项 runtime 兼容共 19/19，包含真实缺数据库 API 子进程。这两个 checkout 用例显式替代 client/daemon，不算实际 PG 连接证据。严格 targeted types 通过；七项真实 PG 仍待执行。
