...
Validation: the initial pure Red collected five failures because the new lease/identity entry points were absent. Focused Green and strict types are recorded against the implementation candidate. The dedicated `vitest.runtime-role-source.config.ts` requires the existing parent-owned PG receipt before collection and refuses zero tests. The actual integration fixture uses migrated isolated PG16, real separate LOGIN passwords, real private files and the real host lock; it does not create Docker resources or invent a valid P12/report. Its scoped handoff fixture is not a whole-root positive. The new setup hook has a 60-second migration budget; individual tests retain Vitest's default budget. Actual PG execution and mandatory runner registration remain pending until the parent schedules and wires the exact suite.
