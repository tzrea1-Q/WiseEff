...
阻止失锁后的写入。
提交后的最后一次报告读取也以真实锁和连接检查结束；若该 await 期间失锁，根返回
unknown，保留已耐久的 credential-step，不将该终态记录覆写成 unknown。
...
同一 hook 还复核已签发的运行角色来源及实际管理会话。非 unknown 的宿主记录在追加前后
也执行该核验。初次观察不授权后续效果：来源丢失或配置漂移会阻止下一项凭据动作或完成确认。
来源不可用时，只要宿主边界仍成立，仍可记录 unknown。真实 PG16 回归验证在凭据事务所需
目录锁内可观察来源，并保持调用者事务不变；这证明事务兼容，不代表完整根退休批准。
