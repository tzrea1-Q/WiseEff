...
- DNS for `WISEEFF_SITE_HOST`, **or** the [IP lab profile](ops/self-hosted/ip-lab.md) when the host has no domain.
...
If the server has an IP and no DNS name, use the IP lab profile instead of copying `.env.example`:
...
See [ops/self-hosted/ip-lab.md](ops/self-hosted/ip-lab.md). For a DNS + Let's Encrypt staging/pilot host:
