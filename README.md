...
The database commands require `DATABASE_URL` to point at a local PostgreSQL database. Start from `.env.example`; it prepares local PostgreSQL, local object storage, simulator device gateway, and production-mode smoke auth defaults. Fill the blank Agent provider URL/model/key values only when testing the live provider path.
