...
function parseQuery(searchParams: URLSearchParams): CatalogPublicationRequest["query"] {
  const query: CatalogPublicationRequest["query"] = {};
...
export function createCatalogPublicationHttpServer(ports: CatalogPublicationPorts): Server {
  return createServer(async (request, response) => {
    const requestId = request.headers["x-request-id"]?.toString() ?? randomUUID();
    try {
      const url = new URL(request.url ?? "/", "http://127.0.0.1");
      const result = await handleCatalogPublication(ports, {
...
export async function listenCatalogPublicationHttpServer(
  ports: CatalogPublicationPorts,
): Promise<{ baseUrl: string; close: () => Promise<void> }> {
  const server = createCatalogPublicationHttpServer(ports);
