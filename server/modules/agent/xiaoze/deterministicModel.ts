  ...
  return {
    async invoke(messages) {
      ...
      if (!hasToolResult) {
        ...
        // 或 `来源重载:<runId>`)pins the approval-gated draft tool so acceptance
        // can drive the interrupt. Match a single line only: the planner appends
        // page context on new lines.
        const draftMatch = text.match(/(?:创建知识草稿|create knowledge draft)[:：]\s*([^\n]+)/i);
        if (draftMatch) {
          let draftLine = draftMatch[1].trim();
          const reloadSourceMatch = draftLine.match(/\s+(?:来源重载|source-reload-run)[:：]\s*(\S+)\s*$/i);
          if (reloadSourceMatch) {
            draftLine = draftLine.slice(0, reloadSourceMatch.index).trim();
          }
            ...
            toolCalls: [
              {
                ...
                args: {
                  ...
                  ...(reloadSourceMatch ? { sourceReloadRunId: reloadSourceMatch[1] } : {})
                }
              }
            ]
          };
