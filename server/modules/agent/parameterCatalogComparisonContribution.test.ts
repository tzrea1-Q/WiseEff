...
import {
  AGT_COMPARISON_CONTRACT_VERSION,
  AGT_COMPARISON_FAMILY,
  AGT_COMPARISON_IDS,
  checksumAgtComparisonBytes,
  provideAgtParameterCatalogComparisonContribution,
  serializeAgtComparisonContribution,
  type AgtComparisonContribution,
  type AgtComparisonContributionInput,
  type AgtComparisonPhase,
  type AgtInventoryMode,
...
function baseInput(
  ...
  inventoryMode: AgtInventoryMode,
  phase: AgtComparisonPhase,
  candidateSha: string,
): AgtComparisonContributionInput {
  return {
    database,
    pool,
    phase,
...
function assertCanonicalChecksum(contribution: AgtComparisonContribution) {
  expect(contribution.contractVersion).toBe(AGT_COMPARISON_CONTRACT_VERSION);
  expect(contribution.family).toBe(AGT_COMPARISON_FAMILY);
  const bytes = serializeAgtComparisonContribution(contribution);
  expect(bytes.toString("utf8").endsWith("\n")).toBe(true);
  expect(bytes.toString("utf8")).not.toContain("\r");
  expect(contribution.checksum).toBe(checksumAgtComparisonBytes(bytes));
}

describe("provideAgtParameterCatalogComparisonContribution", () => {
  ...
  it("fresh pre-activation queries real PostgreSQL and proves zero inventory", async () => {
    freshPreDb = await createDisposableParameterCatalogDatabase("agtfp");
    const database = createPostgresDatabase(freshPreDb.url);
    const pool = getRootPostgresPool(database);
    expect(pool).toBeDefined();
    try {
      const contribution = await provideAgtParameterCatalogComparisonContribution(
  ...
  it("fresh post-p13 independently queries a second database with distinct checksums", async () => {
    freshPostDb = await createDisposableParameterCatalogDatabase("agtfs");
  ...
  it("populated pre-activation and post-p13 enumerate the full inventory independently", async () => {
    populatedDb = await createDisposableParameterCatalogDatabase("agtpop");
    ...
    try {
      const pre = await provideAgtParameterCatalogComparisonContribution(
        baseInput(preDatabase, prePool!, "populated", "pre-activation", POP_PRE_SHA),
      );
      const post = await provideAgtParameterCatalogComparisonContribution(
      ...
      for (const item of [...pre.cases, ...post.cases]) {
        expect(AGT_COMPARISON_IDS.includes(item.comparisonId)).toBe(true);
        expect([
          "exact-equivalent",
      ...
      expect(pre.cases.length).toBe(pre.sourceInventoryCount * AGT_COMPARISON_IDS.length);
    } finally {
