...
export const CANONICAL_PARAMETER_FIXTURE = {
  ...
  otherProjectId: "project-898-canonical-other",
  otherOrganizationId: "org-898-canonical-other",
  editorId: "user-898-canonical-editor",
  reviewerId: "user-898-canonical-reviewer",
  guestId: "user-898-canonical-guest",
  otherOrganizationUserId: "user-898-canonical-other",
  editorUsername: "issue898-editor",
  reviewerUsername: "issue898-reviewer",
  guestUsername: "issue898-guest",
  otherUsername: "issue898-other",
  password: "issue898-local-password",
  categoryModuleId: "pmod-898-canonical-category",
  driverModuleId: "pmod-898-canonical-driver",
  attributionSubjectId: "attr-898-canonical",
} as const;
...
async function insertUsersAndRoles(root: RootDatabase): Promise<void> {
  ...
  if (!pool)
    throw new Error("Issue 898 fixture requires a root PostgreSQL database.");
  ...
  await pool.query(
    `insert into organizations (id, name)
     values ($1, 'Issue 898 Canonical'), ($2, 'Issue 898 Other')`,
    [fixture.organizationId, fixture.otherOrganizationId],
  );
  await pool.query(
    `insert into projects (id, organization_id, name, code, status)
     values
       ($1, $3, 'Issue 898 Canonical Project', 'I898', 'initialized'),
       ($2, $3, 'Issue 898 Other Project', 'I898B', 'initialized')`,
    [fixture.projectId, fixture.otherProjectId, fixture.organizationId],
  );
  await pool.query(
    `insert into projects (id, organization_id, name, code, status)
     values ($1, $2, 'Issue 898 Foreign Project', 'I898X', 'initialized')`,
    [`${fixture.otherOrganizationId}-project`, fixture.otherOrganizationId],
  );
  await pool.query(
    `insert into users (id, organization_id, name, email, title, is_active)
     values
       ($1, $5, 'Issue 898 Editor', 'issue898-editor@example.com', 'Software User', true),
       ($2, $5, 'Issue 898 Reviewer', 'issue898-reviewer@example.com', 'Software Committer', true),
       ($3, $5, 'Issue 898 Guest', 'issue898-guest@example.com', 'Guest', true),
       ($4, $6, 'Issue 898 Other', 'issue898-other@example.com', 'Admin', true)`,
    [
      fixture.editorId,
      fixture.reviewerId,
      fixture.guestId,
  ...
  await pool.query(
    `insert into user_role_bindings (id, user_id, organization_id, project_id, role_id)
     values
       ('urb-898-editor', $1, $5, $7, 'hardware-committer'),
       ('urb-898-reviewer', $2, $5, $7, 'software-committer'),
       ('urb-898-guest', $3, $5, $7, 'guest'),
       ('urb-898-other-admin', $4, $6, null, 'admin')`,
    [
      fixture.editorId,
      fixture.reviewerId,
      fixture.guestId,
...
async function seedCatalogRegistration(
  ...
  await pool.query(
    `insert into attribution_subjects
     ...
     values ($1, $2, 'driver-registration', 'Issue 898 Acme power', 'compatible:acme,power')`,
  ...
  await pool.query(
    `insert into parameter_modules
     ...
     values
       ($1, $3, null, 'Issue 898', $1, 1, 0, '', '', 'business', 'curated', null, null),
  ...
  try {
    ...
    const command: RegisterSubjectCommand = {
      ...
      method: "explicit",
      proof: { reason: "issue-898-canonical-fixture" },
      idempotencyKey: `issue-898-registration-${randomUUID()}`,
      context: { actorKind: "org-admin", principalId: fixture.editorId },
    };
    const written = await writeGuardedRegistration(client, command);
    if (!written.ok) {
      throw new Error(`Issue 898 registration failed: ${written.error.kind}`);
    }
...
export async function seedCanonicalParameterFixture(
  ...
  if (!loaded.ok)
    throw new Error(
      `Issue 898 Catalog fixture unavailable: ${loaded.error.kind}`,
    );

  const seedAuth = makeTestAuthContext({
    userId: fixture.editorId,
    organizationId: fixture.organizationId,
    name: "Issue 898 Editor",
    title: "Software User",
    organizationName: "Issue 898 Canonical",
    roles: [{ projectId: fixture.projectId, roleId: "hardware-committer" }],
  });
  const configSetId = "dcs-898-canonical";
  const fileId = "file-898-canonical";
  const fileVersionId = "file-version-898-canonical-1";
  const sourceName = "issue-898-canonical.dts";
  const definition = loaded.value.getDefinitionById(
    ParameterDefinitionId(X_DEFINITION_ID),
  );
  if (definition.status !== "found")
    throw new Error("Issue 898 Catalog fixture definition is unavailable.");
  const sourceText = `/dts-v1/;\n/ {\n\tlogical-898-canonical {\n\t\tcompatible = \"acme,power\";\n\t\t${definition.definition.propertyKey} = <5>;\n\t};\n};\n`;
  await pool.query(
    `insert into dts_config_set
       (id, organization_id, project_id, name, description)
     values ($1,$2,$3,'Issue 898 canonical source','Issue 898')`,
  ...
  if (!snapshot)
    throw new Error(
      "Issue 898 Catalog fixture published snapshot unavailable.",
    );
  const synced = await withAuditedWrite(
    root,
    seedAuth,
    { requestId: `issue-898-fixture-sync-${randomUUID()}` },
    async (tx) => {
  ...
  const otherAuth = makeTestAuthContext({
    ...
    name: "Issue 898 Other",
    title: "Admin",
    organizationName: "Issue 898 Other",
