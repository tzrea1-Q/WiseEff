...
if (!databaseAvailable) {
  throw new Error(
    "S6-WFA concurrency tests require a reachable real PostgreSQL server with pgvector; skipping is forbidden",
  );
}
...
if (!pgVectorInstalled) {
  throw new Error(
    "S6-WFA concurrency tests require pgvector installed in the real PostgreSQL test database; skipping is forbidden",
  );
}

const ORG = "org-s6-wfa-cx";
const ATTR = "attr-s6-wfa-cx";
const MODULE = "pmod-s6-wfa-cx";
const PROJECT = "project-s6-wfa-cx";
...
describe("protected writeback independent-session races", () => {
  ...
  beforeAll(async () => {
    database = await createEphemeralTestDatabase("s6wfacx");
    ...
    await pool.query(`insert into public.organizations (id, name) values ($1, 'S6 WFA CX')`, [ORG]);
    await pool.query(
      `insert into public.projects (id, organization_id, name, code) values ($1, $2, 'S6 WFA CX', 'S6WFACX')`,
      [PROJECT, ORG],
    );
    await pool.query(
      `insert into public.attribution_subjects (
         id, organization_id, subject_kind, display_name, source_key
       ) values ($1, $2, 'driver-registration', 'S6 WFA CX', 'compatible:acme,power')`,
      [ATTR, ORG],
    );
    await pool.query(
  ...
  it("lets one writeback pin win and the other typed-block without mixing tips", async () => {
    ...
    try {
      const [left, right] = await Promise.all([
        writebackProtectedReference(poolA, {
          snapshot,
          binding,
        ...
        writebackProtectedReference(poolB, {
          snapshot,
          binding,
          definitionRevisionId: REVISION_1,
