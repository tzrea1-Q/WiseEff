...
describe.skipIf(!databaseAvailable)("log service", () => {
  ...
  it("binds an active log domain on upload and stores log_domain_id", async () => {
    await db.query(
      `insert into log_domains (id, organization_id, name, status) values ('domain-1', 'org-1', 'charging-power', 'active')`
    );
    const objectStore = createMemoryObjectStore();
  ...
  it("rejects an archived log domain on upload with 400", async () => {
    await db.query(
      `insert into log_domains (id, organization_id, name, status) values ('domain-1', 'org-1', 'charging-power', 'archived')`
    );
    const objectStore = createMemoryObjectStore();
