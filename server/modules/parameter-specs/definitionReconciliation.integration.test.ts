...
async function seed(db: InMemoryTestDatabase) {
  await db.query(`insert into organizations (id, name) values ($1, 'Reconciliation Org')`, [ORG_ID]);
  await db.query(
  ...
  await db.query(
    ...
    [OLD_SPEC, ORG_ID, OLD_SUBJECT, PROPERTY_KEY, PLATFORM_SPEC, PLATFORM_SUBJECT, DRIVER_ROOT_SPEC],
  );
  await db.query(
...
describe.skipIf(!databaseAvailable)("driver parameter definition reconciliation", () => {
  ...
  it("preflights the dirty twin and applies an audited, idempotent correction", async () => {
    ...
    const dryItems = await db!.query<{ status: string; candidate_parameter_spec_id: string }>(
      `select status, candidate_parameter_spec_id from parameter_definition_reconciliation_items where run_id = $1`,
      [dryRun.runId],
    );
    expect(dryItems.rows).toEqual([{ status: "pending", candidate_parameter_spec_id: PLATFORM_SPEC }]);

    const applied = await reconcileDriverParameterDefinitions(db!, { mode: "apply", organizationId: ORG_ID });
    ...
    expect(placement.rows).toEqual([{ attribution_subject_id: PLATFORM_SUBJECT }]);
    ...
    expect(audit.rows).toEqual([{ actor_type: "system", kind: "parameter-definition-reconciliation" }]);

    const verification = await verifyEffectiveDriverParameterDefinitions(db!, { organizationId: ORG_ID });
    expect(verification).toMatchObject({ status: "ready", checks: expect.any(Array) });
    expect(verification.checks.every((check) => check.count === 0)).toBe(true);

    const rerun = await reconcileDriverParameterDefinitions(db!, { mode: "dry-run", organizationId: ORG_ID });
