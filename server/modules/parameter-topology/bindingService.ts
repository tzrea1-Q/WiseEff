...
type CanonicalCompareQueryRow = CanonicalValueRow & {
  project_id: string;
  project_name: string;
  binding_id: string;
...
export async function listCanonicalBindingCompareRows(
  ...
  const source = await db.query<{ definition_id: string; effective_revision_id: string }>(
    `
    select definition_id, effective_revision_id
