...
import {
  OPS_COMPARISON_CONTRACT_VERSION,
  OPS_COMPARISON_FAMILY,
  OPS_COMPARISON_IDS,
  checksumOpsComparisonBytes,
  provideOpsParameterCatalogComparisonContribution,
  serializeOpsComparisonContribution,
  type OpsComparisonContribution,
  type OpsComparisonContributionInput,
  type OpsComparisonPhase,
  type OpsInventoryMode,
...
function baseInput(
  ...
  inventoryMode: OpsInventoryMode,
  phase: OpsComparisonPhase,
  candidateSha: string,
): OpsComparisonContributionInput {
  return {
    database,
    pool,
    phase,
...
function assertCanonicalChecksum(contribution: OpsComparisonContribution) {
  expect(contribution.contractVersion).toBe(OPS_COMPARISON_CONTRACT_VERSION);
  expect(contribution.family).toBe(OPS_COMPARISON_FAMILY);
  const bytes = serializeOpsComparisonContribution(contribution);
  expect(bytes.toString("utf8").endsWith("\n")).toBe(true);
  expect(bytes.toString("utf8")).not.toContain("\r");
  expect(contribution.checksum).toBe(checksumOpsComparisonBytes(bytes));
}

describe("provideOpsParameterCatalogComparisonContribution", () => {
  ...
  it("production provider has no allow-list, fixture, or prototype-676 inputs", () => {
    const source = readFileSync(fileURLToPath(new URL("./parameterCatalogComparisonContribution.ts", import.meta.url)), "utf8");
  ...
  it("fresh pre-activation queries real PostgreSQL and proves zero inventory", async () => {
    freshPreDb = await createDisposableParameterCatalogDatabase("opsfp");
    const database = createPostgresDatabase(freshPreDb.url);
    const pool = getRootPostgresPool(database);
    expect(pool).toBeDefined();
    try {
      const contribution = await provideOpsParameterCatalogComparisonContribution(
  ...
  it("fresh post-p13 independently queries a second database with distinct checksums", async () => {
    freshPostDb = await createDisposableParameterCatalogDatabase("opsfs");
  ...
  it("populated pre-activation and post-p13 enumerate the full inventory independently", async () => {
    populatedDb = await createDisposableParameterCatalogDatabase("opspop");
    ...
    try {
      const pre = await provideOpsParameterCatalogComparisonContribution(
        baseInput(preDatabase, prePool!, "populated", "pre-activation", POP_PRE_SHA),
      );
      const post = await provideOpsParameterCatalogComparisonContribution(
      ...
      for (const item of [...pre.cases, ...post.cases]) {
        expect(OPS_COMPARISON_IDS.includes(item.comparisonId)).toBe(true);
        expect(item.comparisonId).toBe("PCAT-CMP-D09-LEGACY-OPERATOR-OUTCOME");
        expect([
          "exact-equivalent",
      ...
      expect(pre.cases.length).toBe(pre.sourceInventoryCount * OPS_COMPARISON_IDS.length);
    } finally {
