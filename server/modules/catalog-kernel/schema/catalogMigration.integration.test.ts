...
async function canonicalSchemaFingerprint(db: Database): Promise<string> {
  const [relations, columns, constraints, indexes, triggers, functions] = await Promise.all([
    db.query<{
    ...
    `),
    db.query<{ relation_name: string; constraint_name: string; definition: string }>(`
      select
        class.relname as relation_name,
    ...
    `),
    db.query<{ relation_name: string; index_name: string; definition: string }>(`
      select
    ...
    `),
    db.query<{ relation_name: string; trigger_name: string; definition: string }>(`
      select
        class.relname as relation_name,
    ...
    `),
    db.query<{ function_name: string; identity_arguments: string; definition: string }>(`
      select
...
describe.skipIf(!databaseAvailable)("canonical Catalog migration paths", () => {
  it("applies as the one contiguous suffix after the supported 0136 floor", async () => {
    await withTempDatabase({ prefix: "pcat_floor", migrate: false }, async ({ db }) => {
      const floorApplied = await applyMigrations(db, migrationsDir, { through: floorMigration });
      expect(floorApplied.at(-1)).toBe(floorMigration);
      expect(
        await db.query("select 1 from pg_catalog.pg_namespace where nspname = 'parameter_catalog'")
      ).toMatchObject({ rowCount: 0 });

      const candidateApplied = await applyMigrations(db, migrationsDir, { through: catalogMigration });
      expect(candidateApplied).toEqual([catalogMigration]);
      expect(await applyMigrations(db, migrationsDir, { through: catalogMigration })).toEqual([]);
      ...
      expect(ledger.rows).toEqual([
        { name: catalogMigration, checksum: expect.stringMatching(/^[0-9a-f]{64}$/) }
      ]);
    });
  });

  it("keeps the legacy public parameter tables byte-shape compatible during expansion", async () => {
    await withTempDatabase({ prefix: "pcat_compat", migrate: false }, async ({ db }) => {
      ...
      const canonicalDefinitionColumns = await db.query<{ column_name: string }>(`
        select column_name
        from information_schema.columns
  ...
  it("produces the same deterministic schema fingerprint from fresh and supported-floor paths", async () => {
    ...
    await withTempDatabase({ prefix: "pcat_upgrade", migrate: false }, async ({ db }) => {
      await applyMigrations(db, migrationsDir, { through: floorMigration });
      await applyMigrations(db, migrationsDir, { through: catalogMigration });
    ...
    expect(freshFingerprint).toBe(
      "16469fc1906da10bef959f9ff0b996144841a4d6a980213f3fd4e62e5938b034"
    );
  });
});
