...
Roles are cluster-global. An existing reader is reused only if global role flags, outgoing memberships, inbound membership options, ownership and role settings pass; this database's relation/column/function/schema/database/default/parameter ACLs must remain within the declared capability. Catalog PUBLIC ACLs and relevant future PUBLIC default ACLs must remain revoked. Contamination raises a typed SQLSTATE 42501; the migration does not revoke, normalize or overwrite the contaminated role.
...
The existing migration runner transaction binds role creation, ACL grants and ledger checksum. Failed attempts roll back; committed retries are ledger no-ops. Concurrent database migrations reuse the role after the same audit. No ledger removal or unconditional cleanup is part of recovery.
