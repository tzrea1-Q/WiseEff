...
| `PARAM-IDENTITY-MAP-ADMIN-001` | B | No | Admin resolves identity mapping tasks from `/parameter-admin-next` (construction) / `/parameter-admin` (after ticket 09) with evidence and governance audit. Browser acceptance retarget lands in #198. | pending — `#198` |
