...
| `npm run test:server -- server/modules/parameter-topology/postCutoverWorkflow.integration.test.ts server/modules/parameter-topology/schemaMigration.test.ts server/modules/parameters/schemas.test.ts --run` | Post-cutover exact draft/action identity, set candidate value or delete tombstone proof, two-connection draft row locking, 0060→0061 all-origin invalidation/rollback/idempotency, 0062 action schema, merge/writeback without shadow PPV, immutable base binding revision | After semantic draft submission, migrations 0059–0062, set/delete merge/writeback, concurrency, or binding-revision immutability edits. Requires PostgreSQL. |
...
| `npm run acceptance:e2e -- e2e/acceptance/parameter-topology.acceptance.spec.ts` | Topology governance including draft→activate→resolve plus real set and delete typed draft→submit→role review→merge/writeback/reload; automatically creates, marks, cuts over, verifies, and drops a disposable DB; API runtime resolves the project toolchain without PATH injection. Delete authoring/submission uses public APIs because no delete control exists; all role decisions/merge remain UI operations. | After topology acceptance helpers, semantic edit/submission UI, spec-review UI, set/delete writeback, or immutable merge behavior changes. `DATABASE_URL` supplies only the PostgreSQL server/admin connection; the spec refuses destructive cleanup unless the generated DB name, test marker, and cutover migration run all match. |
