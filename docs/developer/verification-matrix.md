...
| `npm run acceptance:e2e -- e2e/acceptance/parameter-topology.acceptance.spec.ts` | Topology governance including draft→activate→resolve; `acceptanceTaskLookup` (no `items[0]` fallbacks); `semanticFixtureCleanup` prefix-scoped teardown | After topology acceptance helpers, spec-review UI, or immutable merge behavior changes. |
