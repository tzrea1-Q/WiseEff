...
| M6.6 release, rollback, and capacity | `npm run identity:check`, `npm run rollback:rehearsal`, `npm run capacity:gate`, `npm run selfhost:release-gate`, plus `npm run m6:target-evidence` | Deployed self-hosted target, target OIDC identity evidence, `docs/generated/m6-identity-evidence.md`, backup/restore evidence, queue readiness, observability, target synthetic acceptance, rollback rehearsal, capacity metrics | Before treating a self-hosted release candidate as ready for a controlled commercial pilot or moving M6.2-M6.6 plans to completed. |
