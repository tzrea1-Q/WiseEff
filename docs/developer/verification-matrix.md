...
| Xiaoze P2 planning | `npm run acceptance:e2e -- e2e/acceptance/xiaoze-planning.acceptance.spec.ts` plus `npm run test:server -- planningGraph checkpointer suggest agUiEndpoint` and `npm test -- src/features/agent` | Same as P0/P1 plus `XIAOZE_PROACTIVE_ENABLED=true` and `VITE_XIAOZE_PROACTIVE_ENABLED=true` for proactive acceptance | LangGraph planning loop, `MemorySaver` checkpoint resume after approval, read-only proactive suggest API, `useXiaozeSuggestions` / `AgentInsightBar` integration (`XIAOZE-PLAN-MULTISTEP-001`, `XIAOZE-PROACTIVE-001`). |
