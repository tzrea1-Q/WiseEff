...
| M5.12 CI local non-HDC acceptance (L2) | GitHub Actions `acceptance-local-non-hdc` job | PostgreSQL service container, local object store, `XIAOZE_DETERMINISTIC=true`, simulator gateway | Runs on `push` to `main`, nightly schedule, label `full-acceptance`, or `workflow_dispatch` `local-non-hdc`. State models, one quality-run, full `acceptance:browser --mode local-non-hdc`, and evidence artifacts. |
