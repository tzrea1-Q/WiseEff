...
| `npm run dts:toolchain:check -- --required` | dtc + fdtoverlay + dt-validate present **and** match `tools/dts-toolchain/versions.json` (fails on missing, unparseable, or mismatched versions) | Before release-mode publish work or identity cutover rehearsal. Ensure `dt-validate` is on `PATH` (macOS tip: `~/Library/Python/3.9/bin`). |
...
| `npm run acceptance:e2e -- e2e/acceptance/parameter-topology.acceptance.spec.ts` | Topology governance including draft→activate→resolve plus real role-sequenced submit→review→merge/writeback; `acceptanceTaskLookup` (no `items[0]` fallbacks); tenant-scoped transactional `semanticFixtureCleanup` | After topology acceptance helpers, spec-review UI, or immutable merge behavior changes. Requires a dedicated disposable database with a proven `parameter_identity_cutovers` marker; a shared/pre-cutover DB must fail before the business write and is not success evidence. |
