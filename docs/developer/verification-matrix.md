...
| M5.12 CI local non-HDC acceptance (L2) | GitHub Actions `acceptance-quality` (sibling) + `acceptance-local-non-hdc` | PostgreSQL service container, local object store, `XIAOZE_DETERMINISTIC=true`, simulator gateway | Runs on `push` to `main`, nightly schedule, label `full-acceptance`, or `workflow_dispatch` `local-non-hdc`. Quality-run is the sibling job; L2 is state models, full `acceptance:browser --mode local-non-hdc`, and evidence artifacts. |
