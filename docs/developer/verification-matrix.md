...
| `npm run test:server -- server/modules/parameter-topology/postCutoverWorkflow.integration.test.ts server/modules/parameter-topology/schemaMigration.test.ts --run` | Post-cutover exact draft identity, candidate raw-value equality, two-connection draft row locking, migration 0060 invalidation/rollback, merge/writeback without shadow PPV, immutable base binding revision | After semantic draft submission, migration 0059/0060, merge/writeback, concurrency, or binding-revision immutability edits. Requires PostgreSQL. |
