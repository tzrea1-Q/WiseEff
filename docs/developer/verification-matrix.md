...
- `npm run m6:target-evidence` must reject handwritten or partial `Status: passed` evidence. The summary gate parses required target OIDC checks, backup/restore validation summaries, durable queue readiness JSON, observability proofs, rollback steps/artifacts, capacity metrics, release command gates, and target evidence paths before allowing completion.
