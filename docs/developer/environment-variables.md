...
| `WISEEFF_PUBLICATION_MANAGER_DATABASE_URL` | `DATABASE_URL` | publication manager | Independent manager process DSN. LOGIN must be able to `SET ROLE` `catalog_publication_coordinator_role` (claim/job execution columns) and `catalog_synchronizer_role` (installer). Never a database superuser. Ordinary API `DATABASE_URL` must not hold synchronizer credentials. |
