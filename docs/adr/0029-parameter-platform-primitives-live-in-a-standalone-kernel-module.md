...
Cross-cutting parameter primitives live in a standalone, routes-less kernel module, `server/modules/parameter-kernel/`. Kernel files may import only node/deps, `server/shared/**`, `server/modules/auth/**` (auth context types and permission helpers), the audit write boundary (`audit/repository` depends only on node and `server/shared/**`, so the edge is cycle-free), and each other — never `parameters`, `parameter-topology`, `parameter-files`, `parameter-drafts`, or `projects`. There are no compatibility re-exports; every import site points at the kernel directly.

The kernel is populated by verbatim file moves, gated on that import rule:

- **Moved first (import-clean):** `parameterIdentityMode.ts` (identity-mode seam), `legacyParameterIdentityNames.ts` and `legacyParameterIdentityAdapter.ts` (the explicitly transitional pre-cutover compatibility layer).
- **Moved after their leaf dependencies were re-homed:** `policy.ts` imported the `ParameterChangeRequestStatus` type from `parameters/status.ts`, and `sensitiveNode.ts` imported `nodePathFromSourceNodePath` from `parameters/impact.ts`; moving them with those imports intact would have carried kernel→parameters back-edges, relabeling the cycle instead of breaking it. The change-request status list and its derived type now live in the kernel (`workflowStatus.ts`; `parameters/status.ts` builds the wider submission-round vocabulary on top of it), the node-path helper lives in `nodePath.ts` (`parameters/impact.ts` imports it back), and `policy.ts` / `sensitiveNode.ts` moved verbatim with their tests.
...
- With `policy` and `sensitiveNode` in the kernel, the policy/sensitive edges are gone too: `parameter-topology` (routes, service, edit service), `parameter-files` (routes, services), `parameter-specs`, `parameter-modules`, `dts-reload` (policy, sensitive gate, service, configuration service), and `agent/tools/actionTools.ts` all import them from the kernel. Combined with the project-read re-home (ADR-0029), `parameter-topology` non-test code imports nothing from `parameters` — the parameters↔parameter-topology cycle is broken, leaving the one-directional `parameters` → `parameter-topology` imports (write-lock verification, initialization suggestion resolution, migration `LEGACY_SQL`).
