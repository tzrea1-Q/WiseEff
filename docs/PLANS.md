...
- `exec-plans/active/2026-08-17-launch-actionable-tech-debt-closeout.md`: launch-window closeout of tracker rows that do not need HDC, expert logs, or a target environment. Batch 1 archived attribution evidence and moved `2026-08-01-attribution-deferred-implementation.md` to `completed/`; Batch 2 closed TD-056 (parameter-file rollback / display name); Batch 3 landed on `main` — TD-057 via #513, TD-079 hierarchical-modules via #511, import-wizard via #512. TD-079 remaining leftover PPV fixtures are `parameter-files.acceptance.spec.ts`, `project-configuration-workbench.acceptance.spec.ts`, `xiaoze-action.acceptance.spec.ts` pre-cutover fallback, and `e2e/parameter-management.api.spec.ts` (do not flip the shared CI job). TD-082 closed on `main` via #507.
