...
- `exec-plans/active/2026-07-30-platform-tier-and-super-admin.md`: introduce `platform-admin`, the first cross-organization role, then use it to promote proven organization overlays into a platform-scoped driver schema tier that supersedes its contributors and is matched ahead of org overlays and behind vendor (planning only; branches `feat/parse-coverage-scope`, `feat/platform-admin-role`, `feat/driver-schema-platform-tier`; ADR-0009; Phase B blocked on the ADR-0008 overlay PR merging).
...
- `exec-plans/active/2026-07-30-attribution-subjects-and-versioned-specs.md`: attribution subjects + versioned parameter definitions — PR0–PR6 landed on `feat/attribution-subject-versioned-specs` (ADR-0013/0014); open PRs #212–#214 not merged as-is.
- `exec-plans/active/2026-07-31-attribution-governance-follow-up.md`: decided follow-ups after PR0–PR6 — cutover finalize HTTP+UI (PR7), identity-mapping UI (PR8), toast/deprecate defaults/overlay-only claim/subject-primary display/nature-cardinality read-only (PR9). Deferred design questions: `docs/design-docs/2026-07-31-attribution-governance-deferred-questions.md`.
- `exec-plans/active/2026-07-30-parameter-governance-state-machine-completion.md`: make every state machine behind `/parameter-admin` completable and every reachable state meaningful — definition deprecation becomes soft retirement with restore, identity mapping `dismissed` splits into "reject candidate" and "confirm as new identity" so dismissing everything no longer bricks a revision, config revision `published` retires because releasing happens at the file layer, overlay retirement is named apart from definition deprecation because the two have opposite consequences, and the unclassified restore / overlay retirement / module reorder entry points get wired (branches `feat/spec-lifecycle-closure`, `feat/identity-mapping-decision-split`, `feat/parameter-governance-convergence`; migrations `0081`–`0083`; ADR-0011, ADR-0012). Deferred questions and their fact base: `docs/design-docs/2026-07-30-parameter-governance-deferred-questions.md`.
