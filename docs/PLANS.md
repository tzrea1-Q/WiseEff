...
- `exec-plans/active/2026-08-20-self-hosted-one-command-upgrade.md`: mature source-checkout upgrade entry — immutable target resolution, pre-downtime build, queue/write quiescence, verified PostgreSQL/object-store/Redis recovery point, full data-preserving service recreation, health gates, resume, and explicit recovery. Core implementation is on `main`; `fix/self-hosted-upgrade-host-compat` has passed local gates for deployment-user permissions, legacy images, fail-fast errors, and safe stale-lock recovery and awaits PR/CI merge. Clean non-customer forward/recovery evidence remains pending.
...
- `exec-plans/active/2026-08-17-launch-actionable-tech-debt-closeout.md`: launch-window closeout of tracker rows that do not need HDC, expert logs, or a target environment. Batch 1 archived attribution evidence and moved `2026-08-01-attribution-deferred-implementation.md` to `completed/`; Batch 2 closed TD-056 (parameter-file rollback / display name); Batch 3 landed on `main` — TD-057 via #513, TD-079 hierarchical-modules via #511, import-wizard via #512. Batch 4 landed 2026-08-18: workbench fixtures #516, semantic file-sync #519, dts-reload handoff/shapes #517, DTO validation #515, render harness #518, governance ADRs #520. **TD-079 closed** on `fix/td-079-flip-ci-acceptance` (shared CI acceptance is post-cutover). TD-082 closed on `main` via #507. Wave 2 H–N (2026-08-18): TD-013 closed via #529, TD-066 closed via #531; TD-014 / TD-075 / TD-097 / TD-112 remain **partially** open (dts-reload grid via #550). The first deterministic closeout closed TD-071 via #575, TD-073 via #576, and TD-059 via #577; the reload workflow sheet remains outside TD-059. Deterministic closeout wave 2 then closed TD-109 via #580, TD-018 via #582, TD-077 via #583, and TD-114 via #585; TD-003/012 and TD-072/075/076 remain Open for their broader residuals.
- `exec-plans/active/2026-08-22-deterministic-tech-debt-parallel-closeout-wave-3.md`: planned three-worker deterministic closeout for TD-072 / TD-110 / TD-031, then TD-112 in the reused TD-110 slot. TD-112 is scoped only to the `/parameter-admin/projects` `ProjectAdminTable` Admin list and remains conditional on its scope and responsive-behaviour gate.
