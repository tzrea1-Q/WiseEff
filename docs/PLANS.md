...
- `exec-plans/active/2026-08-18-property-key-source-cutover.md`: plan + preview via #544; start + finalize via #549 (migration `0113`); prepare stages file-candidate drafts + spec-editor panel via #553. **TD-117 stays Open.** Everyday loop still requires activating the candidate in the configuration workbench before finalize. Do not enable inline rename while `referenceCount > 0`.
