...
- [Unified SearchField and fuzzy search](exec-plans/active/2026-09-16-unified-fuzzy-search.md): one SearchField primitive, a repo-native multi-field fuzzy core, explicit Search Profiles, and migration of every search input without replacing provider-backed or literal-find engines.
- [Local Device Bridge multi-account rebind](exec-plans/active/2026-09-16-bridge-multi-account-repair.md): keep `/device-bridges/mine` user+organization scoped, rebind the same machine with a new pairing code, rebuild Bridge 0.1.1 artifacts, and stop classifying foreign-account health as an expired token.
- [Project review role governance](exec-plans/active/2026-09-15-project-review-role-governance.md): scoped role configuration, safe organization-role updates, and complete parameter review workflow; P1–P3 completed, P4 acceptance and delivery in progress.
- [Parameter workflow unification and seed parity](exec-plans/active/2026-09-14-parameter-unification-and-seed-parity.md): [specification #849](https://github.com/tzrea1-Q/WiseEff/issues/849) published as `ready-for-agent`; D01–D10 and S1/S2 confirmed, with the 2026-09-15 DTS/JSON scope amendment. PU-00–PU-08 cover complete demo DTS baselines, full workflows/consumers for both formats, ConfigurationSchema, 113 vendor inputs plus four compatibility items, and controlled offline archive. YAML/TOML/ENV project sources and eight compatibility items are deferred to [TD-124](exec-plans/tech-debt-tracker.md); vendor YAML definition metadata remains supported. Implementation remains pending.
- [Agent delivery and verification efficiency](exec-plans/active/2026-09-13-agent-delivery-efficiency.md): #828; EFF-00 baseline and W0 diagnostics first, then equivalent L1, shadow tooling and measured improvements. Module activation remains `observation-pending` until its evidence gates pass.
- [Catalog R2 delivery](exec-plans/active/2026-09-05-catalog-r2-delivery.md): #814–#820 remaining Policy, batch-query, Proposal, Agent and browser evidence work; OP-09 remains outside this round.
...
- `exec-plans/active/2026-09-13-catalog-publication-runtime-acceptance.md`: RA-01–RA-04 operationalize merged PR #827 (`1059acb57`) for isolated actual-server acceptance: manager in standard Compose, executable inspect/adopt/grant/enable, upgrade freeze wiring, honest M1 terminal states. Not production enablement. Does not redo CP-02–CP-10 or reopen #824.
- `exec-plans/active/2026-09-12-catalog-authoring-publication.md`: in-product Catalog definition authoring and online publication (ADR-0043). CP-00–CP-10 landed via #827. Remaining CP-12 / production enablement is separately authorized. This program does not reopen the frozen Wayfinder #668 node graph and does not bundle #824.
- `exec-plans/active/2026-09-05-parameter-catalog-review-remediation.md`: F1–F7 and INT-01 landed on `main` via #812 (`35cbfb18e`). Remaining work is OP-09 / #811 / INT-02 target-host evidence under #735. Program #802 stays open. OP-09 is human-authorized and is not an agent cutover. This plan does not reopen frozen Wayfinder #668 nodes.
...
- [Technical and test documentation consolidation](exec-plans/completed/2026-09-06-technical-test-documentation.md): source-calibrated technical handover and 20 bilingual test-design cases in the existing documentation system; local documentation checks only, schema verification skipped without pgvector.
