...
- `exec-plans/active/2026-07-14-dts-p3-structured-product.md`: DTS structured product closeout (P3) — structured read API + frontend port/mock, value editor by type, config-set/baseline UI, structured diff/change sets, search (path/@address/label/compatible/value), real impact analysis, and node-level/risk-tiered RBAC, migration `0045`. **Status:** implementing on `feat/dts-structured-product` — acceptance IDs `PARAM-DTS-*` + docs in progress; playwright-cli tri-viewport evidence and full `acceptance:e2e` gate may follow.
