...
- `exec-plans/active/2026-07-27-dts-node-enablement.md`: treat DTS `status` as node enablement rather than a parameter — one structural-key source of truth to unblock publish gates, derived enablement/reachability, tree and workbench visibility, three-state editing on the shared draft pipeline (branch `feat/dts-node-enablement`; ADR-0003).
