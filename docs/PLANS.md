...
- `exec-plans/completed/2026-08-05-project-operations-dialog-hardening.md`: project operations surface hardening completed 2026-08-05 on `feat/project-operations-dialog-hardening` — **POD-D1 returned the four views to full-page routes, reaffirming ADR-0001**; shared `ModalDialog` / `ConfirmDialog` primitive (focus trap, focus restore, background inertness, top-most Escape, paired backdrop dismissal, one z-index scale) now owns the dialogs that remain and is the migration target for `2026-08-03-parameter-spec-editor-fidelity.md` items 19–23; `StructuredValueEditor` is styled; baseline release/rollback, member removal and conflict arbitration require confirmation with `requiresConfirmation` enforced; teaching fixtures and raw internals are out of the four views. Residual scope → TD-056 – TD-059.
