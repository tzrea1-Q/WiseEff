...
状态：**#884 `fbb17717a` 与 #885 `be0522655` 已在 `main`（T3.4b）。T3.4a 仍非 full SEAL。T1.4 leftover 55。不要关 #849/#853。**
...
- [x] **T2.3a — 归档处置／恢复决策与独立评审。** 设计 + Spec 复审 PASS（`a53d7ed7-9e12-45a2-a3af-ed0889296b01`）。已点名 residual 删除集；不 DROP 后继表；T2.3b 仍需单独破坏性批准。回执：[t23a-archive-disposal-design.md](849-inventory/t23a-archive-disposal-design.md)。
...
- [x] **T3.1 — 完整 S1 与全量 server 门禁。** TD-125 移除后在 helper PG **55438**／`wiseeff_t23b` 重跑：`test:server` **577 文件 / 4634 通过**。回执：[t31-server-gate-acceptance.md](849-inventory/t31-server-gate-acceptance.md)。未 SEALED。
...
- [x] **T3.4b — 集成 PR、Hosted、合并。** #884 → `fbb17717a`。后续 #885 → `be052265588bd9da0602c099bd82cae0ad02b1c5`（admin squash；L1/quality/smoke 绿；Merge bar 因额度未启动）。回执：[t34b-integration.md](849-inventory/t34b-integration.md)。T3.4a 仍未 full SEAL。不关 Issue。
