...
- TDD Red→Green 证据记录在后文。全新 owned PostgreSQL 的 migration/deletion/trusted-attribution 测试已通过（4 个文件 / 53 个测试）；typed-binding 审计故障回滚及精确 revision/公共投影聚焦切片也通过（3 个文件 / 77 个测试）。当前实现 checkpoint 的完整 server 套件通过（370 个文件 / 2,908 个测试，1 个 intentional skip）。精确树上的 `npm run test:all` 重跑并非绿色：未修改的 frontend 阶段有 8 个超时/失败文件、14 个失败测试（413 个文件 / 3,155 个测试通过），随后阶段未由该串联命令继续；本分支没有 frontend 文件改动。不声称 Hosted CI、HDC、硬件或 live-provider 结果。原 dirty worktree 继续只读且未触碰。
