...
- 当前基线为 `origin/main@82344044b436a8dafecefbb85dfd724cecb05e3f`（主线最高迁移前缀 `0128`）。本次文档提交前的当前实现 checkpoint 是 `2b13356e92a8675c0265bc10272b03245fa244d1`，包含精确 revision/公共投影修复（`610b21f1b`、`23f1b1f63`）、typed-binding 审计原子性修复（`40b90cd14`）、瞬态 draft 注销修复（`2760696fb`）及无 marker 的 draft 读取投影（`2b13356e9`）。本次文档提交只增加证据；最终文档 tip HEAD 在交接报告中给出。
...
- 全新 owned PostgreSQL Green：`server/modules/auth/trustedInvocation.test.ts`、`server/modules/users/deletion.integration.test.ts`、`server/modules/parameter-topology/migration.test.ts`、`server/shared/database/migrations.test.ts` 通过 `4 files / 54 tests`。移除瞬态 draft marker 后，完整 server 又暴露生产读取投影 Red：`9 failed files / 21 failed tests` 均源自已不存在的 `parameter_drafts.initiator_principal_deleted`。将这些读取改为内部 constant-false 投影后，受影响的 `9 files / 113 tests` 及完整 server `370 files / 2,910 passed / 1 skipped` 均在全新 owned 数据库 Green。扩展 #614 切片此前通过 `15 files / 265 tests`。本轮有界修复之前的两次精确树 `npm run test:all` 在未修改 frontend 路径非绿色：首次以 `8 failed / 413 passed` 文件、`14 failed / 3,155 passed` 测试停止，按要求重跑后以 `6 failed / 415 passed` 文件、`12 failed / 3,157 passed` 测试停止（5 秒超时），均未进入 scripts/bridge/server；6 个文件隔离重跑通过 `6 files / 151 tests`；独立 scripts 通过 `69 files / 953 tests`、5 个 skip，bridge 通过 `21 files / 138 tests`。
