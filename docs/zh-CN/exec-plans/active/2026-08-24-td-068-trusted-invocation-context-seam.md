...
| 生成物、runbook、前端/设计、references | Updated generated artifact | `docs/generated/db-schema.md` 记录 owner 授权的 0129-0135 字段/约束、0136 tombstone 投影、0137 scope guards 及 enablement binding 可空身份；`docs/runbooks/`、`src/` 与 `docs/references/` 保持不变。 |
...
- 由于七个 #614 identity migration 与 0135 tombstone 修复均只存在于本分支，owner 授权在主线新 0128 之后再次进行一次保持内容不变的整体重编号。最终有序文件为 `0129_parameter_execution_provenance.sql`、`0130_parameter_system_user_attribution_guard.sql`、`0131_parameter_governance_execution_identity.sql`、`0132_parameter_draft_enablement_owner_index.sql`、`0133_parameter_execution_identity_checks.sql`、`0134_parameter_execution_identity_legacy_compatibility.sql`、`0135_parameter_execution_identity_discriminated_constraints.sql`、`0136_parameter_execution_principal_tombstones.sql`，随后是有界 scope 修复 `0137_parameter_execution_principal_scope_guards.sql`。主线 0116–0128 完全未修改；没有 alias 或 checksum 重写。
- 重编号前 migration invariant 是真实 Red：分支 0128 与主线 0128 冲突，仓库测试无法接受重复前缀；旧共享数据库在缺少旧分支文件时也按设计拒绝继续（该数据库不作为 Green 证据）。重编号后，在全新 owned PostgreSQL 上唯一前缀不变量通过，完整迁移可应用至 0137；upgrade seam 更新为先 replay 至 0132，再应用严格的 0133–0137 链。
- 最终重命名和 scope 修复后，使用 owned pgvector PostgreSQL 正式运行 `npm run db:schema-doc`。`docs/generated/db-schema.md` 现记录实际 135 个迁移至 `0137_parameter_execution_principal_scope_guards.sql`；没有手工伪造 schema artifact，也没有修改共享 `schema_migrations` 历史。计划继续保持 active，并继续排除 #615、TD-123、frontend、HDC、硬件、live-provider 与 Hosted CI 证据。
- Standards 的 scope finding 也完成了真实 Red→Green：`server/modules/users/deletion.integration.test.ts` 在 0137 前允许把 org-A file-version tombstone 替换为 org-B（`1 failed / 6 tests`）；0137 后 snapshot 替换、file-id move、tombstone scope 修改及 file Organization move 均以 `23514` 拒绝，原行保持不变（含 migration upgrade 共 `2 files / 33 tests`）。
