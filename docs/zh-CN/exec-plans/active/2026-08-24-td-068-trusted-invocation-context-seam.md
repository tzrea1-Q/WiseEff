...
- 当前基线为 `origin/main@82344044b436a8dafecefbb85dfd724cecb05e3f`（主线最高迁移前缀 `0128`）。证据文档提交前的当前实现 checkpoint 是 `2b13356e91498a683aeb2bae45a8f6763d2412e2`，包含精确 revision/公共投影修复（`610b21f1b`、`23f1b1f63`）、typed-binding 审计原子性修复（`40b90cd14`）、瞬态 draft 注销修复（`2760696fb`）及无 marker 的 draft 读取投影（`2b13356e9`）。精确本地门禁 checkpoint 是 `d09e9659b567e18a543ef58b2fcf9d506d552bb5`；最终纯证据文档 tip HEAD 在交接报告中给出。
- 最终的分支级注销修复是 `0136_parameter_execution_principal_deleted_marker.sql`。它只向八类保留行引入无身份值、服务端拥有的 `initiator_principal_deleted` 布尔标记；没有 tombstone 表、被删除用户原始 id、snapshot 列、哈希或替代标识。永久删除后，保留的 Agent 行具有 NULL 可问责用户外键、`initiator_type = 'agent'`、完整且非空的 session/tool-call/approval 关联以及 `true` 标记；User/System/Legacy 行保持 `false` 标记和各自判别联合字段。瞬态 `parameter_drafts` 保持 `user_id ON DELETE CASCADE`，不增加 marker，并随账户删除。
...
- TDD Red→Green 证据记录在后文。全新 owned PostgreSQL 的 migration/deletion/trusted-attribution 测试已通过（4 个文件 / 54 个测试）；typed-binding 审计故障回滚及精确 revision/公共投影聚焦切片也通过（3 个文件 / 77 个测试）。移除 draft marker 后，生产 draft repository 在完整 server 中产生真实 Red（9 个失败文件 / 21 个失败测试，PostgreSQL `42703`）；改为不查询 marker 列且在内部投影为未删除后，受影响切片通过（9 个文件 / 113 个测试），完整 server 通过（370 个文件 / 2,910 个测试，1 个 intentional skip）。在 `d09e9659b` 上，完整 `npm run test:all` 整体退出 0：frontend `421 个文件 / 3,169 个测试`、scripts `69 个文件 / 953 passed / 5 skipped`、bridge `21 个文件 / 138 个测试`、server `370 个文件 / 2,910 passed / 1 skipped`。更早的 frontend 非绿色尝试在后文明确保留为历史证据。本分支没有 frontend 文件改动。不声称 Hosted CI、HDC、硬件或 live-provider 结果。原 dirty worktree 继续只读且未触碰。
...
下文所有早于当前状态区块的 checkpoint 均为历史证据。其旧的“final”措辞、commit SHA、迁移编号及本地 CI 计数仅描述当时的树，不代表当前状态。
...
| 生成物、runbook、前端/设计、references | Updated generated artifact | `docs/generated/db-schema.md` 记录 owner 授权的 0129-0135 字段/约束、0136 tombstone 投影、0137 scope guards、0138 snapshot guards 及 enablement binding 可空身份；`docs/runbooks/`、`src/` 与 `docs/references/` 保持不变。 |
...
- 由于七个 #614 identity migration 与 0135 tombstone 修复均只存在于本分支，owner 授权在主线新 0128 之后再次进行一次保持内容不变的整体重编号。最终有序文件为 `0129_parameter_execution_provenance.sql`、`0130_parameter_system_user_attribution_guard.sql`、`0131_parameter_governance_execution_identity.sql`、`0132_parameter_draft_enablement_owner_index.sql`、`0133_parameter_execution_identity_checks.sql`、`0134_parameter_execution_identity_legacy_compatibility.sql`、`0135_parameter_execution_identity_discriminated_constraints.sql`、`0136_parameter_execution_principal_tombstones.sql`，随后是有界 scope 修复 `0137_parameter_execution_principal_scope_guards.sql` 和 snapshot guard `0138_parameter_execution_principal_snapshot_guards.sql`。主线 0116–0128 完全未修改；没有 alias 或 checksum 重写。
- 重编号前 migration invariant 是真实 Red：分支 0128 与主线 0128 冲突，仓库测试无法接受重复前缀；旧共享数据库在缺少旧分支文件时也按设计拒绝继续（该数据库不作为 Green 证据）。重编号和有界 guard 后，在全新 owned PostgreSQL 上唯一前缀不变量通过，完整迁移可应用至 0138；upgrade seam 更新为先 replay 至 0132，再应用严格的 0133–0138 链。
- 最终重命名和 scope 修复后，使用 owned pgvector PostgreSQL 正式运行 `npm run db:schema-doc`。`docs/generated/db-schema.md` 现记录实际 136 个迁移至 `0138_parameter_execution_principal_snapshot_guards.sql`；没有手工伪造 schema artifact，也没有修改共享 `schema_migrations` 历史。计划继续保持 active，并继续排除 #615、TD-123、frontend、HDC、硬件、live-provider 与 Hosted CI 证据。
- Standards 的 scope findings 也完成了真实 Red→Green：`server/modules/users/deletion.integration.test.ts` 在 0137 前允许把 org-A file-version tombstone 替换为 org-B（`1 failed / 6 tests`）；在 0138 前同 Organization tombstone 替换仍成功（`1 failed / 6 tests`）。0137/0138 后跨 Organization 与同 Organization snapshot 替换、file-id move、tombstone scope 修改及 file Organization move 均以 `23514` 拒绝，原行保持不变（含 migration upgrade 共 `2 files / 33 tests`）。
...
- 父级审查发现早期 `0136`–`0138` tombstone/snapshot 设计违反隐私合同：它在 tombstone 中保留已删除的 `users.id`，并由 trusted row projection 恢复。Owner 合同要求永久删除后所有保留业务/审计行的用户外键及派生 `userId` 都是 `NULL`。本次追加修复现已达到实现 checkpoint `2760696fb253ef3d61b32a7cdb9df81d4364900e`，保持 `origin/main@82344044b436a8dafecefbb85dfd724cecb05e3f`（主线最高迁移前缀 `0128`）不变。
- 删除三个 raw-ID 分支迁移，替换为 `0136_parameter_execution_principal_deleted_marker.sql`。它只向八个带用户字段的保留治理表及 binding revisions 增加 `initiator_principal_deleted boolean not null default false`。瞬态 `parameter_drafts` 保持 `user_id ON DELETE CASCADE`，没有 marker 列，也没有监听 marker 更新的 trigger。没有 tombstone 表、raw-ID/snapshot/hash/替代列、身份映射或 alias。`trustedDomainAttributionFromRow` 只使用活动外键；标记为 true 时即使调用者传入旧 id 也强制返回 `userId: null`。
...
- 全新 owned PostgreSQL Green：`server/modules/auth/trustedInvocation.test.ts`、`server/modules/users/deletion.integration.test.ts`、`server/modules/parameter-topology/migration.test.ts`、`server/shared/database/migrations.test.ts` 通过 `4 files / 54 tests`。移除瞬态 draft marker 后，完整 server 又暴露生产读取投影 Red：`9 failed files / 21 failed tests` 均源自已不存在的 `parameter_drafts.initiator_principal_deleted`。将这些读取改为内部 constant-false 投影后，受影响的 `9 files / 113 tests` 及完整 server `370 files / 2,910 passed / 1 skipped` 均在全新 owned 数据库 Green。扩展 #614 切片此前通过 `15 files / 265 tests`。最终完整 `npm run test:all` 在 `d09e9659b` 整体退出 0：frontend `421/3,169`、scripts `69/953 passed/5 skipped`、bridge `21/138`、server `370/2,910 passed/1 skipped`。随后一次独立 server 运行在 `knowledge/parameterReferences.test.ts` 遇到一个与本分支无关的同时间戳审计排序失败；隔离重跑通过 `1 file / 14 tests`，再用全新数据库完整重跑通过 `370/2,910 passed/1 skipped`。更早的 frontend 非绿色尝试仅作为历史证据保留。
...
- 注销集成矩阵覆盖八种保留 Agent 行（history、value、file version、candidate、config revision、submission round、change request、review decision）：所有可问责用户 FK 为 `NULL`，`initiator_type='agent'`，marker true，session/tool-call/approval 精确保留。另一个从公开 `deleteUser` seam 进入的 PostgreSQL 回归证明未提交 Agent draft 会级联删除，`parameter_drafts` 没有 marker 列，也没有监听 marker 更新的 trigger；修复前该用例因 marker 列存在而真实 Red `1/1`，首次 Green 又暴露共享 trigger 函数直接访问缺失字段，改成 record-safe 投影后最终达到 `4 files / 54 tests` Green。information-schema 查询证明不存在 tombstone 表和 `initiator_principal_user_id` 列。直接设置/清除/改类型/重新挂接 marker 均以 `23514` 失败；畸形 User/Agent/System/Legacy 与 binding-revision 行继续被拒绝。
- 父级审查还发现两个隐私修复之后的问题：trusted typed-binding draft 可能选择比请求 base revision 更新的 working-tip locator，且 candidate/file/binding 写入发生在 success-audit savepoint 之前。修复将 trusted draft 绑定到精确 base revision（`610b21f1b`，并以 `23f1b1f63` 保留 legacy working-tip 兼容），并在一个外层事务中包住完整 typed-binding 数据库写入与 `binding-edited` success audit（`40b90cd14`）。真实 PostgreSQL Red 在注入审计失败时留下一个 writeback version、candidate、members、binding revision 和 draft；Green 后这些行全部保持不变。object-store put 仍是外部不可变写入，明确记录为不可达 orphan 边界。
- 同一修复也让 unresolved 的跨 Organization reviewer 不再出现在公共 workflow 投影中：查询不到名称时显示不含身份的占位符而不是原始 id。精确 locator 与 reviewer 脱敏回归已在当前聚焦切片通过；完整 trusted correlation 仍只保留在内部行和审计事件中。
