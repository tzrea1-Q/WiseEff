...
- 迁移 `0120_parameter_execution_identity_checks.sql` 为 draft、review decision、history/value、file version/candidate、config revision 与 binding revision 建立严格约束：可信 User/Agent 行必须携带已校验 principal 且不能携带 System 字段；System 行不能携带用户归属，并必须有非空 `service`/`job` 身份。追加迁移 `0121_parameter_execution_identity_legacy_compatibility.sql` 只保留完全没有 provenance 的 #614 之前历史 `user` 默认行；携带任何 provenance 的行不能走兼容分支。不创建 synthetic user，也不借用 auth.user。生成 schema 文档现记录 119 个迁移及最终约束。
...
- 严格 schema 的受控完整 server 运行最初暴露 116 个没有历史 creator attribution 的旧 fixture。没有削弱 trusted write，而是追加 0121：仅在全部 provenance 字段缺失时兼容这些历史行；随后完整 server 恢复为 363 files / 2,804 passed / 1 skipped。当前 #614 affected PostgreSQL matrix 为 10 files / 131 tests，通过 exact-node、cutover 第二位置、semantic/legacy/enablement writeback、merge notification、migration 与 Agent cleanup 覆盖。
...
- `a2d2a714c` 上的最终本地门禁通过：`npx tsc -b --pretty false`；10 文件 / 131 测试的 PostgreSQL 矩阵；`npm run test:server`（363 文件 / 2,804 passed / 1 skipped）；`npm run test:all`（frontend 420 / 3,137，scripts 69 / 948 passed / 5 skipped，bridge 21 / 138，server 363 / 2,804 / 1 skipped）；build；contract；使用 pgvector 的 `docs:check` 且 schema 检查实际运行；self-host；以及 `acceptance:ci`。lint 为 0 errors、301 个继承的 frontend warnings。build 保留既有 5 个 browser-externalization warning 和 large-chunk warning；测试输出既有 jsdom navigation 与 oversized-process-id informational stderr。`git diff --check origin/main...HEAD` 仅报告已应用的 0118、0119 migration EOF 空行继承 warning；这些 immutable 文件未修改。由于本月额度耗尽未运行 Hosted CI，也不声称其通过。
