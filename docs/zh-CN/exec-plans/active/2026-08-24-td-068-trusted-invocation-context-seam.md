...
- 当前基线为 `origin/main@82344044b436a8dafecefbb85dfd724cecb05e3f`（主线最高迁移前缀 `0128`）。本次文档提交前的当前实现 checkpoint 是 `40b90cd141e9efa426041b254ac3222420f3a945`，包含精确 revision/公共投影修复（`610b21f1b`、`23f1b1f63`）及 typed-binding 审计原子性修复（`40b90cd14`）。本次文档提交只增加证据；最终文档 tip HEAD 在交接报告中给出。
...
- 父级审查发现早期 `0136`–`0138` tombstone/snapshot 设计违反隐私合同：它在 tombstone 中保留已删除的 `users.id`，并由 trusted row projection 恢复。Owner 合同要求永久删除后所有保留业务/审计行的用户外键及派生 `userId` 都是 `NULL`。本次追加修复现已达到实现 checkpoint `40b90cd141e9efa426041b254ac3222420f3a945`，保持 `origin/main@82344044b436a8dafecefbb85dfd724cecb05e3f`（主线最高迁移前缀 `0128`）不变。
...
- 全新 owned PostgreSQL Green：`server/modules/auth/trustedInvocation.test.ts`、`server/modules/users/deletion.integration.test.ts`、`server/modules/parameter-topology/migration.test.ts`、`server/shared/database/migrations.test.ts` 通过 `4 files / 53 tests`。扩展 #614 切片通过 `15 files / 265 tests`；完整 server 通过 `370 files / 2,909 tests`，1 个 intentional skip。精确树上的两次 `npm run test:all` 均在未修改 frontend 路径非绿色：首次以 `8 failed / 413 passed` 文件、`14 failed / 3,155 passed` 测试停止，按要求重跑后以 `6 failed / 415 passed` 文件、`12 failed / 3,157 passed` 测试停止（5 秒超时），均未进入 scripts/bridge/server；6 个文件隔离重跑通过 `6 files / 151 tests`；独立 scripts 通过 `69 files / 953 tests`、5 个 skip，bridge 通过 `21 files / 138 tests`。
