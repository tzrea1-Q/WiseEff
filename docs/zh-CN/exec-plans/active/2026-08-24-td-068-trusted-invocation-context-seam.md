...
- 本轮同时删除未使用的 accountable-user helper，并以 `TrustedInvocationDomainAttributionRow` / `trustedDomainAttributionFromRow` 集中内部 row attribution shape；公共 projection 独立维护。当前 generated schema 已更新为主线之后的 `0134_parameter_execution_identity_discriminated_constraints.sql`。本修复不修改原始 dirty worktree、公共 actor selector、frontend、#615、TD-123、DTS reload、HDC、硬件、live-provider 或 Hosted CI。GitHub Actions 配额耗尽，不声称 Hosted 结果。
...
- 有序映射为：`0116→0128_parameter_execution_provenance.sql`、`0117→0129_parameter_system_user_attribution_guard.sql`、`0118→0130_parameter_governance_execution_identity.sql`、`0119→0131_parameter_draft_enablement_owner_index.sql`、`0120→0132_parameter_execution_identity_checks.sql`、`0121→0133_parameter_execution_identity_legacy_compatibility.sql`、`0122→0134_parameter_execution_identity_discriminated_constraints.sql`。拓扑迁移升级通过 `27/27`；在全新 owned PostgreSQL 上使用正式 `npm run db:schema-doc` 生成 132 个迁移至 `0134`，`docs/generated/db-schema.md` 无生成漂移。
- `0134` 中严格 User/Agent/System/legacy 约束保持不变，畸形的新建/更新行仍被拒绝。同一 branch-only migration 中的窄 FK 删除适配器只识别 PostgreSQL `SET NULL` 的嵌套账户删除转换（trigger depth > 1），把保留的 User 归属变成明确、无 metadata 的 `legacy`；trigger depth 1 的直接应用更新仍拒绝。repository 公共 mapper 恢复既有 null/“已注销用户”展示合同。全新 owned PostgreSQL 回归通过 15-file / 182-test 与 20-file / 261-test 切片，repository `5/5`、review workflow `17/17`、用户删除 `3/3`，完整 server 为 370 files / 2,901 passed / 1 skipped。
