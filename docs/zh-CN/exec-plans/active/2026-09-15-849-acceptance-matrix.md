...
| 4 | 按种子重建：保留非参数数据；旧参数、草稿、历史及源文件离线归档；仅初始化 Atlas／Aurora／Nebula；旧链接归档提示；acme 退役但保留发布历史 | **部分交付** | **离线归档已交付（仅捕获）**：`0149_project_parameter_plane_archives.sql` 与 `seedInitialization/archive.ts` 把 14 个声明关系连同逐表计数和摘要捕获到对象存储；重建守卫现在还会回读对象并校验内容／账本摘要与来源信息。`archive.integration.test.ts`（6 项）覆盖计数、子作用域、跨项目隔离、源行保留、幂等复用、缺失／截断／篡改拒绝、鉴权和 schema 保留约束。迁移 0148 证据还证明已认证组织绑定、先鉴权再写日志、每个 digest 最多一个执行中任务，以及 `completed` 终态不可变（`plan.test.ts` 7；`materialize.test.ts` 7）。**处置（删除）刻意未实现**——账本没有任何列暗示删除已经发生，移除归档行必须另行评审。**归档旧链接提示与 acme 退役仍已交付**并保留历史证据 | JSON 种子物化（B1）、补齐 `csub_drv_sc8562` 容量、acme **定义**生命周期、归档平面处置（删除）、目标静默／恢复、其余 canonical 绑定／值物化（B2）、归档提示界面 |
