...
7. 通过两份既有 acceptance spec 证明 deterministic/offline startup、health、Xiaoze perception 与 action；运行时固定 `XIAOZE_DETERMINISTIC=true`，不得提供或调用 live provider，也不得把缺失目标证据写成通过。
...
| 区域 | 状态 | 精确文件 / 证据要求 |
| --- | --- | --- |
| 仓库地图 | Review | `AGENTS.md`、`docs/zh-CN/root/AGENTS.md`、`ARCHITECTURE.md`、`docs/zh-CN/root/ARCHITECTURE.md`、`docs/README.md`；导航/runtime boundary 未变则记录 unchanged。 |
| 架构入口 | Update | `README.md`、`docs/zh-CN/README.md`、`docs/design-docs/full-stack-architecture.md`、`docs/zh-CN/design-docs/full-stack-architecture.md`；对齐 Xiaoze canonical runtime/configuration 入口，保留 local-deterministic 与 live-provider 证据边界。 |
