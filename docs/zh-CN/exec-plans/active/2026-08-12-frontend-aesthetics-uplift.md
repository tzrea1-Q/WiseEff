...
- [ ] 把 18 个测试文件中 47 处 CSS 原文正则断言迁移为行为/DOM 断言或作用域 fixture,使单体可重构(FA-05)。
- [ ] 按设计系统定案令牌块:语义色(单一色板;shadcn 键做别名)、字阶、间距、圆角、层次、动效、扩展 z-index 阶梯(FA-01、FA-02、FA-07、FA-08、FA-09 地基)。
- [ ] 补定义 16 个被引用未定义的令牌;删除约 29 个死令牌(FA-04)。
- [ ] 字体:删除 Google Fonts import;`--font-sans` 以 Geist Variable + CJK 回退链定案;统一 `--font-mono`;字重收敛到 400/500/600/700(FA-03)。
- [ ] 以 ADR(下一个空号)记录令牌单一来源决策,并更新 `CONTEXT.md` 词汇(design token、elevation、accent)。
- [ ] 基于 `--ring` 的全局 `:focus-visible` 基线;移除 6 处 hover+focus 合并(FA-11 基线)。
...
- [ ] P0:ADR 记录;`CONTEXT.md` + `docs/adr/README.md` 更新;`design-system-reference-llms.txt` 对齐
