...
- **TD-113（ui:check/eslint 存量待清偿）：** 美学纲领 P4 门禁锁住的是诚实存量而非零：`scripts/ui-standards-baseline.json` 仍有 raw-color 1704、raw-font-size 977、raw-shadow 142、raw-z-index 76,`eslint.config.js` 记录 19 条 warn 级规则共 297 条 warning。纲领成功标准「令牌块之外颜色字面量为 0」与完整令牌渗透尚未达成——门禁只防增长。未触碰的遗留界面继续绕过令牌/字阶,已记录的 warning 存量会掩盖新缺陷。修复方向：触及界面时同步清偿并在同一变更里下调基线（`npm run ui:check -- --update-baseline`;eslint 规则清零后提升为 error）。**负责人：Frontend。**
