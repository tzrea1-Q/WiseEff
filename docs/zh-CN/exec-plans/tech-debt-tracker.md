...
- **TD-110（API 模式遗留切片仍带 mock 值）：** 2026-08-13 由重号的 TD-108 改号（桥接 CORS 决策保留原号）。架构审查候选 C2 的主害已落地：`createApiInitialState()` 让 api 模式以空业务切片启动，失败域由 `CLEAR_API_RUNTIME_DOMAIN` 清空并挂横幅。剩余：无 `HYDRATE_*` 通道的遗留切片（`auditEvents`、`developers`、`logAdminUsers`、`persistedConfigSnapshot`）在 api 模式仍携带 mock 值；显式 `idle/loading/ready/empty/error` 分区目前只覆盖看板与节点调试/用户权限两页；mock 播种仍在 `mockData.ts` 顶层而非 mock 适配器内部。修复方向：给遗留切片定归属（补 hydrate 通道、按 `AuditCenterPage` 方式做 api 隔离、或退役），触页时推广分区模式，mock 播种下沉进 mock 适配器。**负责人：Frontend。**
- **TD-109（mock 适配器手抄服务端规则）：** mock 仓库逐字重复 `server/modules/` 的业务状态机（定义生命周期、候选激活门禁、身份映射开闭，五组错误文案逐字对应），且抛裸 `Error` 而非 `WiseEffApiError`，导致前端所有 `error.code` 分支在 mock 模式失效。架构审查候选 C6，是 TD-001 漂移风险的具体形态。修复方向：共享状态机沉入 `src/domain/`（`submitParameterRound`/`canPerform` 的复用模式已证明接缝可行），mock 薄化为「领域规则 + 内存存储」并统一错误信封。**负责人：Frontend。**
