...
- **TD-095（CI 波动，/logs 视觉已收，checkpointer 仍开放）：** **2026-08-15 部分关闭**（`fix/td095-logs-visual-flake`）。`/logs` 视觉抖动是 API 空切片水合与截图抢跑（空态 vs M2 种子失败页），不是默认失败页上的相对时间。`settleQualityRoute("/logs")` 现等待两条种子文件名与「日志处理失败」；`stableMasks` 覆盖 `.rawlog-table__time` 与元数据采集时间。剩余：`durableCheckpointer.integration.test.ts` 同 commit 双跑一红一绿，需稳定 interrupted-plan 持久化与新实例恢复的时序。**负责人：Agent platform。**
