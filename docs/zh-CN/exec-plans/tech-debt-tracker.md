...
- **TD-111（高频页之外 raw error.message 残留）：** 美学纲领 P2a 明示递延（FA-17 残余）且仍然存在：高频页面之外约 23 处渲染点直接显示 `error.message` 而未走 `presentError`——知识库（`KnowledgeAdminPage`、`KnowledgePage`）、产品反馈后台（`FeedbackAdminPage`、`FeedbackAdminDrawer`、`FeedbackDialog`）、parameter-admin-next 治理面板、parameter-topology 弹窗/托盘。这些界面的每条失败路径都可能把服务端原文（常为英文或内部代号）透给用户。修复方向：触及相应界面时改走 `presentError`/错误映射接缝；评审中拒绝新增 raw `error.message` 渲染。**负责人：Frontend。**
...
- **TD-114（美学纲领组件残留）：** 明示递延的组件残留：死布局层 `WorkspaceHeader`/`PageToolbar` 仍在 `src/components/layout/` 且零消费者（该目录唯一活出口是 `useTopBarActions`）；`MetricBentoCard` 的色调轴仍叫 `severity`（FA-16 递延——其 `variant` 名被可视化类型占用）；`ParameterReviewPage` 剩 2 处 `<Button className=...>` 覆盖。死代码误导导航,变体词汇在同一共享组件上双轨。修复方向：删除死布局组件;下次触及该图表卡时统一 `MetricBentoCard` 轴命名;把两处 Button 覆盖折进变体。**负责人：Frontend。**
- **TD-115（formatLastActive 漏格式化 pg 文本时间戳）：** `formatLastActive` 只识别带 `T` 分隔符的 ISO 输入;用户名录 API 可能返回 Postgres 文本格式（`2026-08-12 04:04:25.984+00`）,落入兜底后在「最近活跃」列原样渲染。2026-08-13 在共享开发库做 FA-25 交互态快照验证时实测发现;CI/种子数据不会命中（种子用户渲染「从未」）。有真实登录活动的部署会在用户表暴露原始数据库时间戳（FA-18 同族泄漏）。修复方向：`formatLastActive`/`lastActiveTooltip` 接受空格分隔的 pg 文本形态（或服务端归一化为 ISO）,并补测试。**负责人：Frontend。**
