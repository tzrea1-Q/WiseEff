...
- **TD-042（参数身份 cutover）：** Phase 7 已修好只读 dry-run、语义匹配、切断 active↔legacy PPV FK、临时库 post-cutover API smoke。本地脏库 dry-run 现为 193/193 defs + 557/557 values、blockers=[]。Cutover 工作流 review（Tasks 1–10）已落地 cutover 后语义活动路径、诚实 inferred 计数与无 DB 绕过的拓扑验收。**第四轮**（分支 `fix/parameter-topology-round4-review-blockers`）已落地：有效厂商 dt-schema、`stage-review`→`finalize`、精确 occurrence 锁定合入/回写、matcher override + 审核 `blocker_scope`（含 locator 指纹）、manifest 回填与 `needs_review` 失败关闭门禁、hotspot 全局厂商规格、未匹配 `createSpec` + `confirmPropertyMismatch` 与审计、黄金计数 **173/519**。**第五轮**（分支 `fix/parameter-topology-round5-review-blockers`）已落地：不可变 base/candidate binding revision、语义合入/回写真 fail-closed（须 objectStore + 工具链；无 `WISEEFF_WRITEBACK_SKIP_TOOLCHAIN`）、`parameter_identity_migration_phases` 不可变 phase 审计与 `migration_run_id` 任务关联、租户作用域 resolve、手工规格 draft→激活→resolve、诚实验收 fixture（`acceptanceTaskLookup`、`semanticFixtureCleanup`）。**缺少合法干净非客户快照与维护窗口**，未执行 apply→cutover→整库恢复演练，**仍为 BLOCKER，不得宣称生产 cutover 就绪**；第四至第六轮仅为实现证据。第六轮另含 0058 evidence-only scope 校正、无损规格 ID、全局激活权限、完整 valueShape、submit→review→merge 验收、租户 cleanup 与 test:all 隔离——仍非生产 cutover 就绪。
