...
- **TD-042（参数身份 cutover）：** Phase 7 已修好只读 dry-run、语义匹配、切断 active↔legacy PPV FK、临时库 post-cutover API smoke。本地脏库 dry-run 现为 193/193 defs + 557/557 values、blockers=[]。Cutover 工作流 review（Tasks 1–10）已落地 cutover 后语义活动路径、诚实 inferred 计数与无 DB 绕过的拓扑验收。**第四轮**已落地有效厂商 dt-schema、`stage-review`→`finalize`、精确 occurrence 锁定合入/回写、matcher override、manifest fail-closed 与黄金计数 **173/519**；**第五轮**已落地不可变 base/candidate binding revision、语义合入/回写真 fail-closed、不可变 phase 审计、租户 resolve 与诚实验收 fixture。**第六轮**现自动创建带 marker 校验的可丢弃 cutover 后数据库，并跑通 submit→review→merge→writeback→reload；同时完成 0058 evidence-only scope 校正、无损持久化手工规格身份、全局激活权限、完整 valueShape、真实角色 UI、租户 cleanup 与 test:all 隔离。该空 legacy 合成验收库不是生产近似快照，也不是恢复演练。**缺少合法干净非客户快照与维护窗口**，仍未执行 apply→cutover→整库恢复→旧 API smoke，因此 TD-042 继续为 BLOCKER，不得宣称生产 cutover 就绪。
