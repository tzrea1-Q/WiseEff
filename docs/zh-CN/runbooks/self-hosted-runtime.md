...
- `--parameter-data-mode new-empty` 且已 bootstrap 夹具 `crel_acme_1` 之后，仅当 `catalog_activation_receipts` 仍为空时，用 `schemas/dts/catalog.json` 编译后继包并以 `install-catalog-release.ts --mode advance` 安装。这是未接管实例的历史 D1 合同。一旦存在任何激活 Receipt，同一 bootstrap/advance 入口会被拒绝；之后的 vendor 复用走 CP-09 import adapter 与 CP-07 授权/任务/manager。禁止再次 bootstrap、禁止 seed、禁止向 Catalog 表 INSERT。D1 命令见[厂商 Catalog 后继发布](../../../ops/self-hosted/upgrade.zh-CN.md#厂商-catalog-后继发布)。
