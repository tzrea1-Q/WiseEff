...
- 企业服务器只能通过代理获取构建依赖时，先由部署用户执行 `./scripts/build-network.sh init`，编辑权限为 `0600` 的 `.build-network.env`，再运行 `./scripts/build-network.sh status`。setup/upgrade 会自动使用同一契约；完整安全与 Docker daemon 边界见[受限网络构建配置](../../../ops/self-hosted/upgrade.zh-CN.md#受限网络构建配置)。自托管 runtime 要求 Docker Compose v2。
