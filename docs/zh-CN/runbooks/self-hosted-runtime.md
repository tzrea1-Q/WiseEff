...
- 只有 IP、没有域名时走 [IP 实验室 profile](../../../ops/self-hosted/ip-lab.zh-CN.md)：在 `ops/self-hosted/` 执行 `./scripts/deploy-ip-lab.sh --ip <server-ip>`。不要把 `.env.example` 当实验室安装向导。该路径不是试点/发布证据。
