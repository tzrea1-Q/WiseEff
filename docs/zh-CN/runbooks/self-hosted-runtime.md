...
- `--parameter-data-mode new-empty` 且已 bootstrap 夹具 `crel_acme_1` 之后，用 `schemas/dts/catalog.json` 编译后继包并以 `install-catalog-release.ts --mode advance` 安装。禁止再次 bootstrap、禁止 seed、禁止向 Catalog 表 INSERT。命令见[厂商 Catalog 后继发布](../../../ops/self-hosted/upgrade.zh-CN.md#厂商-catalog-后继发布)。
