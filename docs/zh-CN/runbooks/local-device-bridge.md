...
仓库提供的自托管运行时只支持一个 API 副本。Compose 必须通过 `ops/self-hosted/scripts/compose` 执行；API scale 只允许精确的 `api=1`，其他所有 `api=*` 值都会在调用 Docker 前被拒绝，其他服务的扩容参数仍正常透传。
...
- macOS `.pkg` 安装包通过 postinstall 注册 `~/Library/LaunchAgents/com.wiseeff.bridge.plist`；便携包需手动执行 `wiseeff-bridge service install`（`/node-debugging` 界面不再提供该入口）。
