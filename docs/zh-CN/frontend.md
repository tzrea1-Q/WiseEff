...
- `src/infrastructure/mock/`：mock state 和 mock repository/gateway。失败经 `mockApiError` 抛 `WiseEffApiError`，应用层 `error.code` 分支在 mock 与 API 模式同样生效（TD-109 第一波 / #475）。
