...
API 模式 `/parameters` 保留成熟的 `ParametersPage`/`WorkbenchLayout` 层级，在 `ApiProjectTopologyWorkspace` 内嵌 `DtsParameterWorkbench`。协调器继续负责真实 API 加载，语义行、可折叠 DTS 导航、本轮修改区、类型化详情弹窗和 binding 提交面板则融入原有工作台。这不是只展示拓扑的替换：导航器负责筛选主列表，并保留原有搜索、焦点、详情、草稿和审核交接心智。`amba → i2c@FDF5E000 → sc8562@6E → gpio_int` 使用真实父子关系展示。浏览固定为生效拓扑；源文件/节点/行号与来源链保留在列表行和详情中，不再提供顶栏源/生效整页切换。桌面行展示属性、器件/实例、完整 DTS 位置（含源出处）、raw/effective 值、值形状及 schema/policy/mapping 治理状态，平板/手机堆叠这些字段并在详情 sheet 展示 provenance。搜索覆盖属性、compatible/驱动、地址、完整拓扑路径、源文件/节点路径和 raw 值；清除全部筛选不会清掉待提交草稿。
