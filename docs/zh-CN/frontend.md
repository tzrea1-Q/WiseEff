...
开发环境显式设置 `VITE_PROJECT_CONFIGURATION_WORKBENCH_ENABLED=true` 后，项目清单入口会打开 `/parameter-admin/projects/:projectId/configuration`。该 Phase 1 路由是全屏只读 tracer：配置集/文件 query 选择 Working configuration；成员/未编组身份与 active source 在 mock/API 两种 runtime 中均经既有 `DtsStructuredRepository`、`ParameterFileRepository` 读取；候选与发布写操作保持禁用。源码定位导航（#229）在选中成员下嵌套结构节点树，经 `ProjectPrimaryDtsViewer` 滚动/高亮 CST span，提供按文件分组的统一搜索（省略 `by`=全维度）并可跨成员跳转，恢复 `configSet`/`file`/`node`/`property`/`sourceMode` 深链；树元数据与源码独立加载/重试。键盘辅助使用 Alt+F / Alt+N / Alt+G / Alt+1 / Alt+2（非输入时 `/`），不覆盖浏览器/系统快捷键。生产构建强制关闭该 flag，flag 关闭时四视图旧弹窗仍可访问。
