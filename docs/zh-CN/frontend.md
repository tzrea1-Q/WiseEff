...
API 模式 `/parameters` 只渲染 `ApiProjectTopologyWorkspace` 和 binding 草稿提交面板。遗留 `ParametersTable`、推荐值漂移文案、用 recommended value 初始化草稿、扁平详情与扁平导出均仅限 mock。类型化编辑必须填写原因，保留 API 返回的 binding/spec/candidate 身份，通过正式 submission API 提交后，由指定角色在 `/parameter-review` UI 推进。
