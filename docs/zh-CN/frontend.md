...
**定义库治理（`/parameter-admin/specs`）：** `OrganizationSpecGovernancePanel` 承载 `ParameterSpecLibrary` 与内嵌 `SpecReviewQueue`。**新建定义**打开 `SpecCreateDialog`（驱动组/节点类型 `attributionSubjectId`、必填 `propertyKey` + `reason`，可选 compatible 以便创建后带 `coverageClaim` 激活），调用 `POST /api/v2/parameter-specs`。**软废弃/恢复**经 `ParameterSpecDetailDialog` 原因门禁（`POST .../deprecate`、`POST .../restore`）；成功反馈为面板内 `form-hint` 状态行（非 toast）。独立 `ParameterAdminAuditBanner` 已移除——各面板经 `ParameterAdminProvider` 推送紧凑审计提示（`PUSH_AUDIT_HINT`）；项目运营面板仍可内联展示最近一条。**节点对应**（`/parameter-admin/identity-mapping`）由 `IdentityMappingReview` 提供**确认对应** / **驳回**（`resolved` / `dismissed`）；后端 `new-identity` + `confirmAllCandidates` 与 `singleton-cardinality` 阻断尚未在后台 UI 暴露，但校验/发布仍 fail-closed。
