...
- 端口 `KnowledgeRepository`:mock 用 `src/infrastructure/mock/mockKnowledgeRepository.ts`（fixtures 覆盖草稿/已发布/已归档、提取失败文件与两条 Agent 草稿队列态,并模拟索引状态,端口形状一致）;API 用 `src/infrastructure/http/knowledgeClient.ts` 对接 `/api/v1/knowledge/*`（含 `distill-from-log`、`related-to-log`、`entries/:id/reject`、`index/status`、`index/rebuild`、`entries/:id/index/retry`）。过期保存映射为 `KnowledgeRevisionConflictError`,编辑器渲染为可读的刷新重试冲突提示,绝不静默覆盖。
