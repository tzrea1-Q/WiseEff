...
- `exec-plans/active/2026-07-27-dts-node-enablement.md`：把 DTS `status` 当作节点启用状态而非参数 —— 结构键单一事实来源以解除发布闸门阻塞、派生启用与可达性、拓扑树与工作台可见、三态编辑接入共享草稿管线（分支 `feat/dts-node-enablement`；ADR-0003；中文对照 `docs/zh-CN/exec-plans/active/2026-07-27-dts-node-enablement.md`）。
