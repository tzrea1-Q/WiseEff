...
- `exec-plans/active/2026-08-05-project-operations-dialog-hardening.md`：加固被 PR #224 搬进弹窗的项目运营界面 —— **POD-D1 已于 2026-08-05 定案：把四个视图还原为整页路由，重申 ADR-0001**，同时保留 PR #224 的项目清单滚动修复与现有深链；为仍然保留的弹窗建一套共享原语（焦点陷阱、初始焦点与归还、背景惰性化、仅最顶层响应 Escape、遮罩关闭成对判定、统一 z-index 阶梯）；为完全无样式的 `StructuredValueEditor` 补齐样式；给基线发布/回滚与冲突裁决加确认与强制拦截；从四个视图移除教学/mock 资产与裸内部值（分支 `feat/project-operations-dialog-hardening`；负责提供 `2026-08-03-parameter-spec-editor-fidelity.md` 所消费的共享弹窗原语）；英文对照同路径。
