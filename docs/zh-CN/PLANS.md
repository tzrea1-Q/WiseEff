...
- `exec-plans/active/2026-08-18-property-key-source-cutover.md`：方案 + preview 已由 #544 合入；start + finalize 已由 #549 合入（迁移 `0113`）；prepare 暂存文件候选草稿 + 定义编辑器面板已由 #553 合入。**TD-117 保持 Open。** 日常作业仍须在配置工作台把候选合入现行源后再 finalize。`referenceCount > 0` 时不要启用行内改名。
