...
- `exec-plans/active/2026-08-12-frontend-aesthetics-uplift.md`：前端美学提升纲领——令牌单一来源地基、原语收敛（Button/Dialog/Toast/表格/状态）、页面缺陷清扫（本地化泄漏、布局预算、移动端抽屉）、动效与主题就绪、`ui:check` 质量门禁；标准见 `design-docs/ui-design-system.md`，门禁见 `developer/ui-quality-checklist.md`（阶段分支自 `fix/ui-foundation-tokens` 起）。
...
- `exec-plans/active/2026-08-12-knowledge-base-mvp.md`：组织级 agentic 知识库 MVP——扁平多标签的 markdown/文件知识条目 + wiki 式轻治理生命周期、仅已发布内容进入混合检索（pgvector + 全文检索降级，ADR-0025）、小泽知识工具带引用溯源、审批门控的 Agent 草稿、日志结论一键沉淀（分支 `feat/knowledge-base-foundation`、`feat/knowledge-base-rag`、`feat/knowledge-base-distillation`；设计文档 `docs/design-docs/2026-08-12-knowledge-base-design.md`）。
...
- `exec-plans/active/2026-08-12-xiaoze-turn-stream.md`：把小泽回合流式管线深化为单一 `xiaozeTurnStream` reducer 模块，持有"已经流出了什么"的唯一一份状态——吸收帧构造、回合状态跟踪与最终回复 resync；resume 分支并入同一 open/ingest/finalize 路径，批准回合从此流出运行步骤（关闭 TD-070）；线协议不变（分支 `refactor/xiaoze-turn-stream`；中文伴随页 `docs/zh-CN/exec-plans/active/2026-08-12-xiaoze-turn-stream.md`）。
