...
- `exec-plans/active/2026-08-13-knowledge-log-recommendations.md`：日志分析结果的相关知识推荐（知识库延后路线图第 1 项）——`GET /api/v1/knowledge/related-to-log` 从存储的结论/影响文本推导相似度查询，经现有混合检索并施加相关度截断（仅已发布条目，`knowledge:view` + `logs:view` + 组织隔离），日志结果页新增带引用深链与诚实检索模式说明的「相关知识」区块（分支 `feat/knowledge-log-recommendations`；KB-REC-001）。
