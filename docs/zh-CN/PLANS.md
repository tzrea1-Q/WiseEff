...
- `exec-plans/active/2026-08-18-property-key-source-cutover.md`：方案 + preview 已由 #544 合入；start + finalize 已由 #549 合入（迁移 `0113`）。**TD-117 保持 Open。** prepare 不暂存草稿/CR，也无启动作业 UI。`referenceCount > 0` 时不要启用行内改名。
