...
- `exec-plans/active/2026-08-20-self-hosted-one-command-upgrade.md`：成熟的源码 checkout 升级入口——锁定唯一目标、停机前构建、停止队列/写入、验证 PostgreSQL/对象存储/Redis 恢复点、保留数据地完整重建服务、健康门禁、继续与显式恢复。设计完成，等待在 `feat/self-hosted-one-command-upgrade` 实现。
