...
- `exec-plans/active/2026-07-30-attribution-subjects-and-versioned-specs.md`：归属主体 + 版本化参数定义 —— PR0–PR6 已在 `feat/attribution-subject-versioned-specs` 落地（ADR-0013/0014）；不按原样合入 #212–#214。
- `exec-plans/active/2026-07-31-attribution-governance-follow-up.md`：PR0–PR6 之后已锁定的收口 —— cutover finalize HTTP+UI（PR7）、身份映射 UI（PR8）、toast/软废弃默认/仅 overlay claim/主体主展示/性质基数只读（PR9）。待讨论见 `docs/design-docs/2026-07-31-attribution-governance-deferred-questions.md`（中文对照 `docs/zh-CN/design-docs/2026-07-31-attribution-governance-deferred-questions.md`；计划摘要 `docs/zh-CN/exec-plans/active/2026-07-31-attribution-governance-follow-up.md`）。
- `exec-plans/active/2026-07-30-parameter-governance-state-machine-completion.md`：补齐参数管理后台的状态机 —— 参数定义的「废弃」定义为软下线（仍参与解析，只失去审核可选性与默认可见性）并可恢复到 `activated_at` 指示的原状态、节点对应任务的「驳回」拆成「驳回候选」与「确认为新身份」使全部驳回不再永久锁死修订、config revision 的 `published` 退役（发布只发生在文件层的发布基线）、overlay 的动作改名「停用解析」以免与「废弃」语义相撞、并接上未归类恢复 / overlay 停用 / 模块排序这些已有 API 但缺入口的操作（分支 `feat/spec-lifecycle-closure`、`feat/identity-mapping-decision-split`、`feat/parameter-governance-convergence`；迁移 `0081`–`0083`；ADR-0011、ADR-0012）。遗留问题与事实依据见 `docs/design-docs/2026-07-30-parameter-governance-deferred-questions.md`。
