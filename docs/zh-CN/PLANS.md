...
- `exec-plans/active/2026-08-13-knowledge-parameter-references.md`：参数定义与知识条目的结构化引用（知识库延后路线图第 2 项）——`knowledge_parameter_references` 把条目绑定到 `parameter_specs.id` 代理键（ADR-0017 身份纠错与 ADR-0011 废弃后均存续），条目编辑器内编辑权限门控的引用编辑并审计添加/移除，知识侧定义 chips 带如实「已废弃」徽章，定义详情新增仅已发布的「相关知识」列表，`knowledge.getDocument` 报告被引用定义（分支 `feat/knowledge-parameter-references`；KB-XREF-001）。
