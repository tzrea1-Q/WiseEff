...
本地账号注册会在评估组织（有种子时是 ChargeLab，否则是唯一的 bootstrap Organization）中按允许自助选择的平台角色创建基于用户名的账号。操作者可用 `AUTH_LOCAL_SELF_REGISTER=false` 关闭自助注册：API 拒绝 `POST /api/v1/auth/register`，认证页隐藏「注册」。登录与注册共用进程内滑动窗口限流（`AUTH_LOCAL_AUTH_MAX_ATTEMPTS` / `AUTH_LOCAL_AUTH_WINDOW_MS`），超出返回 `RATE_LIMITED`（HTTP 429）。失败登录写入 `auth-event` / `login-failed`（用户名未知时 actor 与 target 可为 null）。当前用户改密会校验当前密码、更新 salted `scrypt` 哈希、保留本次会话并吊销其它会话；Admin 重置密码需要 `users:manage`，并吊销该用户全部会话。修改组织显示名称需要 `users:manage`，并写入 `organization-update` 审计。服务端会拒绝 Admin 自助注册；Hardware/Software Committer 注册申请会创建 inactive 账号、对应基础 User 角色和待审批申请，但不会发放 session token，也不能在 Admin 审批前登录。Admin 在用户治理后台批准后，服务端才会激活账号并授予申请的 Committer 角色。当前暂不支持邮箱验证，因此注册不能被当作邮箱域名归属证明或邀请接受流程。在 `NODE_ENV=development` 下，`db:seed:m0` 可为 ChargeLab 演示 persona upsert 固定 username 与仅限开发者的共用演示密码，便于本地按角色测 UI；非 development 的 seed 不得写入这些演示凭据，演示密码也绝不能用于生产或客户库。浏览器本地账号 token 当前保存在 `localStorage` 的 `wiseeff.localAuthToken`；需要 SSO、MFA、refresh-token rotation 或更强浏览器会话隔离的部署应使用 OIDC 或经过加固的反向代理/session 集成。

**owned 验收子进程与证据边界：** Gate0 phase 与 nested disposable runtime 不得整体继承 `process.env`。child 完整环境只能由少量 OS 启动 allowlist 加本轮精确 owned 变量重建；PostgreSQL 密码、Docker/Git 凭据 helper、CI job JWT、provider key、authorization 以及通用 token/secret/password 变量，只有被 owned runtime 明确定义时才允许跨边界。本轮 root 与 nested 生成的凭据通过仅 loopback、带 token 认证的 sink 汇集，只有加密 exact-value 上下文持久发布后才 ACK。root owner、visual/browser phase 以及 root/nested API/frontend 都持久化不含凭据的 process-start identity。owner 崩溃后，fresh CI finalizer 先证明 owner 已退出，再只对 identity 匹配的 writer 逐次复核并发 TERM/KILL；数据库和对象根继续留作失败取证。随后它在私有 staging 中脱敏、扫描，并原子发布一个已扫描的只读 ZIP；CI 只上传该 ZIP。nested ownership manifest 始终不含秘密。owned operation evidence 只能指向 runRoot 下真实存在的普通 report/trace 后代，并哈希不可变 runtime snapshot，不能哈希 phase/cleanup 字段仍在变化的 descriptor。
...
**规范参数目录数据库角色（S2-RBAC）：** Catalog 关系位于 `parameter_catalog`，由 NOLOGIN 角色 `catalog_migration_owner` 拥有；该角色同时持有 Catalog 内部引用所需的 public 外键目标表 `SELECT`、Catalog `SECURITY DEFINER` 函数（含 `parameter_catalog.resolve_legacy_identity_owner`）读取的全部 public 源表 `SELECT`，以及对 `public.parameter_modules` 的 `UPDATE`，以便 SECURITY DEFINER placement 锁执行 `SELECT ... FOR SHARE`。`catalog_synchronizer_role`（NOLOGIN）只能插入不可变 Catalog 行，对当前 release / Definition head 做列级更新（`catalog_state.current_catalog_release_id`、`parameter_definitions.current_revision_id`），以及对 CHECK 身份谓词和 `parameter_catalog.acquire_current_pointer_lock_exclusive()` 的 `EXECUTE`。它不获得仅供触发器使用的函数的 `EXECUTE`。它可以 `INSERT` `parameter_catalog.catalog_activation_receipts`，并对核验 tuple 所需的 `catalog_publication` 关系 `SELECT`；不能插入 Artifact/Candidate/Job/Authorization，也不能 `EXECUTE` `catalog_publication.revise_publication_policy`。`parameter_governance_writer_role`（NOLOGIN）仅有必要的组织治理 DML、对 `public.audit_events` 的追加 `INSERT`，以及只对 `parameter_catalog.assert_catalog_subject_active(text,text,text,text)` 的 `EXECUTE`。它没有 Catalog 表的 `SELECT` 或 DML，也没有 Binding、Cutover、Verification 或 publication schema 授权，不能写 Receipt，也不能 `EXECUTE` CHECK 谓词、排他锁或仅供触发器使用的函数。延迟约束 `assert_subject_placement_kind` 与 `assert_observation_match_binding_revision` 为 `SECURITY DEFINER`，因此 writer 插入无需 Catalog / Binding 表授权即可触发它们。`catalog_publication` 是由 `catalog_migration_owner` 拥有的独立 schema，`PUBLIC` 已撤销。`catalog_publication_coordinator_role`（NOLOGIN）可插入 Artifact、Candidate、Authorization 与 Job，对 Job 执行列做列级更新，`UPDATE` `publication_guard`，`SELECT` `publication_freeze`，并 `EXECUTE` `catalog_publication.set_publication_freeze` 与 publication 守卫锁；不能 `EXECUTE` `catalog_publication.revise_publication_policy`（CP-04 的 TypeScript 管理封装会记录可信 actor，仍不向 coordinator GRANT EXECUTE；隔离测试仅 `SET ROLE catalog_migration_owner`），不能写 Catalog 核心表或 Receipt，也不能 `SET ROLE` 到 synchronizer 或 migration owner。`catalog:author` / `catalog:publish` / `catalog:review-high-risk` 是封闭联合中的能力，测试通过 `AuthContext.permissions` 注入；默认 Admin / org-admin / Agent 矩阵保持目录只读。请求体中的 `role` / `organization` / `riskClass` / `approved` 不可信。默认 `publication_policies` 保持 `publication_enabled=false` 与 `low_risk_single_actor_publish=false`；这不是生产启用。隔离测试只能在临时库名上用 `EPHEMERAL_POLICY_REVISION_CONFIRMATION` 启用。正式实例走带观测数据库 OID、当前 Catalog id/digest、策略版本、freeze 与接管引脚的 check/execute 路径；启用不清除 freeze，不要求先成功发布，也不默认打开低风险单人发布。封装仍不向 coordinator `GRANT EXECUTE` `revise_publication_policy`。`catalog_baseline_reader_role`（NOLOGIN）仅对 Catalog 与 publication 关系 `SELECT`。普通 Web/API LOGIN（`wiseeff_api`，或实验室按 run 作用域生成的等价 LOGIN）**自身没有 Catalog 或 publication 写授权**（本代码库没有 `application_role`）。公共读路径是对该 LOGIN 授予与 baseline reader 相同的 Catalog/publication `SELECT`（无 `INSERT`/`UPDATE`/`DELETE`、无序列 `UPDATE`、无 writer `EXECUTE`）。该 LOGIN 以 `NOINHERIT` 成为 `catalog_baseline_reader_role`、`parameter_governance_writer_role` 与 `catalog_publication_coordinator_role` 的成员，以便发布 persist/enqueue/freeze 组合入口 `SET LOCAL ROLE` coordinator，以及 Parameter Governance 组合入口 `SET LOCAL ROLE` writer。可信治理写入只授予 `GOVERNANCE_RELATIONS`（草稿、提案、观测、placement、审阅项、publication intent），不包括 Catalog 核心、head 或 Receipt。已发布项目 Binding/Value 写入（`parameter_catalog.project_parameter_bindings`、`project_parameter_values`、`binding_history_events`）是 API LOGIN 上的工作台正式值入口，不是 Catalog 核心不可变面，也不是 synchronizer 物化。它不是 `catalog_synchronizer_role` 或 `catalog_migration_owner` 的成员，也不能 `SET ROLE` 到它们。Worker LOGIN（`wiseeff_worker` 或实验室等价角色）不是任何 Catalog/publication 角色的成员，也没有这些 schema 授权。Publication manager LOGIN 仅以 `NOINHERIT` 成为 coordinator 与 synchronizer 的成员；没有直接表 DML，也不是 migration owner 的成员。Application、Agent 与 verifier 的 CONNECT-only 登录仍不是这些角色的成员，也不能 `SET ROLE`。`PUBLIC` 在 Catalog 与 publication 的 schema、表、序列和函数上保持撤销。API/worker LOGIN 对 Catalog 核心（Subject/Definition/Revision/Release/head/Receipt）做 `INSERT`/`UPDATE`/`DELETE` 会以 SQLSTATE `42501` 失败，归类为 `PCAT-DB-P01` / `PCAT-PRIV-CATALOG-IMMUTABILITY-BYPASS`。未 `SET ROLE` coordinator 的 publication 写入同样是 `42501`。这些角色对 public 参数表的遗留结构写入会以 SQLSTATE `42501` 失败，归类为 `PCAT-DB-P02` / `PCAT-PRIV-LEGACY-WRITER-BYPASS`。只有 Parameter Governance composition root 持有可 assume governance writer 的连接池。
...
**可信调用溯源决策（ADR-0038 / TD-068）：** `AuthContext` 继续表示已认证、可问责的用户主体；普通 bearer credential 不证明背后有物理在场的人。#610 已实现共享的服务端内部 `user` / `agent` / `system` 判别联合上下文、严格的服务端构造器，以及必填的策略/审计 seam。Agent 上下文携带 session/tool-call 关联；需审批的写入执行时还必须带 approval id。审批只授权动作，不会把 initiator 改写成 `user`。system 须点名 service/job，绝不伪造用户。客户端 header/body 不能声明该上下文，缺失时不得默认 `user`；human-required 策略只接受 `user` initiator。在源码基线 `67d4a77325b6009b77c2373bd788298a6d022bcf`，`agent/orchestrator.ts` 已构造 Agent 调用上下文，`dts-reload/routes.ts` 已为重载变更路由构造用户调用上下文，领域服务会验证该上下文。修改生产写路径时仍须逐条检查传递链与聚焦测试。本源码说明不关闭整个 TD-068 计划，也不产生发布证据；legacy 调用方传入的 `actorType` 不是可信溯源边界。mock 不构成安全证明。
...
**Xiaoze P1 行动：** `action.submitParameterChange` 为 mutating 且 approval-gated。AG-UI runtime 开启 orchestrator 自有的 Agent 审批链——`beginApproval` 持久化 tool-call + approval 记录并发出 interrupt——且仅通过 `resolveApproval` 恢复，在事务内重新鉴权、审计 `actorType=agent`。审批状态以数据库行（`agent_tool_calls` + `agent_approvals`）为唯一载体、绝不驻留进程内存，因此 begin 与 resolve 跨重启和多副本仍然正确（ADR-0024）。批准前由服务端将 canonical Binding、DefinitionRevision、ProjectValue、配置版本和来源 pin 固定到 durable tool payload。人工编辑替换提议参数，但保留原基线；范围变化或基线漂移必须拒绝。执行复用 `createCanonicalValueDraft` 与 `submitCanonicalValueChange`，registry 绑定批准事务；失败时用 savepoint 一并回滚来源准备、草稿、请求及其成功审计。这里只产生 pending 请求，当前 ProjectValue 须经独立产品审核后才改变。来源准备前仍执行敏感节点守卫：critical Agent 写入立即拒绝（`403`、`requireHuman: true`），不产生草稿或请求。设备写闸门在 P1 仍由调试界面与后端拥有，不在小泽内执行：独立高风险写入仍用 `confirm-high-risk-write`。当 Agent 驱动的设备工具启用时，必须先落 `agent_approvals`，再把该 `approvalId` 传入调试 write/rollback；调试服务会复核该行已批准且匹配本次 tool/write，任意字符串一律拒绝。

经批准的 Agent 参数操作仍保持 Agent 身份。durable approval 仅允许 canonical 来源准备、草稿创建和提交待审核请求；source commit 与产品审核仍只允许 User，Agent 批准不授权正式值写入。凭证必须绑定认证用户、session、tool call、人工决定、项目、Binding、批准目标值和精确 canonical 基线。设备写入保留独立批准边界。
