...
生效驱动定义身份由服务端掌握：规范 `AttributionSubject`、schema 关联、组织放置、生命周期优先级和 active 版本基数，绝不接受客户端展示名称、实测模块名或调用方指定的优先级。未知或歧义 DTS 证据只保留审核/出现证据，不能创建已识别 binding。迁移 `0117`/`0118` 拒绝新的不完整 active DTS 属性写入，`0120` 仅定义旧版未链接暂存写入的兼容边界，`0121` 修正 nodename-only 主体 taxonomy；对账修正使用可信 system/job 溯源和带审计事务，`parameter-definitions:check` 独立在主体缺失、active 定义/版本重复、放置缺失或 binding tip 未审核时阻断发布。运维必须遵循[生效目录运行手册](runbooks/effective-driver-parameter-catalog-reconciliation.md)中的快照与恢复边界。
