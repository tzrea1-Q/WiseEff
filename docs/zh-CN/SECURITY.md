...
**可信调用溯源决策（ADR-0038 / TD-068）：** `AuthContext` 继续表示已认证、可问责的用户主体；普通 bearer credential 不证明背后有物理在场的人。#610 已实现共享的服务端内部 `user` / `agent` / `system` 判别联合上下文、严格的服务端构造器，以及必填的策略/审计 seam。Agent 上下文携带 session/tool-call 关联；需审批的写入执行时还必须带 approval id。审批只授权动作，不会把 initiator 改写成 `user`。system 须点名 service/job，绝不伪造用户。客户端 header/body 不能声明该上下文，缺失时不得默认 `user`；human-required 策略只接受 `user` initiator。Xiaoze 入口重建、DTS 重载五条写路径及全部参数敏感生产写的传递仍由 #611–#615 负责；这些迁移完成前，legacy 调用方传入的 `actorType` 闸门仍不能视为可信溯源。mock 不构成安全证明。
