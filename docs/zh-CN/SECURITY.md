...
- **生产失败关闭发布**：release 模式工具链校验（`dtc` + `fdtoverlay` + `dt-validate`）不得绕过。绕过触发 critical 告警 `WiseEffConfigPublishValidationBypass`。语义身份切换仅限维护窗口且只能整快照恢复——见 `docs/runbooks/parameter-identity-cutover.md`。迁移证据保留遗留 ID/值，但不得把 `recommended_value` 自动提升为 schema default 或 policy。
