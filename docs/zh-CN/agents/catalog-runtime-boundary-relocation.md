...
种子重建需要保留旧定义，同时保留已持久化驱动版本的连续性证据。初始实现新增了旧表直接读取，也使 ingest 整文件身份校验失效。修正复用 `getParameterSpecRow`：内部指定版本选项在已有驱动版本连接中按精确 ID 过滤，并返回持久化 ID。普通定义响应与 upsert 行为不变。缺失、跨组织或属于其他驱动根的版本不能作为连续性证据；请求旧版本时也不会被最新版本替代。

本次有界身份决定保留原始清单、全部 allowance 分片、可信基点、历史记录和校验器规则。当前 source-workflow 记录只重新绑定 20 个 ingest 目标，保留全部原始切片与元数据。记录摘要为 `b998321716d00d58ea83b03cd283a52bafb40ac40437549ee453e7153c83742c`，目标 blob 为 `76a25e2a1a776b4904eac61a446b5227729485bd`。

owner 仓库仍有全部 114 个出现位置。两份独立固定记录把 61 个移动的原始身份从 blob `1a0afbc098f46218eda2759a70b4dd15f1ee391a` 绑定到 `bcc76d7d8409c24adcc44b6871fc33eb1e5da5a0`：50 对仅位置变化，要求切片、evidence、column、锚点和字节顺序不变；11 个出现位置属于同一条修改后的查询，复用既有重写切片校验器，绑定源与目标切片的精确摘要。两份记录摘要依次为 `47908781431317f8c0a61d50d4c3d8deaf21ec09990788d6d0e92bf5d713c345`、`73271be0b26fca47e5811695590084059007c6d8b488ad7679e8da2e2b2e484c`。不允许任何新增出现位置。诊断清单仍为 3568 项、3491 项已允许、77 项未允许；新增 61 个精确别名只把别名总数由 697 改为 758，不会把尚未完成的旧迁移判为边界检查通过。

独立 Standards 与 Spec 审查已批准精确数据设计；最终记录在激活前通过了未修改的严格校验器和最新源码扫描。聚焦脚本验证通过 135 项既有测试及 5 项新增证明完整性测试。原生 PostgreSQL 验证通过 25 项操作器/定义测试和 19 项摄入/物化/配置集测试。历史脏归属回归先复现了跨组织版本暴露，增加仅指定版本模式使用的驱动/定义归属一致性条件后通过；普通定义查询保持原状。构建、文档治理和 PostgreSQL schema artifact 检查通过。部署服务器重建与浏览器验证仍需单独完成。
