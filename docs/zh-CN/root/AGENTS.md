...
- 任务/恢复路由：从仓库子目录开始工作时，先确认真实 `cwd`，再按仓库根到 cwd 逐目录明确检查 `AGENTS.override.md`、`AGENTS.md` 和已配置的 fallback 名称；把已选和缺失路径记入[交付控制记录](../agents/agent-delivery-protocol.md)，并只阅读任务包指定的小节。指令继承只来自这条路径；旁侧或模块文档必须在路径中被明确命中。
