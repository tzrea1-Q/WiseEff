...
- 小泽 live provider 由 `XIAOZE_LLM_API_BASE_URL`、`XIAOZE_LLM_MODEL`、`XIAOZE_LLM_API_KEY` 这一 canonical 组三键配置；离线验收使用显式 deterministic 模式。
...
- M6.5 自托管 `observability` Compose profile 由 `ops/self-hosted/scripts/observability` 管理，包含 Prometheus、Grafana、Alertmanager、exporter、自动 provisioning 和全服务 blackbox 探针；API/worker 指标及监控 UI 都只属于受控运维面。
