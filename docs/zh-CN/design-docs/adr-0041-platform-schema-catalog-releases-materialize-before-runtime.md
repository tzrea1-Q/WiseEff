...
[ADR-0040](../../adr/0040-canonical-parameter-catalog-relational-model.md) 已确定上位模型：Proposal 接受只产生 publication intent；只有已发布的不可变 Catalog Release 才能输入 catalog 真相；Catalog Release synchronizer 是 `CatalogSubject`、`ParameterDefinition` 与 `DefinitionRevision` 的唯一数据库物化者。任意持久化 definition 内容变化，包括 documentation，也必须创建新的不可变 DefinitionRevision。
...
后续 manifest 省略条目永远不代表退役。撤回必须使用显式 tombstone，记录稳定身份、既有 selector、最后 current revision、适用时的 successor，以及执行状态转换的 release。退役 subject 或 definition 不再用于新匹配或新选择，但保留稳定 ID、永久自然键、revision、历史重放所需 alias、引用和审计。恢复通过后续 forward release 复用同一身份；definition 内容有变化时创建新 revision。
...
PostgreSQL 是可查询物化结果，不是 authoring source。只有 Catalog Release synchronizer 可以插入 CatalogSubject、ParameterDefinition 或 DefinitionRevision 行，或推进 definition/release head。Proposal、ingest、review、HTTP、Agent、脚本和普通应用服务都没有并行物化路径。
...
- release row、subject、definition、所需的精确 revision、alias、tombstone、materialization fingerprint 与 current pointer 必须作为一个原子领域事务提交；任何错误都保持上一 verified release 与所有既有 Definition head 不变，数据库中不存在可见的部分 candidate catalog；
...
独立 synchronization verification 必须通过与 writer 不同的只读路径重新计算 normalized release 与数据库 fingerprint。它要证明：bundle 和 lineage 有效；每个发布对象恰好物化一次；所有 Definition head 与 release 一致；documentation-only revision 未改变 Binding/ProjectValue cutover 状态；alias 与 tombstone 一致；current pointer 指向完整且已验证的 release；不存在 organization-owned catalog object；runtime cache 可从该 release 构建。Project binding、HTTP、浏览器、可观测性、自托管端到端和最终 legacy 删除验收仍由 [Choose verification, upgrade, and legacy-retirement gates](https://github.com/tzrea1-Q/WiseEff/issues/679) 决定。

### 7. 失败、回滚和历史永不改写发布真相

Current pointer 切换前，synchronization 失败会留下零项 catalog 领域变化，并保持上一 verified release 为 current。切换后、public traffic 或 candidate business write 之前，只有独立验证证明上一 snapshot 完整、数据库迁移仍兼容且没有 candidate write，operator 才能切回；否则恢复已记录的 recovery point。一旦发生 candidate business write 或 public traffic，禁止 pointer-only rollback。Operational data recovery 由 [Choose populated-data cutover, archive, and rollback strategy](https://github.com/tzrea1-Q/WiseEff/issues/678) 决定。
...
历史业务值固定其精确 DefinitionRevision；ingest、matching、alias resolution 和治理证据还要记录产生该决策的 Catalog Release digest。重放已是 current 的 digest 只执行 verified no-op。保留的旧 release 可以确定性物化到全新或隔离的只读 replay projection，或按 digest 加载其不可变 normalized snapshot；它绝不能把生产 current pointer 向后推进。当前 alias、lifecycle、documentation 或 revision 不得重解释历史。生产 replay 不能只依赖 Git history 或网络下载。所需历史 release 缺失、不属于其记录 lineage 或 digest 不符时，replay 必须失败关闭。
...
| Release 校验      | 文件缺失/未列出、文件或 aggregate digest 错误、身份重复、稳定 ID/自然键改绑、alias 冲突、非法 lifecycle/tombstone transition 以及 lineage 缺口都在 catalog 写入前失败。                     |
...
| 失败原子性        | 在任意 release component staged 后、提交前注入失败，不得看见 candidate subject/definition/revision/alias/tombstone，既有 release 与 Definition head 全部不变。                              |
| 漂移              | 未知 installed digest、被修改的物化行、意外 catalog 行或 compiled-model/database fingerprint 不一致会阻止 synchronization 和启动，不静默修复、不降级为空 catalog。                          |
| 退役与恢复        | 无 tombstone 的缺失失败；显式退役停止新匹配，同时保留稳定 ID、自然键、revision、replay 所需 alias、引用和审计；后续 release 恢复时复用同一身份。                                            |
| 历史重放          | 保留的旧 release 在隔离 replay projection 中复现精确 normalized snapshot 与决策且不改变生产 head；历史缺失或 digest 无效时失败关闭。                                                        |
| 回滚              | Pre-traffic switch-back 仅在零 candidate write 且既有状态经验证兼容时成功；发生 candidate write/traffic 后必须拒绝，改走新 forward release 或 recovery-point restore。                      |
