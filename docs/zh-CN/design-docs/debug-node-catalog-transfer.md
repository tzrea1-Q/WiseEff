...
（含归档、禁用、无绑定）和所有 binding，并通过一个一致快照读取：在生产 root pool 上这些读取运行在专用的
`READ ONLY REPEATABLE READ` 事务中；在调用方已开启的事务内则加入其快照。它不受当前表格分页、搜索词、
模块选择、协议筛选或树展开状态影响，响应中的 `counts` 与同一对象集合同源。

目标自身的完整性同样属于该合同。若某节点的 `debug_node_module_id` 在本组织内无法解析、模块父级缺失或成环、
或 binding 找不到对应节点，则该目录无法被完整表示：**导出整次失败**并返回 `VALIDATION_FAILED` 与带定位的
问题清单，而不是生成一份静默丢行或错位的文件。预览与导入会把同样的状况报告为带定位的阻断冲突。
...
`buildCatalogImportPlan` 只校验、匹配并分类一次。节点分类描述的是**节点行本身**（名称与九个节点字段）；
仅 binding 变化在 binding 上分类与计数，因此属性未变的节点会显示为「不变」，紧邻其「更新」的 binding，
而不会被重复计数：
...
重复源 ID、ID 与名称路径命中不同目标、两个文件条目争用同一目标、文件中两个完全重复的新节点、重复 binding 协议、
目标端存在多个同模块同名的节点（歧义匹配绝不靠取第一行解决）、目标模块引用成环或悬空、目标 binding 成为孤儿，
以及目标节点模块引用无法解析。

v2 文件声明的 `counts` 与实际对象集合矛盾时同样阻断：管理员无法判断哪一半可信，预览会给出 `count-conflicts`，
任何提交都不被允许。
