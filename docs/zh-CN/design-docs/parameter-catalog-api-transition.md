...
[ADR-0045](adr-0045-configuration-schema-subject-and-seed-rebuild.md) 记录已确认的配置模型扩展，以及限于指定部署的归档重建替代路线。仅对明确枚举且验证完成的重建，旧链接立即显示经授权的已归档提示；其他部署保留本文正常旧读取窗口。该扩展属于规划，不是已经上线的行为或 S13 完成证据。
