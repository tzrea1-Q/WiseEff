...
## 21. Canonical 来源流程补充（#906）

受 canonical source pin 保护的 DTS/JSON 文件，上传后仍是不可变候选。工作台预览精确的 Binding、当前 Value、source pin、配置修订及文件版本基线。支持的候选只修改一个 Binding，完整字节必须与既有来源 owner 准备的内容一致；非目标修改被拒绝。提交复用既有 canonical 值变更请求，人工批准后由来源事务应用准备好的 successor；提交本身不激活上传文件。

手动同步校验当前 canonical 来源和值的一致性，不补旧参数行、不创建旧草稿。历史回滚基于当前文件版本进入同一来源变更审核流程。重试返回已有请求及其当前状态，不创建第二次审批，也不将原始上传误标为活跃版本。

只读预览返回完整来源 cohort 的基线令牌，候选预览还固定上传字节。提交和回滚必须携带该令牌，并在来源锁内重新校验。即使文件版本未变，pin 或 cohort 变化也会使预览失效。只读查询不获取修改用的行锁。

已有 canonical 草稿保持原样。草稿冲突或来源基线漂移会阻止提交，不替换选定工作。本次有界接入明确不支持多 Binding、来源结构/成员变更，以及缺少原子批量或精确基线草稿删除接口的冲突裁决；对应控件说明限制。这不构成整文件覆盖授权，也不削弱 `assertLegacySourceMutationAllowed`。

本补充的验收范围为 API 模式下 `1440×900` 的单 Binding DTS/JSON 流程、专用 PostgreSQL 与对象存储，以及定向漂移、重试、失败和权限验证。上文历史上的更广视口要求不作为本次新增证据。
