...
仅使用标准库的 `scripts/ci-required-results.ts` 校验新报告、完整固定命令回执和预期作业结果；聚合作业使用 Node 22，不重新安装依赖。回执绑定实际 checkout SHA/tree、适用的 PR base/head、run 与 attempt；报告摘要哈希只关联字节，不证明成功。原始报告保留在所属 runner，作业输出只包含有界身份及计数，不带消息、日志、环境值或凭据。这不替代完整浏览器证据，也不允许复用 PG/Hosted 的“已通过”结果。回归覆盖报告损坏、过期、缺失，必需作业跳过或取消，未知标志/事件/模式，前提丢失及 trusted-base 命令变化。命令见[验证矩阵](../developer/verification-matrix.md)；效果在可比样本充分前保持 `observation-pending`。
