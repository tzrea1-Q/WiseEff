...
> English: [English](../../design-docs/deployment-operations.md)
...
- `XIAOZE_LLM_API_BASE_URL`、`XIAOZE_LLM_MODEL`、`XIAOZE_LLM_API_KEY`（live Xiaoze LLM；离线验收可用 `XIAOZE_DETERMINISTIC=true`）
...
- live Xiaoze LLM 需要 atomic `XIAOZE_LLM_API_BASE_URL`、`XIAOZE_LLM_MODEL`、`XIAOZE_LLM_API_KEY` 组三键；离线验收可用 `XIAOZE_DETERMINISTIC=true`。
- 生产/自托管需要 `XIAOZE_CHECKPOINTER=postgres`（除非 `XIAOZE_DETERMINISTIC=true`）。
- `.env.example` 的 local non-HDC profile 故意只留 `XIAOZE_LLM_API_BASE_URL`、`XIAOZE_LLM_MODEL`、`XIAOZE_LLM_API_KEY` 为空。
