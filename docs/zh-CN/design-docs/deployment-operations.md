...
当前运行时只有手工 `git fetch` / commit checkout / Compose rebuild 流程。拟议的[一键升级模块](2026-08-20-self-hosted-one-command-upgrade-design.md)会在操作员 seam 上把该流程做成事务式编排：锁定唯一 commit、停机前预构建、停止 writer、验证 PostgreSQL/对象存储/Redis 恢复点、不删除 volume 地重建所有服务、观察迁移和健康门禁，并持久化可继续的恢复状态。它与 `setup.sh` 分离；升级不能渲染 `.env`、轮换凭据或导入 seed。
