...
| Reconciliation, cutover, review, observation, and history row | ReviewEvidence、Observation evidence 或 immutable target history/Archive                                                               | 保留 status、count、decision reason、observation evidence 与 audit；不能重新执行 historical decision。                     |
...
## G0.1 mapping 与 rehearsal 补充

公开 `LegacyLookupIdentifierType` 继续使用 transition 合同冻结的七个 API kinds；Cutover persistence 使用 active #668 规格冻结的内部 49 项 `LegacyMappingSourceKind` inventory。两个 registry 不能共用数据库 constraint 或 TypeScript union。Classifier 把每个 internal kind 精确分入 11 个 closed classes，并为每类固定 owner extractor 与 typed target-or-Archive disposition：formal definition content、formal subject、module/placement、overlay publication evidence、observation/match evidence、Binding/value lineage、legacy semantic store、draft/review workflow、file/import/initialization workflow、reconciliation/cutover/review/observation/history，以及 policy/audit/unresolved protected reference。Platform owner 精确为 `(platform, platform)`；Organization/Project 使用 source row 的 exact ID；ownerless child 继承 parent，parent 缺失或冲突就进入 R0。Composite ID 对有序字段使用 `serializeContract`，禁止 delimiter 与 hash-only identity。Unresolved protected reference 包含 consumer table/primary-key/column/raw referenced ID。Registry 固定 source literal 到 physical table、primary key 与 owner extractor，caller 不能自报。最少 exact internal target literals 为 `parameter-observation`、`observation-match`、`review-item`、`review-resolution`、`publication-intent`、`policy`、`audit-event`、`migration-history`；display name 不能替代 literal。这些 target 都是 first-class internal outcome，不能塞进七项公开 lookup kind。Unknown/unresolved protected reference 继续作为 Archive/blocker evidence。
