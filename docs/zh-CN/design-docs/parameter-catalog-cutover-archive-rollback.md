...
公开 `LegacyLookupIdentifierType` 继续使用 transition 合同冻结的七个 API kinds；Cutover persistence 使用 active #668 规格冻结的内部 49 项 `LegacyMappingSourceKind` inventory。两个 registry 不能共用数据库 constraint 或 TypeScript union。Classifier 把每个 internal kind 精确分入 11 个 closed classes，并为每类固定 owner extractor 与 typed target-or-Archive disposition：formal definition content、formal subject、module/placement、overlay publication evidence、observation/match evidence、Binding/value lineage、legacy semantic store、draft/review workflow、file/import/initialization workflow、reconciliation/cutover/review/observation/history，以及 policy/audit/unresolved protected reference。Observation、match、review、publication、policy、audit、migration-history target 都是 first-class internal outcome，不能塞进七项公开 lookup kind。Unknown/unresolved protected reference 继续作为 Archive/blocker evidence。

历史 fixture source `6c3adfc35c0e3be6d5d381013dace9408190380e` 与原 checksum 保持 immutable evidence，不是 executable trust。S0-FIX 必须先 append 一个只改原 18 files 的 repair commit `R`，再由第 19 个 external source-lock test 固定 `R`、全部 path/mode/per-file hash 与新的 length-framed bundle checksum `B`。Runner 使用 PostgreSQL-aware lexer 禁止 psql meta-command 和 transaction/session escape；在 mutation 前、forward rehearsal 后、rollback 后各验证一次 fixture；cleanup fail closed，只有完整清理后才输出 `CLEANUP_OK`；closed-world secret scan 覆盖 source 和 generated artifacts。Real PostgreSQL 与 `test:scripts` 属于实现门禁；local synthetic rehearsal 绝不是 target、release 或 production evidence。
