...
本文是 Wayfinder 地图 [Wayfinder: replace the parameter catalog with one canonical definition model](https://github.com/tzrea1-Q/WiseEff/issues/668) 中 [Choose populated-data cutover, archive, and rollback strategy](https://github.com/tzrea1-Q/WiseEff/issues/678) 的已接受决策产物。
...
- [ADR-0040、ADR-0041 与 ADR-0042 最终集成决策集](https://github.com/tzrea1-Q/WiseEff/tree/9fe269d4facc31b49fc1e0535d2d51ba7140644b/docs/adr)；以及
...
Catalog 编译与同步仍位于 [Choose the catalog kernel interface and transaction boundary](https://github.com/tzrea1-Q/WiseEff/issues/673) 将决定的 catalog-kernel interface 后。本文消费该 interface，不重复或抢占它。
...
| P11 Independent verification              | P5-P10 checkpoint 完成。                                                                                       | 使用与 writer 不同 code path/credential 的 read-only verifier 重算 catalog、mapping、registration、history、Archive 与 consumer-reference invariant；所有要求指标必须为零或精确值。                                                                                                                                                                                                                                                          | 可确定性重跑；失败时流量保持隔离，只能 same-run idempotent 修正或 whole-state restore，禁止 ad hoc SQL。                                                          |
| P12 Atomic application read switch        | P11 report digest 已批准。                                                                                     | compare-and-swap application read-mode pointer，从 legacy 切换到 canonical，并记录 exact Catalog Release、mapping epoch 与 verifier report。不做 dual-read business fallback；shadow comparison 只能是 read-only evidence。                                                                                                                                                                                                                  | 未知 commit 结果通过读取 pointer 判定；switch-back 只遵守下述 zero-write 规则。                                                                                   |
...
exact SQL/failure code 属于 implementation specification，但上述每项必须有一个 deterministic query/check、expected count、stable failure code 与 evidence field。[Choose verification, upgrade, and legacy-retirement gates](https://github.com/tzrea1-Q/WiseEff/issues/679) 可以增加 release aggregation、browser/observability gate，但不能弱化这些 cutover invariant。
...
| P12 application read 已切换、candidate write/traffic 前                               | **有条件允许**                   | 原子恢复 application read pointer + Catalog pointer/head，再验证 old stack；否则 whole-state restore。                                                                                              | complete old projection/mapping 验证；zero candidate write、queue delivery、public traffic。                             |
...
9. **Independent verifier：**使用与 writer 不同的 read-only process/role，持久化 count、checksum、release fingerprint、R6/R8 proof、legacy consumer coverage 与 report digest。
10. **Activation：**service 仍停止时 compare-and-swap application read pointer，并 retire 全部 legacy writer；重跑 writer reachability 与 complete verifier。
...
| [Capture a representative populated-database rehearsal fixture](https://github.com/tzrea1-Q/WiseEff/issues/671) 的 Local synthetic PostgreSQL | deterministic representative graph、failure injection、idempotency、same-key separation 与 rollback containment。                                                                                              | actual target row、storage、OIDC、queue、proxy、capacity 或 release readiness。 |
