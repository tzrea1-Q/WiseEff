...
| Binding draft 提交身份 | 拓扑 draft 精确拥有 `draftId`、binding、spec、candidate config revision（`parameter_drafts.candidate_config_revision_id`，迁移 `0059`）、value/reason 与 write lock。提交会锁定 draft 行，并证明 candidate binding revision 具有完全一致的 raw value。迁移 `0060` 记录并失效缺少 candidate identity 的历史 manual draft，用户必须通过 typed editor 重建。Cutover 后拒绝遗留保存/提交；不得再用 binding ID 冒充遗留 `parameterId`。 |
