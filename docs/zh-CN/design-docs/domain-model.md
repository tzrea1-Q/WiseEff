...
| 参数执行身份 | 治理数据区分可问责 principal 与实际执行 initiator。可信 User/Agent 行携带经校验的 principal 用户；Agent 还必须保留非空 session/tool-call/approval 关联。System 行不携带用户归属，并持久化明确的 service/job identity。迁移 `0116`–`0121` 引入增量投影与历史兼容；前向迁移 `0122` 施加数据库级判别联合（旧行使用 `NOT VALID` 保留，所有新建/更新行严格执行）。显式 `legacy` 只能是无 metadata 的 #614 之前 writer 标记，由 #615 后续收口。binding revision 保留 initiator 投影，而可问责 principal 由父级 draft/change request 承担。System 的 high/no-match 操作仍保持成功，并用粗粒度 service/job label 展示，不能显示认证用户姓名。完整 correlation 仅存在于内部领域/审计投影，公共 DTO 与通知只暴露稳定展示标签。 |
