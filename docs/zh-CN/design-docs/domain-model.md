...
| 目标组织主体注册 | 替代目录对每个 `(Organization, 正式 Platform Driver 或 NodeType subject)` 使用一条持久 `active | retired` registration，并始终保留且恰有一个 placement。Organization creation 不注册任何 subject。Organization Admin 可以预注册；唯一证明的 catalog match 会幂等注册并默认落入保留「未分类」根，unknown/ambiguous evidence 则进入 review。该 subject 的所有 definition 动态继承当前 placement。module 与 observation 永远不能创建 subject 或 definition identity。这是目标决策，不是当前生产行为（ADR-0042）。 |
