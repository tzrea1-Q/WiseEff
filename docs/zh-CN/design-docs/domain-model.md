...
| 参数执行身份 | 治理数据区分可问责 principal 与实际执行 initiator。可信 User/Agent 行携带经校验的 principal 用户；Agent 额外保留 session/tool-call/approval 关联。System 行不携带用户归属，并持久化明确的 service/job identity。迁移 `0116`–`0120` 建立严格投影，`0121` 只保留完全没有 provenance 的 #614 之前 legacy `user` 行；任何携带 provenance 的行都在数据库边界接受双向约束。binding revision 保留 initiator 投影，而可问责 principal 由父级 draft/change request 承担。System 的 high/no-match 操作仍保持成功，并用 service/job label 展示，不能显示认证用户姓名。 |
