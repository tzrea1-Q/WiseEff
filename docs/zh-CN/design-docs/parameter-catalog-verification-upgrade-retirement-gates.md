...
| `PCAT-DB-P01` | 每个 production role 直接对 immutable Catalog row 执行 INSERT/UPDATE/DELETE 均失败。任何 LOGIN 都不得是 `catalog_migration_owner` 的成员；`catalog_synchronizer_role` 与 `parameter_governance_writer_role` 的 LOGIN 成员不得 `rolinherit`；文档化的 `NOINHERIT` composition-root 成员关系仍是受审计的 `SET LOCAL ROLE` 路径。仅直接 `pg_auth_members` 可达性——限制见下方说明。 | `PCAT-PRIV-CATALOG-IMMUTABILITY-BYPASS` | role/action negative matrix 与 SQLSTATE checksum         | Startup 前 repair grant / security                                            |
...
**P01 范围说明（2026-09-15）。** `docs/SECURITY.md` 刻意让 API LOGIN 成为 `parameter_governance_writer_role` 的 `NOINHERIT` 成员、让 publication manager 成为 `catalog_synchronizer_role` 的 `NOINHERIT` 成员，使 composition root 能在受审计路径上 `SET LOCAL ROLE`；`provisionRuntimeLogins` 正是这样授权的，并把 `catalog_migration_owner` 从所有 LOGIN 上 revoke。早先的 P01 检查的是"存在任何 LOGIN 成员"，因此**在每个正确 provision 的集群上都会失败**；该检查已按上表规则修正。两点保留记录而非静默处理：迁移 `0138` 的内联 P01 注释仍写着"不能 `SET ROLE` 到 `catalog_synchronizer_role` 或 `catalog_migration_owner`"，它早于该模型，且已应用的迁移不可改写；门禁只读取直接 `pg_auth_members` 行，因此间接可达性（例如某 LOGIN 继承了本身已是特权角色成员的 NOLOGIN 角色——`pg_has_role(..., 'SET')` 会暴露而当前谓词不会）是已知的既有缺口，需要单独的门禁变更。
