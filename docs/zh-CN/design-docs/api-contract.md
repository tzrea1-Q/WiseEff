...
`POST /api/v1/parameter-submission-rounds` 使用两种不可混合的 item。遗留扁平提交 `{ parameterId, targetValue, reason }` **仅可在语义 cutover 前使用**；cutover 后遗留草稿保存和遗留提交均返回 `409`。Cutover 后拓扑提交为 `{ draftId, projectParameterBindingId, parameterSpecId, targetValue, reason }`，不得同时携带 `parameterId`。服务端按调用者组织、项目和用户锁定该 draft 行，复核 binding/spec/value/reason、迁移 `0059` 持久化的 candidate revision、candidate binding revision 的精确 raw value，以及 binding/file/occurrence write lock，再创建提交轮次。跨项目或 draft 不存在返回 `404`；身份、candidate value 不一致、candidate 非 draft 或 lock 过期返回 `409`；失败不写成功审计。`parameter-submit` 审计记录已接受的 draft、binding 与 spec ID。

**第六轮证据（分支 `fix/parameter-topology-round6-review-blockers`）：** 迁移 `0058` 仅信任 evidence 的 review-task scope 校正（含缺 evidence 时重新打开）；手工实体 ID 与持久化 specification key 均无损（`vendor,limit` ≠ `vendor-limit`）；组织 Admin 不可激活全局 draft；完整 valueShape 激活链路；租户作用域 fixture 清理；`test:all` 下的 API runtime/fixture/查询隔离；迁移 `0059` 与显式 binding-draft 提交身份；以及前向迁移 `0060`，记录并失效缺少 candidate identity 的历史 manual draft。精确提交会拒绝 cutover 后遗留 shape、锁定 draft 行并证明 candidate value 一致。拓扑验收自动创建带 marker 校验的 cutover 后可丢弃数据库，并跑通真实 submit→review→merge→writeback→reload 角色链。该结果仅为实现验收；TD-042 继续阻断生产 cutover 就绪声明。
