...
| `GET` | `/api/v2/parameter-specs/:specId/property-key-cutover` | Admin 读取**开放**的属性键 cutover run（`{ item }`）。项带 `fileId` / `fileName` / `nodePath`，供工作台深链。`stagedRewrite.status` 是文件候选的**实况**（`ready`、`active`、`abandoned`、`missing` 等），不是 prepare 当时的快照。不激活候选、不写现行源。无开放 run → `404`。 |
