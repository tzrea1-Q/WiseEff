...
| `POST` | `/api/v2/parameter-specs/:specId/property-key-cutover/prepare` | Admin 按**当前**源位置重分类开放 run 的项。请求体 `{ reason? }`。**不**创建文件候选 / 变更请求草稿，也**不**改写 DTS 或 catalog 三元组。源里已是新键（或 binding 已不在）→ 诚实 skip；`would-rewrite` 保持 `pending`；冲突 / 缺失 / 无 occurrence 保持 `incompatible`。审计：`spec-property-key-cutover-prepared`。 |
