...
**第六轮证据（分支 `fix/parameter-topology-round6-review-blockers`）：** 迁移 `0058` 仅信任 evidence 的 review-task scope 校正；无损手工规格 ID（`vendor,limit` ≠ `vendor-limit`）；组织 Admin 不可激活全局 draft；完整 valueShape 激活链路；拓扑验收 submit→review→merge 回写；租户作用域 fixture 清理；`test:all` 下共享 PG advisory lock。
