...
**第六轮证据（分支 `fix/parameter-topology-round6-review-blockers`）：** 迁移 `0058` 仅信任 evidence 的 review-task scope 校正；无损手工规格 ID（`vendor,limit` ≠ `vendor-limit`）；组织 Admin 不可激活全局 draft；完整 valueShape 激活链路；拓扑验收强制真实 submit→review→merge 角色链与 cutover 后 writeback 证据；租户作用域 fixture 清理；`test:all` 下的 API runtime/fixture/查询隔离。当前共享本地库仍在 cutover 前，因此该验收会在业务写入前正确停止，不是 merge 成功证据。
