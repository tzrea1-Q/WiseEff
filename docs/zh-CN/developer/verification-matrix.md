...
| `npm run test:server -- server/modules/parameter-topology/postCutoverWorkflow.integration.test.ts server/modules/parameter-topology/schemaMigration.test.ts server/modules/parameters/schemas.test.ts --run` | Cutover 后精确 draft/action identity、set candidate value 或 delete tombstone 证明、双连接 draft 行锁、0060→0061 全 origin 失效/回滚/幂等、0062 action schema、无 shadow PPV 合入/回写与 base binding revision 不可变 | 修改 typed draft 提交、0059–0062、set/delete 合入回写、并发或 binding revision 不可变性后。需 PostgreSQL。 |
| `npm run test:server -- server/modules/parameter-topology/postCutoverWorkflow.integration.test.ts server/modules/parameter-topology/schemaMigration.test.ts server/modules/parameters/routes.test.ts --run` + `npx vitest run src/components/parameter-topology/ApiProjectTopologyWorkspace.test.tsx` | 显式 draft/binding/spec/action wire identity、cutover 后 legacy 拒绝、candidate/lock 校验、迁移 0059–0062，以及切换项目时 candidate/draft/message/迟到响应隔离 | 修改 typed binding 提交、项目切换、0059–0062 或 write lock 后。服务端集成需 PostgreSQL。 |
...
| `npm run acceptance:e2e -- e2e/acceptance/parameter-topology.acceptance.spec.ts` | 拓扑治理含 draft→activate→resolve，以及真实 set/delete typed draft→submit→角色审核→merge/writeback/reload；自动创建、标记、cutover、校验并销毁可丢弃数据库。因当前无 delete 控件，delete 创建/提交走公开 API，全部角色决议/merge 仍走 UI。 | 修改拓扑验收辅助、规格审核 UI、set/delete 回写或不可变合入行为后。`DATABASE_URL` 仅提供 PostgreSQL 服务端/管理连接；生成库名、test marker、cutover migration run 任一不匹配时拒绝破坏性清理。 |
