...
| Gate0 fixed-point 安全合同 | root owner、visual/browser phase group、root/nested API/frontend 都持久化精确 process-start identity；TERM、KILL 分别在发信号前重新验证持久 identity。recovery owner 与 reclaimer 崩溃残留 link 只在 dead/expired identity 加 inode/token 证明后回收。每次 exact secret 注册在 ACK 前原子发布加密二扫上下文。owner 遭 `SIGKILL` 后，fresh finalizer 先证明 owner 已退出，再停止 phase group、重读晚到 nested 登记并停止 nested/root writer；DB/object 始终保留取证。exact context 会保留到私有 staging 与其中生成的单一 ZIP 都达到 0 violation；之后才原子发布只读 ZIP 并退役 registry。CI 只上传该精确 ZIP，绝不上传 live runs root。nested object intent 在首次 mkdir 前登记；启动失败保留 DB/object 证据，没有 marker/cutover 证明的数据库绝不 drop。CI timeout 统计不依赖 YAML step 键顺序，枚举全部 step。 | 修改 Gate0 provision、进程清理、manifest lock、artifact safety、不可变上传 finalizer 或 CI workflow 解析后，运行聚焦 scripts 测试与 `npm run acceptance:ci`。 |
