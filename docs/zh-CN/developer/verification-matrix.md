...
| Gate0 fixed-point 安全合同 | root owner、visual/browser phase group、root/nested API/frontend 都持久化精确 process-start identity；TERM、KILL 分别在发信号前重新验证持久 identity。root provisioning 在 run 目录可见前先原子发布不含秘密的 journal，并显式记录 migration/seed/API/frontend 状态。provision 命令、root/nested runtime 与 visual/browser phase 命令共用受控启动协议：先落 durable intent，process-group launcher 自行发布 PID/start identity，父进程 ACK 前真实命令不得 exec。因此 owner 遭 `SIGKILL` 后，fresh finalizer 能接管“API 已登记、frontend 明确 not-started”、phase identity 回写中断等 partial writer；未知或 PID reused 时绝不 signal，DB/object 始终保留取证。recovery owner 与 reclaimer 崩溃残留 link 只在 dead/expired identity 加 inode/token 证明后回收。每次 exact secret 注册在 ACK 前原子发布加密二扫上下文。exact context 会保留到私有 staging 与其中生成的单一 ZIP 都达到 0 violation；之后才原子发布只读 ZIP 并退役 registry。artifact safety 会递归脱敏/扫描 nested ZIP（含 magic sniff 识别的改名 ZIP）；raw envelope 校验会在上传前拒绝重复/不安全路径、前后附加数据、加密或不支持压缩、metadata 凭据、CRC/尺寸不一致，以及整棵树范围的深度、entry 数、解压总量与膨胀率超限。CI 只上传该精确 ZIP，绝不上传 live runs root。nested object intent 在首次 mkdir 前登记；启动失败保留 DB/object 证据，没有 marker/cutover 证明的数据库绝不 drop。CI timeout 统计不依赖 YAML step 键顺序，枚举全部 step。 | 修改 Gate0 provision、进程清理、manifest lock、artifact safety、不可变上传 finalizer 或 CI workflow 解析后，运行聚焦 scripts 测试与 `npm run acceptance:ci`。 |
