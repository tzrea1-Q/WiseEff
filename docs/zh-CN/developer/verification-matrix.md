...
| `npm run test:server -- server/modules/parameter-topology/postCutoverWorkflow.integration.test.ts server/modules/parameter-topology/schemaMigration.test.ts --run` | Cutover 后精确 draft identity、candidate raw value 一致性、双连接 draft 行锁、0060 失效/回滚、无 shadow PPV 合入/回写与 base binding revision 不可变 | 修改 typed draft 提交、0059/0060、并发、语义合入/回写或 binding revision 不可变性后。需 PostgreSQL。 |
| `npm run test:server -- server/modules/parameter-topology/postCutoverWorkflow.integration.test.ts server/modules/parameter-topology/schemaMigration.test.ts server/modules/parameters/routes.test.ts --run` + `npx vitest run src/components/parameter-topology/ApiProjectTopologyWorkspace.test.tsx` | 显式 draft/binding/spec wire identity、cutover 后 legacy 拒绝、candidate/lock 校验、迁移 0060，以及切换项目时 candidate/draft/message/迟到响应隔离 | 修改 typed binding 提交、项目切换、0059/0060 或 write lock 后。服务端集成需 PostgreSQL。 |
...
| `npm run acceptance:e2e -- e2e/acceptance/parameter-topology.acceptance.spec.ts` | 拓扑治理含 draft→activate→resolve 与真实角色顺序 submit→review→merge/writeback；自动创建、标记、cutover、校验并销毁可丢弃数据库；业务流程无 repository/DB 绕过 | 修改拓扑验收辅助、规格审核 UI 或不可变合入行为后。`DATABASE_URL` 仅提供 PostgreSQL 服务端/管理连接；生成库名、test marker、cutover migration run 任一不匹配时拒绝破坏性清理。 |
