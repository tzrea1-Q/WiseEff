...
| `npx vitest run scripts/check-operation-evidence.test.ts scripts/run-browser-acceptance.test.ts` + `npm run acceptance:evidence` | full run 的 record/artifact 按 runId+sourceCommit 隔离；focused 不覆盖 `latest-full`；混合运行和缺失 artifact fail-closed。owned Gate0 记录的 report/trace 必须是 runRoot 下真实存在的普通后代，runtime identity 必须绑定经 SHA-256 复核的不可变 snapshot。 | 修改 Playwright output、operation evidence helper、browser runner 或 checker 后。 |
