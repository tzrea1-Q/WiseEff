...
| Gate0 fixed-point 安全合同 | root 与 nested API/frontend 记录都持久化精确 process-start identity；TERM、KILL 分别在发信号前重新验证 manifest identity。recovery owner 与 reclaimer 崩溃残留 link 只在 dead/expired identity 加 inode/token 证明后回收。每次 exact secret 注册在 ACK 前原子发布加密二扫上下文；finalize 前先封存 writer，owner 遭 `SIGKILL` 后的新 CI 进程仍可脱敏，0 violation 后才删除外部 key 与密文。nested object intent 在首次 mkdir 前登记；启动失败保留 DB/object 证据，没有 marker/cutover 证明的数据库绝不 drop。CI timeout 统计不依赖 YAML step 键顺序，枚举全部 step。 | 修改 Gate0 provision、进程清理、manifest lock、artifact safety 或 CI workflow 解析后，运行聚焦 scripts 测试与 `npm run acceptance:ci`。 |
