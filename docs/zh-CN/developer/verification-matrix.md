...
| `npm run selfhost:ip-lab:init` | 为无域名 IP 实验室生成 `ops/self-hosted/.env` | 本机已有 Node.js 22 时使用。服务器无 Node 时用 `ops/self-hosted/scripts/deploy-ip-lab.sh`。 |
| `npm run selfhost:ip-lab:preflight` | 校验 IP 实验室 `.env` 与选定 Caddyfile | IP 实验室 `compose up` 前，或改过实验室环境变量后。 |
| `npm run selfhost:ip-lab:provision` | 导入 M0–M3 并把实验室管理员挂到 ChargeLab | IP 实验室栈就绪后在 API 容器内执行。 |
