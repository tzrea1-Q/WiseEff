...
| Gate0 fixed-point 安全合同 | root 与 nested API/frontend 记录都持久化精确 process-start identity；TERM、KILL 分别在发信号前重新验证。recovery owner 与 reclaimer 崩溃残留 link 只在 dead/expired identity 加 inode/token 证明后回收。provision secret 在 child 前登记，并加密保留给新进程 artifact 二扫；0 violation 后删除外部 key 与密文。nested 启动失败保留 DB/object 证据，没有 marker/cutover 证明的数据库绝不 drop。CI timeout 统计不依赖 YAML step 键顺序，枚举全部 step。 | 修改 Gate0 provision、进程清理、manifest lock、artifact safety 或 CI workflow 解析后，运行聚焦 scripts 测试与 `npm run acceptance:ci`。 |
