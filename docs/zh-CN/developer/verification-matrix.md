...
| `npm run test:server -- server/modules/parameter-topology/postCutoverWorkflow.integration.test.ts server/modules/parameters/routes.test.ts --run` + `npx vitest run src/components/parameter-topology/ApiProjectTopologyWorkspace.test.tsx` | 迁移 `0059` 的 draft→candidate 关联、显式 draft/binding/spec wire identity、跨项目与 stale lock 拒绝，以及切换项目时 candidate/draft/message 隔离 | 修改 typed binding 提交、项目切换或 write lock 后。服务端集成需 PostgreSQL。 |
