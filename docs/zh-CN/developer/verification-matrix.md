...
| `npm run acceptance:gate0` | owner 在创建任何数据库、对象根或进程之前，先执行公开的钉扎门禁 `npm run dts:toolchain:check -- --required`；CI 由 `setup-dts-toolchain` 安装，本地操作者另行显式 bootstrap，Gate0 本身绝不联网安装依赖。之后创建 checked-absent `wiseeff_acceptance_full_*` 数据库并完成 migrate/seed/marker 证明，创建 checked-absent run-scoped 对象根和精确 loopback API/frontend 进程，再把同一份不含秘密的 descriptor 交给 visual 与 full browser。未知监听者以及 PID/DB/object marker 不匹配均 fail-closed；嵌套 disposable suite 会在第一个外部资源创建前先写入 `provisioning` 清单项，随后在 migration identity 与每个 detached PID 出现时原子更新。根 owner 在 finalize 时读取该清单：成功前每个 child 必须终态为 cleaned；timeout/失败时会接管部分 provision 与运行中的 child，精确停止并验证所有已记录进程组，同时逐项记录数据库/对象证据的保留原因。 | TD-122 的权威本地 L2 基线。60 分钟 owner 时限从进程入口开始，覆盖 DTS、source check、provision、HTTP startup、两阶段、进程停止、artifact finalize 和 cleanup，并为 finalize 预留 5 分钟。`SIGINT`/`SIGTERM` 会取消 provision/phase，但进程在同一套有界失败收尾完成前不会退出；硬时限仍覆盖 cleanup 与 artifact scan。全部 Playwright output/report/snapshot、preflight、浏览器/operation evidence、日志、trace、截图、清单和 inventory 都写入独占 runRoot，不递归删除共享目录或未知文件。失败保留精确 DB/object/descriptor/artifact 并记录清理状态；只有成功才删除精确 DB/object。上传前只对显式文本格式（含 ZIP 内文本条目）做递归脱敏；不支持格式或二进制一旦命中凭据即 fail-closed；普通路径和 ZIP entry name 也会以不泄密的 artifact ID 扫描，且 `npm run acceptance:artifacts:check -- --root test-results/acceptance-runtime-runs` 必须为 0 violation。该基础 PR 本身不关闭 TD-122。 |
