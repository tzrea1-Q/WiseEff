...
- `PARAM-ADMIN-003`：PC 1440×900 下，Admin 在 `/parameter-admin/projects` 的 DataTable 经 URL `q`/`status`/`sort` 搜索、筛选、排序与分页；刷新、`popstate`、后退与前进都恢复状态；键盘进入行且编辑/删除隔离不冒泡，表格与页面不横向溢出。`coverage: automated`，见 `e2e/acceptance/parameter-admin-projects.acceptance.spec.ts`。
