...
| `PCAT-LEGACY-LINK-001` | P0 | automated | PCAT-UI-11 | authorized/scope-hidden；mapped/410/409/404；`/parameters?parameter=:legacyId` 已归档提示 | 三视口 | status+headers；mapping-head/Archive/legacy migration evidence DB；提示不进入编辑/提交且可关闭；三视口 screenshots | `e2e/acceptance/parameter-catalog-negative.acceptance.spec.ts` |
