...
| `PCAT-UI-11` | automated | authorized + scope-hidden callers | `/parameters?parameter=:legacyId` 的已归档提示，以及 legacy mapped redirect、410 gone、409 conflict、404 unknown/scope-hidden | 三视口 | Catalog resolution + workbench 410；mapping-head/Archive/legacy migration evidence DB；必要 operator audit；已归档提示在 1440×900、768×1024、390×844 的截图，且不进入编辑草稿并可关闭 | `e2e/acceptance/parameter-catalog-negative.acceptance.spec.ts` |
