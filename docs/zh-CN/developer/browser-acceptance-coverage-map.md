...
- `DEBUG-ADMIN-846-VIEWPORTS`：在 1440×900、768×1024、390×844 三档宽度下打开真实导入预览弹窗（连接真实 PostgreSQL 数据）：弹窗始终落在视口内，确认操作可见且可点击，分类计数与协议路径/访问模式/启用状态差异正常渲染，页面无横向溢出，取消可关闭弹窗，浏览器诊断记录无控制台或页面错误；每档宽度均附截图。
...
- `PROJ-CONFIG-READ-001`：PC 1440×900 下，Admin 从项目清单进入规范项目配置路由，优先恢复有效 URL 配置集、否则确定性选择默认配置集；树区分成员/未编组文件并展示角色与活跃版本身份，经 API ports 读取选中活跃 DTS 源码；源码主导的树、检查器与任务 sheet 以及空/加载/失败恢复状态均可用（`e2e/acceptance/project-configuration-workbench.acceptance.spec.ts`；组件/端口/服务端测试；证据见 `work/ui-checks/project-configuration-workbench-readonly/`）。
...
- `PROJ-CONFIG-CUTOVER-001`：PC 1440×900 下旧 `/files` `/config-sets` `/structure` `/conflicts` 深链重定向到等价工作台上下文并保留焦点；新链接只使用 `/configuration`；集成切换不丢失能力；自动化归属 `e2e/acceptance/project-configuration-workbench.acceptance.spec.ts`；证据见 `work/ui-checks/project-configuration-workbench-cutover/`。
...
- `XIAOZE-POPUP-MOVE-001`：桌面/平板用户可在关闭态拖动悬浮球，并在展开态让悬浮球和非模态弹窗整体移动；继续覆盖缩放、复位、刷新、业务页面操作和切换，手机保持全屏（`e2e/acceptance/xiaoze-popup-layout.acceptance.spec.ts`）。
...
本 PCAT 登记表中的视口值描述复用参数 UI 在当前 #849/#853 验收中的单一 PC 执行配置；历史 #668/#847 收据以及无关的 publication/debugging 验收保持不变。

| Requirement ID | 状态 | 角色 | 路由/交互 | 视口 | 必需 API / DB / audit / screenshot 证据 | Acceptance owner |
| --- | --- | --- | --- | --- | --- | --- |
| `PCAT-UI-01` | automated | User、Org Admin、Platform Admin、Agent | `/parameter-admin/specs` 进入唯一 Parameter definitions；无 Effective/Governance peer | 单 PC 1440×900 | catalog/list 200；current-release DB projection；零读 mutation；PC 入口/列表截图 | `e2e/acceptance/parameter-catalog.acceptance.spec.ts` |
| `PCAT-UI-02` | automated | User、Org Admin | opaque Subject/Definition/release deep link 在 reload/Back/Forward 后恢复 | 单 PC 1440×900 | pinned cursor/detail API；selected-ID DB agreement；无 audit；恢复截图 | `e2e/acceptance/parameter-catalog.acceptance.spec.ts` |
| `PCAT-UI-03` | automated | User、Org Admin | 搜索/选择 Subject/Definition，查看 owner、current/pinned revision、usage、Registration/Placement | 单 PC 1440×900 | detail/revision API；owner/head DB；无 audit；PC pane 截图 | `e2e/acceptance/parameter-catalog.acceptance.spec.ts` |
| `PCAT-UI-04` | automated | Org Admin | 同页 Review Queue 选择合法 resolution 和显式 Placement，并提交一次 | 单 PC 1440×900 | ETag/release/idempotency API；Resolution+exact Registration/Placement/Proposal DB；成功/拒绝审计；前/确认/后截图 | `e2e/acceptance/parameter-catalog-governance.acceptance.spec.ts` |
| `PCAT-UI-05` | automated | authorized reader | Definition timeline 稳定分页 Catalog fact 与授权 History/Audit | 单 PC 1440×900 | composite cursor API；pinned DB facts；无读审计；timeline 截图 | `e2e/acceptance/parameter-catalog.acceptance.spec.ts` |
| `PCAT-UI-06` | automated | 全部表示角色 | ready 只展示授权动作并保留 release anchor | 单 PC 1440×900 | role API matrix；denied DB unchanged；必要 refusal audit；逐角色截图 | `e2e/acceptance/parameter-catalog.acceptance.spec.ts` |
| `PCAT-UI-07` | automated | Org Admin；其他角色 negative | unregistered Subject 仍可读，显式 `use-default`/`choose-parent` 注册并保留 ID | 单 PC 1440×900 | release/idempotency API；exactly-one Registration/Placement DB；成功/拒绝审计；选择/结果截图 | `e2e/acceptance/parameter-catalog-governance.acceptance.spec.ts` |
| `PCAT-UI-08` | automated | authorized reader | 区分 loading/error 和四个 emptyReason；stale-visible loading 禁写 | 单 PC 1440×900 | state API；fixture DB predicate；零 mutation/audit；PC 各状态截图 | `e2e/acceptance/parameter-catalog.acceptance.spec.ts` |
| `PCAT-UI-09` | automated | authorized reader；Org Admin negative | retired/deprecated 历史可读，禁止新动作 | 单 PC 1440×900 | lifecycle read + 409；retained DB history；refusal audit；badge/disabled 截图 | `e2e/acceptance/parameter-catalog.acceptance.spec.ts` |
| `PCAT-UI-10`（#847 停用：浏览器用例依赖已移除的「定义修订」面板） | automated | Org Admin、Platform Admin | release/ETag/parent/idempotency conflict 保留输入、刷新证据、要求重确认 | 单 PC 1440×900 | exact 409 reason；无 partial DB；durable refusal/no success audit；conflict 截图 | `e2e/acceptance/parameter-catalog-negative.acceptance.spec.ts` |
| `PCAT-UI-11` | automated | authorized + scope-hidden callers | `/parameters?parameter=:legacyId` 对所属组织显示可关闭且不可编辑的已归档提示；其他组织得到不可区分且不含迁移证据的 404；同时覆盖 legacy mapped redirect、410 gone、409 conflict、404 unknown/scope-hidden | 单 PC 1440×900 | Catalog resolution + 所属组织 workbench 410 + 其他组织 404；mapping-head/Archive/按租户隔离的 legacy migration evidence DB；必要 operator audit；已归档提示在 PC 1440×900 截图，且不进入编辑草稿并可关闭 | `e2e/acceptance/parameter-catalog-negative.acceptance.spec.ts` |
| `PCAT-UI-12` | automated | Agent | 按 invoking principal 只读，所有治理 mutation/spoof 被拒 | 单 PC 1440×900 | scoped reads + 403；零 DB mutation；trusted refusal audit；只读截图 | `e2e/acceptance/parameter-catalog-negative.acceptance.spec.ts` |
| `PCAT-UI-13` | automated | 全部表示角色 | API/mock 重放相同状态与权限，不得有 mock-only governance | 单 PC 1440×900 | real API digest；API-half DB；real-half audit/mock no-extra-authority；配对截图 | `e2e/acceptance/parameter-catalog-negative.acceptance.spec.ts` |
| `PCAT-UI-14` | automated | authorized reader | 单页/list/detail/timeline/queue/dialog 无重叠、溢出、隐藏动作或焦点问题 | 单 PC 1440×900 | critical API success；DB/audit 明确 N/A；snapshot+screenshot；console/page/request failure=0 | `e2e/acceptance/parameter-catalog.acceptance.spec.ts` |
| `PCAT-UI-15`（#847 停用其 Proposal 段：依赖已移除的「定义修订」面板） | automated | Org Admin、Platform Admin、ordinary/Agent negatives | 真实导航、搜索、详情、timeline、queue、registration、proposal、conflict、deep link、keyboard/focus | 单 PC 1440×900 | operation request ledger；final DB predicates；每个 mutation/refusal audit；checkpoint screenshots + runtime/trace/report pins | `e2e/acceptance/parameter-catalog-governance.acceptance.spec.ts` |
...
`PCAT-UI-16`（定义集合工作区分页与真实计数，单 PC 1440×900）与 `PCAT-UI-17`（每行一个弃用/恢复动作 + 一个身份纠错动作）已在 `e2e/acceptance/parameter-catalog-definitions.acceptance.spec.ts` 覆盖。`PCAT-UI-17` 另要求纠错对话框在预览时携带 release pin 与幂等键、在 create/continue 上携带 `If-Match`，并完整走通成功路径：真实浏览器 + 真实 API + 真实发布管理器下，影响预览为 可迁移 1 / 被阻止 0，执行结果为 `completed`、项目"已完成"，迁移后的取值保留 `.dts` 源位置；当前 PC 要求为 1440×900，历史 `work/ui-checks/847/correction-{form,preview,result}-{desktop,tablet,mobile}.png` 仍保留但不作为当前 PC 要求（`work/` 不入库）。单元/集成证据：`src/features/parameter-catalog-governance/DefinitionCorrectionDialog.test.tsx`、`server/modules/parameter-catalog-migration/provenance.integration.test.ts`。
