...
- 升级数据平面就绪按服务定义：PostgreSQL/Redis 只认 Docker `healthy`，MinIO `running` 只代表进程存活，`minio-init` 成功退出 `0` 才证明 endpoint、凭据和 bucket 可用。写入 `old-stack-restored` 前，独立恢复 helper 还会检查数据平面、queue resume、API、worker `8788` liveness/Docker health、web、旧 image identity 和 proxy/public；失败保持 `recovery-required`、停止 proxy、暂停 queue，并保存有长度限制且脱敏的 `failed_phase`、`failure_service`、`failure_code`、`failure_summary`、`recovery_started`、`recovery_verified`、`next_action`。本地 mock 测试不能代替目标环境证据。
