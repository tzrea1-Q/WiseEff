...
既有的 `0117_user_account_deletion.sql` 迁移保持原样不变。之后生效驱动参数目录使用迁移
`0118_effective_driver_parameter_catalog.sql` 扩展主体/放置和对账证据，
`0119_effective_driver_parameter_catalog_contract.sql` 守住新的 DTS 身份写入，
`0120_effective_driver_parameter_catalog_finalize.sql` 完成安全图回填并拒绝未来重复
active 版本；`0121_effective_driver_parameter_catalog_legacy_write_compat.sql` 保留旧版
...
或属性键不一致的身份写入。验证门禁也会阻断重复的 active node-type source/property 身份，
...
- 升级数据平面就绪按服务定义：PostgreSQL/Redis 只认 Docker `healthy`，MinIO `running` 只代表进程存活，`minio-init` 成功退出 `0` 才证明 endpoint、凭据和 bucket 可用；等待 initializer 的每一轮和退出 `0` 后都会复查 MinIO。候选 `apply` 和 `resume` 在 queue resume 前都要求 API readiness、worker `8788` liveness/Docker health 和 web 直连；`queue-resumed`、`starting-proxy`、`validating-public` 三个 advanced resume 阶段还会先隔离候选 proxy，在 API、worker、web 全部通过前保持 proxy 关闭，worker 不健康时阻止 proxy/public。最终验证使用 Docker 支持的格式化镜像查询、按无序身份集合比较 named-volume，并为每个失败不变量记录稳定错误码。写入 `old-stack-restored` 前，独立恢复 helper 先检查数据平面、API、worker 健康、web、旧 image identity，再恢复 queue、重建 proxy 并最后检查 proxy/public；失败保持 `recovery-required`，保存 proxy 或 queue/worker 隔离是否真实成功，并保存有长度限制且脱敏的 `failed_phase`、`failure_service`、`failure_code`、`failure_summary`、`recovery_started`、`recovery_verified`、必要时的 `recovery_failure_summary` 和一个可执行的 `next_action`：迁移前旧栈 `resume`；符合条件的迁移后收尾失败使用带 token、保留数据的 `recover-candidate`；更早或不受支持的迁移后失败使用带 token 的整点 rollback。候选恢复会重新隔离流量，校验恢复点 manifest 与本地候选镜像，再按 worker、queue、proxy、公网验证恢复，绝不恢复数据。本地 mock 测试不能代替目标环境证据。
