...
- 升级数据平面就绪按服务定义：PostgreSQL/Redis 只认 Docker `healthy`，MinIO `running` 只代表进程存活，`minio-init` 成功退出 `0` 才证明 endpoint、凭据和 bucket 可用；等待 initializer 的每一轮和退出 `0` 后都会复查 MinIO。候选 `apply` 和 `resume` 在 queue resume 前都要求 API readiness、worker `8788` liveness/Docker health 和 web 直连；`queue-resumed`、`starting-proxy`、`validating-public` 三个 advanced resume 阶段还会先隔离候选 proxy，在 API、worker、web 全部通过前保持 proxy 关闭，worker 不健康时阻止 proxy/public。写入 `old-stack-restored` 前，独立恢复 helper 先检查数据平面、API、worker 健康、web、旧 image identity，再恢复 queue、重建 proxy 并最后检查 proxy/public；失败保持 `recovery-required`，保存 proxy 或 queue/worker 隔离是否真实成功，并保存有长度限制且脱敏的 `failed_phase`、`failure_service`、`failure_code`、`failure_summary`、`recovery_started`、`recovery_verified`、必要时的 `recovery_failure_summary` 和一个可执行的 `next_action`：迁移前旧栈 `resume`，或迁移后带 token 的 rollback。本地 mock 测试不能代替目标环境证据。
