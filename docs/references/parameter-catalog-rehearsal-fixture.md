...
The historical source commit is `6c3adfc35c0e3be6d5d381013dace9408190380e`; its historical bundle SHA-256 is `017b3e614f1f4eba5a70f0c6b0cd3316b7e5ebd1aa9ccec4cf8e514c56dba7ff`. Both are immutable provenance only. They are not executable trust and are never recomputed from repaired bytes. The external source-lock test pins the exact checked-out 18 paths and regular-file modes, every repaired blob hash, and the new length-framed bundle checksum `B` without including itself. Evidence has three bounded modes: a direct feature head has one raw `HEAD` parent equal to `R`; a complete historical descendant or merge proves `R` is an ancestor, follows all descendant ancestry paths to identify exactly one post-`R` lock-only child, and proves the 18 source paths do not drift from `R` to `HEAD` while later merges leave the lock path untouched. After that lock child, every feature-lineage commit and parent must retain the lock child's exact 19-path tree entries (path, mode, and blob), even if a later merge restores the final tree; an older base parent may legitimately lack newly added paths. A complete merge may therefore have a base parent that predates the new lock file, but at least one parent must carry the exact lock-child lineage, and its 19 entries plus the merge tree's 19 entries must equal the final lock entries; any lineage or merge-tree drift fails. A GitHub Actions depth-one merge may lack `R` and is accepted only when trusted `GITHUB_EVENT_PATH`, `GITHUB_SHA`, and event name bind the current `HEAD` and merge parents (a pull-request head SHA must appear among those parents, while a push event's `after` and `head_commit.id` must equal `HEAD`). Every shallow mode verifies the `HEAD` tree and working-tree bytes and modes for all 18 paths against the embedded manifest and recomputes `B`; it never claims unavailable `R`-object proof. A root or locally forged merge without trusted event evidence fails.
...
The retained source profile remains aggregate evidence from the populated self-hosted database. The `wf671-` graph is representative synthetic data derived from those observed cohorts, not a redacted row-for-row production clone. Static/source-lock results are D/L evidence. Complete-clone evidence covers R-object tree and ancestry, all descendant paths to the one lock-only child, requires every feature-lineage commit and parent after that child to retain all 19 exact entries, permits an older base merge parent only when a lineage parent carries that child, and rejects any lineage or merge-tree entry drift; a direct shallow feature head covers only its raw parent and checked-out manifest; a GitHub Actions shallow merge additionally requires trusted event-file binding and covers only the checked-out merge tree/working tree, never unavailable R-object proof. Root or locally forged merges without that event binding are not evidence. A selected, non-skipped run against a real local PostgreSQL server is PG evidence only. Neither local synthetic nor local PG results imply Hosted, target-host, release, production approval, or compatibility-window evidence.
