...
The historical source commit is `6c3adfc35c0e3be6d5d381013dace9408190380e`; its historical bundle SHA-256 is `017b3e614f1f4eba5a70f0c6b0cd3316b7e5ebd1aa9ccec4cf8e514c56dba7ff`. Both are immutable provenance only. They are not executable trust and are never recomputed from repaired bytes. The external source-lock test pins the exact checked-out 18 paths and regular-file modes, every repaired blob hash, and the new length-framed bundle checksum `B` without including itself. In a depth-one checkout, an embedded independent expected manifest proves the checked-out bytes; when historical Git objects are present, the same test additionally recomputes the historical provenance evidence.
