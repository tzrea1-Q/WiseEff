...
- Runtime seams: local/S3-compatible object storage, dedicated log worker runner, Redis/BullMQ durable queue mode for log-analysis dispatch, simulator/HDC device gateway, deterministic/live Agent provider with WiseEff HTTP, OpenAI-compatible, and Pi-backed formats.
- Observability: M6.5 adds `GET /metrics`, log-analysis terminal job duration/failure-reason counters, Agent provider call counters, provider-readiness evidence labels, device gateway operation counters, HTTP route-template spans, Agent provider spans, debugging gateway spans, `npm run observability:check`, self-hosted Prometheus config, alert rules, Grafana dashboards, and incident/observability runbooks. Target-environment scrape and trace-collector evidence still has to be collected before calling an environment observability-ready.
