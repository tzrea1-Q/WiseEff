...
- Observability: M6.5 adds `GET /metrics`, Agent provider call counters, device gateway operation counters, `npm run observability:check`, self-hosted Prometheus config, alert rules, Grafana dashboards, and incident/observability runbooks. Target-environment scrape evidence still has to be collected before calling an environment observability-ready.
