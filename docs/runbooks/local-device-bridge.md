...
- **Switching WiseEff accounts on the same machine**: `GET /api/v1/device-bridges/mine` stays user+organization scoped. After logout from account A and login as account B, local health may still report A's `bridgeId` while B's `/mine` list does not include it. The page asks B to **rebind** the machine with a new pairing code; it does not describe that state as an expired token, and B cannot list, rename, revoke, or debug through A's bridge. Rebind issues B a new bridge id/token, force-restarts the local process, and only then auto-detects. Install Bridge 0.1.1 or later; an older installer can pair but leave the previous process running.
