...
The stock self-hosted runtime supports exactly one API replica. Run Compose through `ops/self-hosted/scripts/compose`; it rejects `api>1` scale options before Docker runs and continues to pass `api=1` or scaling for unrelated services.
