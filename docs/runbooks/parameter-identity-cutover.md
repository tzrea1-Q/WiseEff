...
Apply all forward SQL migrations first. In particular, migrations `0059` through `0062` must exist in `schema_migrations` before accepting post-cutover typed drafts. Migration 0060 first records/deletes pre-existing candidate-less manual drafts. Forward migration 0061 closes the origin gap: it records and deletes **every** remaining draft without `candidate_config_revision_id`, including `file_sync` and conflict-derived rows, and preserves origin/file-version identifiers in `parameter_draft_identity_invalidations`. This is intentional invalidation, not a guessed backfill: notify affected users and require them to recreate the edit through the typed binding editor after cutover. Never attach an arbitrary candidate to preserve an old draft. Migration 0062 persists the reviewed `set|delete` action through draft, submission item, and change request.

If the database is already at 0059 or 0060, capture the count of **all** candidate-less drafts during the write freeze before applying the remaining migrations. After 0061, the active candidate-less count must be zero and invalidation evidence must cover the captured identifiers/origins. Candidate-backed drafts remain active. Legacy draft-save and `parameterId`-only submission return `409` after cutover. Exact submission additionally locks the draft row. A `set` proves candidate binding raw-value equality; a `delete` proves the candidate has no replacement binding revision and has the matching delete occurrence effect. Both actions retain the exact write lock and fail closed on contradictory proof.
