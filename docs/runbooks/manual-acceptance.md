...
The populated review visual fixture is allowed only on an ephemeral database owned by the acceptance run. Before `npm run seed:quality:visual-review` or `npm run cleanup:quality:visual-review`, set `WISEEFF_QUALITY_ALLOW_VISUAL_FIXTURE=true` and set `WISEEFF_QUALITY_FIXTURE_DATABASE_NAME` to the exact result of `current_database()`. The commands fail closed before mutation on a name mismatch or fixed-ID ownership collision. Never set these variables against a shared, customer, staging, or target synthetic database. Target synthetic quality deliberately planned-skips only the populated `/parameter-review` visual case and must still run every other visual case plus the a11y/responsive projects. For an intentional baseline change, update only named screenshots, inspect every changed PNG, and rerun without `--update-snapshots`; do not use wholesale snapshot acceptance.

M5.12 adds CI and target synthetic archiving for these gates. The PR merge bar is L1 (`detect`, `docs:check`, `build-and-test`, `@ci-smoke` on product paths, one quality-run on UI/product paths, sentinel `Merge bar`). The GitHub Actions `acceptance-local-non-hdc` job is L2: it runs on `main`, nightly, label `full-acceptance`, and `workflow_dispatch` `local-non-hdc`, provisions PostgreSQL, runs the state-model gate, one quality-run, and local non-HDC browser acceptance, then uploads `wiseeff-acceptance-local-non-hdc`. Manual `workflow_dispatch` runs can choose `target-non-hdc` or `full-pilot`; these use `--no-start-runtime`, the provided target frontend URL, GitHub Secrets for target API/auth inputs, and upload `wiseeff-acceptance-<mode>`. Treat these artifacts as acceptance evidence attachments:
