...
| Reliability | 7.5/10 | Deployment and reliability docs exist; `/health/live`, `/health/ready`, and the M5 pilot readiness gate cover the release baseline; M2 has local object storage, job polling, leased jobs, failed records, and rerun support; M6.4 adds Redis/BullMQ durable dispatch while PostgreSQL remains the source of truth; M6.5 adds `/metrics`, Prometheus config, alert rules, Grafana dashboards, and local observability checks. | Target Redis evidence, durable object storage target evidence, deep Agent/device/audit business metrics, target Prometheus/Grafana/Alertmanager evidence, release rollback, capacity, and external pilot evidence remain future work. |
