...
| --- | ---: | --- | --- |
| Frontend prototype | 8.2/10 | Broad Vitest and Testing Library coverage across pages, components, permissions, admin flows, logs, debugging, Agent UI, strict API bootstrap state, and API-mode no-demo-fallback regression tests. | Mock mode remains intentionally available for demos/tests, so parity coverage must stay close to each workflow. |
