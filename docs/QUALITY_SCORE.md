...
| Production/pilot evidence | 7.5/10 | M5 gates exist, PR #39 merged, GitHub CI passed, `docs:check` guards active plan metadata, M5.2 local PostgreSQL-backed API-mode E2E passed, local `/health/ready` is green for database/object store/worker/live Agent health, local non-HDC smoke passed with only `deviceGateway` blocked, browser acceptance passed in local non-HDC mode, M5.9 adds deterministic state-model checks for parameter/log/debugging/permission invariants, M5.10 operation evidence now includes required API/DB/audit/runtime/trace/reproduction summaries for declared assertions, M5.11 adds deterministic accessibility, visual, and responsive browser quality gates, M5.12 adds CI local non-HDC acceptance and manual target/full-pilot synthetic artifact archiving, M6.1 adds self-hosted runtime config/smoke gates, M6.6 adds release-gate and capacity-gate evidence writers, production-auth API-mode E2E passed with static bearer injection, live Agent chat passed with a longer timeout, and a local PostgreSQL plus local object-store backup/restore drill passed. | Full target-environment staging evidence still has to be run and reviewed, and HDC device-lab, deployment rollback rehearsal, self-hosted object-store backup evidence, dynamic production identity/OIDC, durable queue, target observability, target capacity metrics, live-provider tool-flow evaluation, and strict full-pilot `npm run smoke:m5` remain open. |
