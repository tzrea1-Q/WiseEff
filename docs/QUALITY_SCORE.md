...
| Reliability | 7.3/10 | Deployment and reliability docs exist; `/health/live`, `/health/ready`, and the M5 pilot readiness gate cover the release baseline; M2 has local object storage, job polling, leased jobs, failed records, and rerun support; M6.4 adds Redis/BullMQ durable dispatch while PostgreSQL remains the source of truth for job state, retries, dead-letter metadata, audit, and evidence. | Target Redis evidence, durable object storage target evidence, SSE hardening, real gateway observability, production monitoring, queue metrics/capacity tuning, and external pilot evidence remain future work. |
