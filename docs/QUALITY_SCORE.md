...
| Backend M0/M1/M2 foundation | 7/10 | TypeScript server skeleton, auth context, audit routes, M1 parameter services, M2 log migrations/services/worker, CORS preflight support, backend tests, and API-mode Playwright smokes. | E2E currently requires external PostgreSQL setup, the worker is in-process, and API contracts are still handwritten. |
