...
| Reliability | 7.3/10 | Deployment and reliability docs exist; `/health/live`, `/health/ready`, and the M5 pilot readiness gate cover the release baseline; M2 has local object storage, job polling, leased jobs, failed records, and rerun support; M6.4 adds Redis/BullMQ durable dispatch while PostgreSQL remains the source of truth for job state, retries, dead-letter metadata, audit, and evidence; M6.5 adds `/metrics` plus Prometheus/Grafana/alert configuration. | Target Redis evidence, durable object storage target evidence, target Prometheus/Grafana/Alertmanager evidence, SSE hardening, real gateway observability, queue metrics/capacity tuning, release rollback, capacity, and external pilot evidence remain future work. |
...
| Production/pilot evidence | 7.5/10 | M5 gates exist, PR #39 merged, GitHub CI passed, `docs:check` guards active plan metadata, M5.2 local PostgreSQL-backed API-mode E2E passed, local `/health/ready` is green for database/object store/worker/live Agent health, local non-HDC smoke passed with only `deviceGateway` blocked, browser acceptance passed in local non-HDC mode, M5.9 adds deterministic state-model checks, M5.10 evidence-grade operation records, M5.11 accessibility/visual/responsive gates, M5.12 CI local non-HDC acceptance, M6.1 self-hosted runtime config/smoke gates, M6.3 self-hosted S3-compatible readiness probes plus backup/restore evidence and restore-target safety gates, M6.4 local durable queue checks, M6.5 local observability config/runtime gates, and M6.6 release-gate plus capacity-gate evidence writers. | Full target-environment staging evidence still has to be run and reviewed, and HDC device-lab, deployment rollback rehearsal, real self-hosted target restore evidence, dynamic production identity/OIDC target evidence, target Redis/BullMQ queue evidence, target observability scrape/alert/dashboard evidence, target capacity metrics, live-provider tool-flow evaluation, and strict full-pilot `npm run smoke:m5` remain open. |
...
M6.1 is covered by self-hosted compose/env/proxy metadata checks and a smoke runner for deployed Linux targets. M6.6 adds `npm run capacity:gate`, `npm run selfhost:release-gate`, `ops/self-hosted/releases/`, and the release/rollback runbook so a release candidate has version, artifact, migration, backup, rollback, capacity, synthetic acceptance, and HDC-scope evidence slots.

Remaining M6 risks: the M6.2-M6.5 implementation PRs and target evidence are still separate workstreams. M6.6 local script output is not a capacity pass or rollback pass unless backed by target metrics, rollback rehearsal, queue drain/pause/resume, observability snapshots, and target synthetic artifacts.
