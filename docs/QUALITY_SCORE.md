...
| Reliability | 7.3/10 | Deployment and reliability docs exist; `/health/live`, `/health/ready`, the M5 pilot readiness gate, M6.1 self-hosted checks, and M6.5 `/metrics` plus Prometheus/Grafana/alert configuration cover the release and observability baseline. M2 has local object storage, job polling, leased jobs, failed records, and rerun support. | Durable queue target evidence, self-hosted object-store backup evidence, deep Agent/device/audit business metrics, target Prometheus/Grafana/Alertmanager evidence, release rollback, capacity, and external pilot evidence remain future work. |
