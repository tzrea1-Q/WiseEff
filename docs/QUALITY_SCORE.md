...
- Run `npm run acceptance:gate0` for the authoritative owned-runtime local L2 baseline. Use `npm run acceptance:browser` directly only for focused/manual browser candidates, and run `npm run acceptance:evidence` when operation-evidence coverage changes. Gate0 artifacts are uploadable only after recursive credential redaction and a zero-violation `npm run acceptance:artifacts:check -- --root test-results/acceptance-runtime-runs` result.
