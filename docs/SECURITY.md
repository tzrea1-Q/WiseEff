...
**Owned acceptance subprocess and evidence boundary:** Gate0 phases and nested disposable runtimes never inherit `process.env` wholesale. Their complete child environment is rebuilt from a small OS launch allowlist plus the exact owned runtime variables; PostgreSQL passwords, Docker/Git credential helpers, CI job JWTs, provider keys, authorization values, and generic token/secret/password variables cross the boundary only when the owned runtime explicitly defines them. Root and nested credentials generated during one run are registered through a loopback-only, token-authenticated sink held in owner memory and are explicitly supplied to artifact sanitization and scanning; the nested ownership manifest remains secret-free. Owned operation evidence points only to existing regular report/trace descendants of its run root and hashes an immutable runtime snapshot, never the descriptor whose phase and cleanup fields continue to change.
