...
**Canonical Parameter Catalog database roles (S2-RBAC):** Catalog relations live in `parameter_catalog` and are owned by the NOLOGIN role `catalog_migration_owner`. `catalog_synchronizer_role` (NOLOGIN) may insert immutable Catalog rows and perform only column-limited current-release / Definition-head updates (`catalog_state.current_catalog_release_id`, `parameter_definitions.current_revision_id`). `parameter_governance_writer_role` (NOLOGIN) has necessary Organization-governance DML, append-only `INSERT` on `public.audit_events`, and `EXECUTE` only on `parameter_catalog.assert_catalog_subject_active(text,text,text,text)`. It has no Catalog table `SELECT` or DML and no Binding, Cutover, or Verification grants. `PUBLIC` remains revoked on the Catalog schema, tables, sequences, and functions. Application, Agent, ordinary API/worker, and verifier logins are not members of these roles and cannot `SET ROLE` to them. Direct Catalog `SELECT`/DML by a production role fails with SQLSTATE `42501` and is classified as `PCAT-DB-P01` / `PCAT-PRIV-CATALOG-IMMUTABILITY-BYPASS`. Legacy structural writes on public parameter tables by those roles fail with SQLSTATE `42501` and are classified as `PCAT-DB-P02` / `PCAT-PRIV-LEGACY-WRITER-BYPASS`. Only the Parameter Governance composition root holds a pool that may assume the governance writer.
