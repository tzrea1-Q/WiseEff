...
**Canonical Parameter Catalog database roles (S2-RBAC):** Catalog relations live in `parameter_catalog` and are owned by the NOLOGIN role `catalog_migration_owner`, which also holds `SELECT` on the public FK targets used by Catalog internals, `SELECT` on every public relation read by Catalog `SECURITY DEFINER` functions (including `parameter_catalog.resolve_legacy_identity_owner`), and `UPDATE` on `public.parameter_modules` so the SECURITY DEFINER placement lock can `SELECT ... FOR SHARE`. `catalog_synchronizer_role` (NOLOGIN) may insert immutable Catalog rows, perform only column-limited current-release / Definition-head updates (`catalog_state.current_catalog_release_id`, `parameter_definitions.current_revision_id`), and `EXECUTE` the CHECK identity predicates plus `parameter_catalog.acquire_current_pointer_lock_exclusive()`. It does not receive `EXECUTE` on trigger-only functions. `parameter_governance_writer_role` (NOLOGIN) has necessary Organization-governance DML, append-only `INSERT` on `public.audit_events`, and `EXECUTE` only on `parameter_catalog.assert_catalog_subject_active(text,text,text,text)`. It has no Catalog table `SELECT` or DML, no Binding, Cutover, or Verification grants, and no `EXECUTE` on CHECK predicates, the exclusive lock, or trigger-only functions. Deferred `assert_subject_placement_kind` and `assert_observation_match_binding_revision` guards are `SECURITY DEFINER` so writer inserts can fire them without Catalog or Binding table grants. `PUBLIC` remains revoked on the Catalog schema, tables, sequences, and functions. Application, Agent, ordinary API/worker, and verifier logins are not members of these roles and cannot `SET ROLE` to them. Direct Catalog `SELECT`/DML by a production role fails with SQLSTATE `42501` and is classified as `PCAT-DB-P01` / `PCAT-PRIV-CATALOG-IMMUTABILITY-BYPASS`. Legacy structural writes on public parameter tables by those roles fail with SQLSTATE `42501` and are classified as `PCAT-DB-P02` / `PCAT-PRIV-LEGACY-WRITER-BYPASS`. Only the Parameter Governance composition root holds a pool that may assume the governance writer.
