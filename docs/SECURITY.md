...
The live Agent provider registry supports `AGENT_API_FORMAT=wiseeff` and `openai`. Provider output is advisory until the WiseEff tool registry, authorization, approval, and audit paths accept it. Safe provider evidence may expose provider mode, provider format, model id, and prompt version in readiness JSON and traces; it must not expose API keys, Authorization headers, raw prompts, raw provider payloads, or customer data. Provider traces capture latency, token usage, estimated cost, safety status, safety reasons, and fallback reason so security review can distinguish grounded planning from degraded output.
