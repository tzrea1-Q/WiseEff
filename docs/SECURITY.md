...
- DTS reload debugging requires `debugging:dts-reload`. Sensitive-node matches escalate as documented above (`parameter:edit-critical`, and `confirm-sensitive-reload` for critical). Device deploy additionally requires `confirm-dts-reload`, a device lease, bridge capability negotiation for `debug.mountTarget` / `debug.pushFile`, and a reload snapshot (ADR-0021). Deploy executes in-request on the process holding the bridge WebSocket (ADR-0020).
