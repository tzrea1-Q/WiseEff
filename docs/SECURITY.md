...
- **Tenant isolation:** the worker reads log bytes, log-domain rows, knowledge links, and parameter context through organization-scoped repository queries; log-domain governance (including knowledge links) requires `logs:admin-domains` and is audited (`log-domain-*`).
