...
- Backend user governance lives under `/api/v1/users` and requires `users:manage`, durable role updates, self-lockout prevention, and audit evidence. Permanent deletion is non-self and organization-scoped; ordinary Admins cannot delete platform super admins.
...
**Owned acceptance subprocess and evidence boundary:** Gate0 phases and nested disposable runtimes never inherit `process.env` wholesale. Their complete child environment is rebuilt from a small OS launch allowlist plus the exact owned runtime variables; PostgreSQL passwords, Docker/Git credential helpers, CI job JWTs, provider keys, authorization values, and generic token/secret/password variables cross the boundary only when the owned runtime explicitly defines them. Root and nested credentials generated during one run are registered through a loopback-only, token-authenticated sink and are acknowledged only after encrypted exact-value context is durably published. Root, phase, and nested API/frontend process-start identities are persisted without credentials. After an owner crash, the fresh CI finalizer proves the owner exited, stops every identity-matched writer before TERM/KILL, retains DB/object forensic resources, sanitizes and scans one private staging tree, and atomically publishes one scanned read-only ZIP; CI uploads only that ZIP. The nested ownership manifest remains secret-free. Owned operation evidence points only to existing regular report/trace descendants of its run root and hashes an immutable runtime snapshot, never the descriptor whose phase and cleanup fields continue to change.
...
**Canonical Parameter Catalog database roles (S2-RBAC):** Catalog relations live in `parameter_catalog` and are owned by the NOLOGIN role `catalog_migration_owner`, which also holds `SELECT` on the public FK targets used by Catalog internals, `SELECT` on every public relation read by Catalog `SECURITY DEFINER` functions (including `parameter_catalog.resolve_legacy_identity_owner`), and `UPDATE` on `public.parameter_modules` so the SECURITY DEFINER placement lock can `SELECT ... FOR SHARE`. `catalog_synchronizer_role` (NOLOGIN) may insert immutable Catalog rows, perform only column-limited current-release / Definition-head updates (`catalog_state.current_catalog_release_id`, `parameter_definitions.current_revision_id`), and `EXECUTE` the CHECK identity predicates plus `parameter_catalog.acquire_current_pointer_lock_exclusive()`. It does not receive `EXECUTE` on trigger-only functions. It may `INSERT` `parameter_catalog.catalog_activation_receipts` and `SELECT` `catalog_publication` relations needed to verify publication tuples; it cannot insert artifacts, candidates, jobs, or authorizations, and cannot `EXECUTE` `catalog_publication.revise_publication_policy`. `parameter_governance_writer_role` (NOLOGIN) has necessary Organization-governance DML, append-only `INSERT` on `public.audit_events`, and `EXECUTE` only on `parameter_catalog.assert_catalog_subject_active(text,text,text,text)`. It has no Catalog table `SELECT` or DML, no Binding, Cutover, Verification, or publication-schema grants, no receipt writes, and no `EXECUTE` on CHECK predicates, the exclusive lock, or trigger-only functions. Deferred `assert_subject_placement_kind` and `assert_observation_match_binding_revision` guards are `SECURITY DEFINER` so writer inserts can fire them without Catalog or Binding table grants. `catalog_publication` is a separate schema owned by `catalog_migration_owner`, with `PUBLIC` revoked. `catalog_publication_coordinator_role` (NOLOGIN) may insert artifacts, candidates, authorizations, and jobs, perform column-limited job execution updates, `UPDATE` `publication_guard`, `SELECT` `publication_freeze`, and `EXECUTE` `catalog_publication.set_publication_freeze` plus the publication guard lock; it cannot `EXECUTE` `catalog_publication.revise_publication_policy` (CP-04's TypeScript management wrapper records a trusted actor and still does not GRANT coordinator EXECUTE; isolated tests SET ROLE `catalog_migration_owner` only), cannot write Catalog core tables or receipts, and cannot `SET ROLE` to the synchronizer or migration owner. `catalog:author` / `catalog:publish` / `catalog:review-high-risk` are closed-union capabilities injected on `AuthContext.permissions` in tests; default Admin / org-admin / Agent matrices stay catalog-read-only. Request-body `role` / `organization` / `riskClass` / `approved` are untrusted. Seeded `publication_policies` keep `publication_enabled=false` and `low_risk_single_actor_publish=false`; this is not production enablement, and the management wrapper refuses to enable publication on a shared/non-ephemeral database. `catalog_baseline_reader_role` (NOLOGIN) has `SELECT` on Catalog and publication relations only. Ordinary web/API is a LOGIN with no catalog or publication write grants (this codebase has no `application_role`). `PUBLIC` remains revoked on the Catalog and publication schemas, tables, sequences, and functions. Application, Agent, ordinary API/worker, and verifier logins are not members of these roles and cannot `SET ROLE` to them. Direct Catalog `SELECT`/DML by a production role fails with SQLSTATE `42501` and is classified as `PCAT-DB-P01` / `PCAT-PRIV-CATALOG-IMMUTABILITY-BYPASS`. Legacy structural writes on public parameter tables by those roles fail with SQLSTATE `42501` and are classified as `PCAT-DB-P02` / `PCAT-PRIV-LEGACY-WRITER-BYPASS`. Only the Parameter Governance composition root holds a pool that may assume the governance writer.

**Ordinary debug-node write safety:** A runtime node write is allowed only for RW access because WiseEff must capture a pre-write value and rollback snapshot before touching the device; true WO nodes are rejected rather than written without recovery evidence. For an accepted write, write-command execution and post-write readback are independent facts. A different observed representation is not authorization or success evidence and is not treated as failure; a technical readback failure produces a warning and stale-current-value marker. Snapshot rollback remains a separate, strict restore-verification operation.
...
**Trusted invocation provenance decision (ADR-0038 / TD-068):** `AuthContext` remains the authenticated and accountable user principal; it does not claim that a physical human is behind an ordinary bearer credential. #610 implements the shared server-internal discriminated invocation context (`user` / `agent` / `system`), strict server-owned constructors, and required policy/audit seams. Agent context carries session/tool-call correlation and, for approval-requiring mutations, the approval id; approval authorizes the action but never changes the initiator to `user`. System context names a service/job and never invents a user. Client headers/bodies cannot declare this context, omission never defaults to `user`, and human-required policy accepts only the `user` initiator. At source baseline `67d4a77325b6009b77c2373bd788298a6d022bcf`, `agent/orchestrator.ts` constructs Agent invocation context and `dts-reload/routes.ts` constructs user invocation context for the reload mutation routes; domain services validate that context. Review each production write's propagation and its focused tests when changing it. This source statement does not close the broader TD-068 program or establish release evidence. Legacy caller-supplied `actorType` is not a trusted provenance boundary. Mock mode is not security proof.
...
M3 debugging emits backend audit events for target detection, session creation, node reads, node writes, and snapshot rollback. Ordinary-write audit metadata includes the session, operation, node path, requested value, previous value, observed readback value, independent `writeOutcome` / `readbackOutcome`, failure reason, and snapshot id when applicable. Ordinary writes do not use representation equality as a verification result; strict `verified` comparison remains limited to rollback and other explicitly verifying workflows.
...
M6.2 user-governance writes emit backend audit events for user creation, permanent deletion, profile update, activation/deactivation, role replacement, and local registration role-request approval/rejection. These mutations must stay transactionally coupled to durable state updates and must prevent the active Admin from removing its own final Admin capability. Permanent deletion cascades account-owned credentials, sessions, role bindings, personal notifications, leases, and drafts; durable business/audit rows remain with nullable `ON DELETE SET NULL` user references. The High-severity delete audit retains the opaque target id and non-secret lifecycle facts, never deleted-user PII or credential material.

Local account auth writes backend audit events for registration, login, failed login (`login-failed`), logout, current-user password change, Admin password reset, and current-user profile updates. Logout must revoke the active session token server-side. Password change keeps the current session and revokes the others; Admin reset revokes every session for the target user. Profile updates must not allow the current-user route to change email, roles, activation state, or organization.
