...
Implementation status, 2026-09-15 Scratch round on branch `feat/849-parameter-unification` (base `8f03cfa4302aebbe3bc3c37ef2197c082c2a3e2e`). Delivered with real PostgreSQL and real-browser evidence: the four named coexistence defects in `server/modules/parameter-bindings/catalogProjectValueRoutes.ts` are removed (legacy merge, empty-canonical fallback, direct-save "draft", whole-batch legacy apply fallback); a canonical pending-value draft owner now exists (`project_parameter_value_drafts`, migration `0144`) with create/list/remove and the invariant that drafting never moves the current value, its tip or the active source revision; and a canonical submit → review → apply vertical now exists (`project_parameter_value_change_requests`, migration `0145`) where submission freezes the canonical identity and base pins, approval re-resolves them and commits the value, its protected source writeback, the ProjectValue history, workflow status and audit in one owned transaction, rejection and withdrawal write nothing, stale pins are rejected, self-approval is refused and an already-applied request replays idempotently. The canonical binding change history (`parameter_catalog.binding_history_events`) now has a read surface (`GET /api/v2/projects/:projectId/parameter-bindings/:bindingId/change-history`) exposing exact value and definition-revision pins, the recorded reason and the success audit reference, without archived legacy payloads. DTS/JSON project sources remain the only supported formats and YAML/TOML/ENV are now refused with an explicit `UNSUPPORTED_FORMAT` outcome before staging/apply (vendor YAML Catalog metadata still imports); and a deterministic, checked-in seed reconciliation manifest covers all 125 inputs. The four current-scope compatibility seeds now have reviewed real per-project source files (`src/config/seed-sources/<project>/power-config.json` and `charging-thermal.dts`) with exact locators registered in the manifest, and the DTS/JSON writeback fidelity of those exact locators is proven. **Not delivered this round:** PU-01 ConfigurationSchema end-to-end publish evidence (Slices A, B and C — contract kinds, the hardened model-id parser, the three-way release schema, compiler validation and the re-pinned goldens — Slice B — migration 0146 storage closure with all six trigger functions redefined, relation count 45 and the recomputed schema fingerprint — and Slice C — install dispatch, snapshot reconstruction, cache reachability, the three-way matcher and the v3 capability revision — are landed and green; the review's Slice C exit evidence — one end-to-end publish of a configuration-schema release — is now covered by `configurationSchemaPublish.integration.test.ts`, so all three slices of the reviewed plan are landed. What remains for PU-01 is only what the review placed outside these slices. The two JSON seeds therefore still lack their formal governed subject and the two DTS seeds still lack a reviewed formal subject), the complete self-consistent demo DTS baselines for all 120 board occurrences with the dangling overlay targets resolved, seed publication and three-project initialization, the archive-rebuild profile and its S2 rehearsal, the remaining cross-domain consumer switch, and the full three-viewport operation matrix. See the round report for the exact per-item result, failure and risk list. Target execution and deployment remain unauthorized.
...
| D10 | Complete self-consistent demo DTS baselines for the three built-in projects, preserving business properties and project values. | Give the 24 dangling overlay targets explicit, reviewed source identity; reuse a formal Driver only with declared contract evidence or author a formal NodeType where justified. Do not persist resolver stubs or infer compatible by name similarity. Label the sources as demos, not real-device firmware. |
...
The existing 12 compatibility seeds have no `sourceFileName`/`sourceNodePath`. The two DTS fragments are not present in the three committed board files. Existing parameter-file ownership supports DTS and JSON; YAML/TOML/ENV writeback is absent. `logical-service` is only a Driver nature and does not bypass the authoritative compatible requirement. Any accepted new selector or subject therefore requires an explicit ADR and contract extension.
...
Verified mixed-path examples in `server/modules/parameter-bindings/catalogProjectValueRoutes.ts`: GET merges old and new bindings; an empty Catalog falls back to old data; the new-binding drafts endpoint directly saves a current value and returns that value ID as a draft ID; mixed import batches can fall back to the old apply service. These paths cannot remain in the unified current workflow.
...
The binding contract distinguishes a DTS logical-node occurrence from a configuration instance. Define a tagged source identity at the existing Binding boundary and include project, config-set context, instance identity, file identity, immutable file/config revision and a format-specific locator. Keep the same Binding/ProjectValue owners; do not add parallel software-config value tables. Audit the current `(project, logicalNode, definition)` uniqueness against config-set identity: either the existing logical-node identity already incorporates that context or the unique key must be extended explicitly. Never collapse two config sets or two instances that use the same definition.
...
The complete reconciliation inventory is 125 inputs: 113 vendor definitions plus 12 compatibility items. Current-round delivery is **117 inputs (113 vendor + two JSON + two DTS)**; the other eight have an explicit `defer` disposition linked to TD-124 with original metadata and project values retained. They are not active seeded definitions/bindings this round and their deferred conversion does not block current-scope acceptance. Unknown current-scope fields/identities remain blockers. Board reconciliation may require additional reviewed definitions or source corrections. Each board still has 176 raw occurrences, including 120 business and 56 structural occurrences; 61 currently match and 59 involve 24 dangling labels. D10's accepted complete demo baseline resolves their source identity; stubs are not authoritative NodeTypes. YAML-authored vendor definition documents remain in scope as Catalog metadata, distinct from deferred YAML project-source support.
...
The 59 currently unmatched business occurrences per board are grouped below by observed `&label`, not asserted subject identity. `server/modules/dts/resolver.ts` synthesizes unresolved labels as node names; `danglingAnchorStub.ts` explicitly disallows treating those stubs as persistent business nodes.
...
The 61 matchable occurrences comprise 25 Driver and 36 NodeType occurrences, reusing 56 definitions. Current `catalogProjectValueSync.ts` supplies `nodeTypeFallback: absent`; PU-03 must route actual no-Driver nodes through the authoritative NodeType fallback instead. The read-only matcher count is not proof that today's sync imports even those 61.
...
- Frontend work requires snapshots/screenshots and actual interactions at 1440x900, 768x1024 and 390x844, with console/network checks. Dedicated PostgreSQL, local, browser, Hosted and target proof remain distinct.
...
Current authorization covered documentation, factual investigation and publication of one specification Issue with `ready-for-agent`; #849 completes that publication. This task does not include implementation, child Issues, new dependency edges, PRs, merges, seed execution or deployment. Later implementation uses bounded Scratch branches with explicit path ownership, independent reviews and merge order. Do not absorb the partial #824 candidate or rewrite the frozen Wayfinder graph implicitly.
