...
Status: **local-non-HDC Gate0, smoke, and target-non-hdc are green. T3.2 as a whole row is not complete.** Gate0 `beab90bc9` / run `full-20260918t224937833z-beab90bc9cec-5f49a8e9` / helper PG **55438**: visual **passed**, browser **passed**, inventoried failures **0**, operation-evidence **passed**. Smoke on `58dab25a4` / :5174/:18787 / `wiseeff_t32`: **4 passed**. `acceptance:quality`, `acceptance:coverage`, and `acceptance:operations` **passed**. target-synthetic-acceptance **passed** on `d520964e4`. minimal-upgrade is still not pass. Not SEALED. Not pushed. No PR/Hosted.
...
| target-synthetic-acceptance | **passed** on `d520964e4`. Owned HMAC runtime `full-20260919t103129605z-d520964e4687-0393859b` on helper PG **55438** (DB `wiseeff_acceptance_full_20260919t10312960_d520964e_0393859b`), frontend `http://127.0.0.1:5180`, API `http://127.0.0.1:18800`. Command: `npm run acceptance:browser -- --mode target-non-hdc --no-start-runtime --runtime-descriptor <abs runtime.json> --frontend-url http://127.0.0.1:5180`. Preflight **passed** (`non_hdc_local`). Playwright **174 passed / 32 skipped / 0 failed**. Operation evidence **passed** (192 records, `invalid: none`). Status **passed**. Artifacts under `test-results/t32-target-synthetic-runtime/full-20260919t103129605z-d520964e4687-0393859b/`. Not Hosted; not production. |
| minimal-upgrade | **not pass.** Ran `npx tsx scripts/run-minimal-upgrade-acceptance.ts $(git rev-parse HEAD) "$(docker info --format '{{.ID}}')"`. Docker `linux/aarch64` vs required `linux/x86_64`. Assertion: `the existing self-hosted base-image contract requires native amd64`. T3.4a not SEALED. |

## Remaining program boundary

T3.2 is not a complete local-acceptance row. Local-non-HDC Gate0 on `beab90bc9` is green; smoke/quality/coverage/operations on later HEAD `58dab25a4` are green; target-non-hdc on `d520964e4` is green. Pickup: [t32-remaining-verification.md](t32-remaining-verification.md) (minimal-upgrade only). That row is not pass. T3.4a remains not SEALED.
