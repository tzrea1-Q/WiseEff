...
| B2-15 | Determinism / order | Same inputs in different file or change-set order produce the same successor digest, the same binding IDs, and the same oracle set. `buildCompleteSuccessor` already sorts the change set; seed file order must not mint identity | builder determinism + oracle |
