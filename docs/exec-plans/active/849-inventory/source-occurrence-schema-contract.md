...
#### Canonical deletion extension (2026-09-21)

The [workflow deletion amendment](source-occurrence-workflow-contract.md#canonical-property-deletion-amendment-2026-09-21) adds immutable `project_parameter_values.value_state text not null default 'present'`, constrained to `present|deleted`. Existing values remain present. Only the target of an approved canonical source-delete transaction appends a deleted value. Its retained payload/digest must equal the frozen present base value and is historical context, not a live value. Existing immutable-value protection covers the new column.

Extend the same source-pin relation with nullable `base_source_pin_id text`, `delete_request_id text` and `delete_proof jsonb`. All three are null for present pins and non-null for deleted pins. Deferred composite FKs bind the base pin and delete request to `(organization_id, project_id, binding_id)` using owner-complete unique keys. All pin columns remain immutable. A deleted ProjectValue requires exactly one matching delete pin; an ordinary present value cannot use it, and a deleted value cannot use an ordinary pin or remain unpinned.

The DTS delete locator has exactly `kind='dts-delete', nodeOccurrenceId, fileVersionId, propertyName`, all string fields. This is the explicit exception to the ordinary DTS `property_occurrence_id` requirement: that column is null because the final delete effect establishes absence. The exact revision/member/node graph and unique maximum-order delete effect remain mandatory. The JSON delete locator has exactly `kind='json-delete', rootPointer, pointer, parentPointer, memberKey, fileVersionId`, all strings; its root, decoded pointer tokens and file version must match the owned base pin and occurrence. Ordinary pin shapes and digest preimages are unchanged.

`delete_proof` is a strict object with no extra keys. Common string fields are `kind, beforeValueDigest, beforeSourceDigest, afterSourceDigest, scannerVersion`; each digest is canonical `sha256:` plus 64 lowercase hexadecimal characters. JSON uses `kind='json-delete-v1'`, `scannerVersion='json-span-v1'` and exactly `rootPointer, pointer, parentPointer, memberKey`. DTS uses `kind='dts-delete-v1'`, `scannerVersion='dts-cst-v1'` and exactly `nodeOccurrenceId, propertyName`. The graph, rather than a supplied effect ID, selects the unique final DTS effect.

The deferred owner check binds the base pin's present value to the request's `base_current_value_id` and `source_pin_id`; organization, project, Binding, Definition and source occurrence must agree. Require `action=delete`, approved status, exact `applied_value_id`, applied source result, history and audit linkage. Match the proof digests to the old value and old/new immutable file checksums; match candidate ID, base/proposed/diff digests and frozen manifests to the immutable candidate/request. Check the new pin's revision/member/file version against the applied source result. Reuse existing owner checks and 0152's graph/first-pin fences without weakening ordinary pin checks or rewriting prior migrations.

Current views and resolvers exclude deleted tips; exact-ID readers expose their state. The original deletion pin remains the terminal historical source proof on later sibling changes. Such commits advance only present values and prove every retained deleted anchor remains absent. No deletion-proof table, Binding rekey, re-add flow or deleted-tip propagation is introduced.
