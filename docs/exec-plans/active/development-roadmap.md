...
M5 的目标不是一次性完成完整企业生产上线，而是把 M0-M4 已完成的全链路能力硬化到可进入受控商用试点的状态。重点是减少合同漂移、替换开发态身份、让长任务/对象存储/设备网关/Agent Provider 具备生产边界，并补齐部署、监控、备份、回滚和验收门槛。
...
7. Release/operations：补齐 CI/CD 质量闸门、staging/prod smoke、监控告警、备份恢复演练、回滚手册和试点验收报告。

验收：

- `npm run test:m5`、`npm run test:all`、`npm run build`、关键 API-mode E2E 和 `git diff --check` 通过。
...
- staging 和 production 有独立部署配置、健康检查、备份/恢复、回滚演练记录和试点上线 checklist。
