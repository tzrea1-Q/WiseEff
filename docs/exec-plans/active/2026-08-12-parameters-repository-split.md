...
Slice 4 lands the shared-kernel decision slice 3 deferred (ADR-0028): cross-cutting parameter primitives live in a standalone, routes-less `server/modules/parameter-kernel/` that never imports `parameters`/`parameter-topology`/`parameter-files`/`parameter-drafts`. Same verbatim rules; no re-exports; all import sites repointed.
