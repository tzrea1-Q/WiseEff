# Parameters Repository Split — Slices 1–4 (Architecture Review Candidate 3)

- **Status:** Active (slice 1 merged as PR #321; slice 2 in review as PR #340; slice 3 implemented; slice 4 implemented — identity half of the kernel)
- **Branch:** slice 1 `refactor/parameters-repository-split-1` (from `main` @ `faa6cc19`); slice 2 `refactor/parameters-repository-split-2` (from `main` @ `5826b2b2`, after PR #328's identity-mode single seam); slice 3 `refactor/parameter-drafts-module` (stacked on slice 2 @ `6ffb07e8`, after the #336 test repoint); slice 4 `refactor/parameter-kernel-module` (stacked on slice 3 @ `2bfeb647`)
- **Owner:** Backend
...
- ~~**No parameters↔parameter-topology cycle break.**~~ — the drafts-ownership decision landed in slice 3 (ADR-0028), and slice 4 (ADR-0029) extracted the identity half of the shared-kernel seam (`parameterIdentityMode`, `legacyParameterIdentityNames`, `legacyParameterIdentityAdapter`). The cycle still closes through the deferred kernel members (topology → parameters: `policy`, `sensitiveNode`), the `getProjectById` project-read edge (topology → parameters), and parameters → topology imports of write-lock verification, `resolveInitializationSuggestion`, and migration `LEGACY_SQL`; the closing slice moves `policy`/`sensitiveNode` after their leaf dependencies (`status.ts` type, `impact.ts` node-path helper) are re-homed.
...
| Architecture | `ARCHITECTURE.md` | Slices 1–2: no change. Slice 3: added the `server/modules/parameter-drafts/` bullet to the backend boundaries list. Slice 4: added the `server/modules/parameter-kernel/` bullet |
...
| ADRs / domain index | `docs/adr/README.md`, `CONTEXT.md` | Slice 3: ADR-0028 added to both indexes; `Parameter draft` glossary entry added to `CONTEXT.md`. Slice 4: ADR-0029 added to both indexes |
