...
- The first default shared-8787, no-bypass run remains a separate local diagnosis: preflight passed after a deterministic API restart, but the browser matrix then failed with 69 passed / 11 failed / 4 HDC or local-device hardware-conditional skips because of shared runtime/auth/gateway boundaries. Its local output is intentionally not published as generated audit evidence; this summary must not be read as a persisted failure artifact or as a second acceptance result, and it is not overwritten by the isolated evidence below.
...
- That isolated run reported 84 Playwright tests: 80 passed / 4 HDC or local-device hardware-conditional skipped / 0 failed. Workflows A–E and G–I passed; F was skipped. Requirement coverage was 59/59; operation coverage was 56/56 with 71 records, zero invalid records, and zero validation errors. The outer runtime used API `18787`; eight topology operations used the disposable post-cutover API `50265`, while all records remained in the same immutable run/source namespace. Run ID: `full-20260719T081615349Z-f3b66dce16d2`. `npm run acceptance:evidence` exited 0. The published `latest-full` SHA-256 is `9428820846ec215aca9e93851df8e873acbad09e23f21c4db705a51a1e8fa57f`.
