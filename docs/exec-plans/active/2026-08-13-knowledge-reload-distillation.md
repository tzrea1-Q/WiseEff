...
2. **Migration** (number-checked against `origin/main`; `0109` at plan time, renumbered to `0110` before the final commit after main claimed `0109` for the module-parent repair): one additive migration adding `knowledge_entries.source_reload_run_id` (nullable, `references dts_reload_runs(id) on delete set null`) plus a partial index, alongside Phase 3's `source_log_id`.
