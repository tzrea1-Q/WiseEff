...
- [ ] Follow-up (tracked as TD-071–TD-077, not this plan): seedM1 borderline timing; simple-fake repository suites still holding ~99 SQL-text assertions (TD-072 remainder); frontend render harness (candidate 3); TD-075 phase 2 (registry unification + results-based coverage judgment); acceptance fixture seam (candidate 6); CSS text assertions (candidate 8).
