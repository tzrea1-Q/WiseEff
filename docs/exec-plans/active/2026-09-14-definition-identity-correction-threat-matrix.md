...
| VL-06 | pending review | an open `parameter_review_items` row (`status = 'open'`) exists for the organization and matcher revision | execute | The project is `blocked`; the review item keeps `status = 'open'` and its `etag_version`; no resolution row is written | `server/modules/parameter-catalog-migration/execute.integration.test.ts` (**NON-EXEC**) | yes |
...
| SR-01 | missing source provenance | a selected binding has no `source_ref`/`config_revision_id` occurrence | execute | The project is `blocked` with `missing-source-provenance`; no source rewrite is attempted; the value rows are unchanged | `server/modules/parameter-catalog-migration/execute.integration.test.ts` (**NON-EXEC**) | yes |
| SR-02 | unsupported source format | the binding's source file is not `.dts` | execute | The project is `blocked` with `unsupported-source-format`; the source bytes are unchanged; no other format is silently declared supported (default 6; gate at `server/modules/parameter-specs/propertyKeyCutover.ts:899-903`) | `server/modules/parameter-catalog-migration/execute.integration.test.ts` (**NON-EXEC**) | yes |
| SR-03 | ambiguous match | two source occurrences match the same key in one file | execute | The project is `blocked` with `ambiguous-source-match`; no occurrence is rewritten; `source_ref` values remain as recorded | `server/modules/parameter-catalog-migration/execute.integration.test.ts` (**NON-EXEC**) | yes |
...
| SR-05 | source object missing | the file version referenced by `config_revision_id` is absent | execute | The project is `blocked`; no value or binding row is written; blocked evidence records the missing file reference | `server/modules/parameter-catalog-migration/execute.integration.test.ts` (**NON-EXEC**) | yes |
