...
| Upgrade entry and restart | Existing controller, run-bound initialization, readiness and queue/proxy restoration | Native amd64 terminal apply, original login and node, first real publication, normal API/worker restart and same-SHA preservation passed in run 34310318652. Wrong-target/interruption execution remains pending. Local ARM guard is retained. |
| Preservation and recovery | Every protected original record/relationship, objects and necessary tasks; actual restore | Same-run terminal upgrade and whole-state restore preserve all original fields of 183 records across 126 tables, original object bytes/metadata and actual queue payload/state. New HTTP upload and worker complete. Earlier object-outage retry and individual store probes remain separate component evidence. |
...
The terminal probe is `scripts/run-minimal-upgrade-acceptance.ts`, launched by the `minimal-upgrade` workflow-dispatch mode. It creates its own stock Compose deployment and invokes `upgrade.sh`; the local native ARM preflight rejects before deployment creation. The driver now includes original-field/object/task oracles, original member permission isolation, a real repository publication, normal restart, same-SHA execution and combined rollback. Those stages remain unexecuted until a successful run reaches them; `complete:false` also preserves the outstanding page edit/import and interruption acceptance. Do not treat this intermediate driver as a tested production manual.
