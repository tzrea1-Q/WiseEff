...
Status: **Phase A, Phase B, Phase C, G0.1, and the first controlled implementation wave are complete; G0.2 is adopted; the owner-authorized G0.3 efficiency amendment is in progress**. The accepted module seams, implementation/ticket granularity, and dependency edges remain frozen. G0 and G0.1 merged through PRs #682 and #738; the corrected S0-ID, S0-FIX, and S1-BND nodes are closed through PRs #739, #743, and #741. G0.2 and G0.3 change execution sequencing and budgets only; they do not add a launch node or change any CD, CF, ID, or RE edge.
...
- S0-ID is the sole owner of `parseCanonicalCompatibleSelector`, `parseCanonicalNodeName`, and `parseCanonicalPropertyKey` in `normalization.ts`, plus golden tests. Each constructor accepts `unknown`; a non-string returns `not-string`. Successful construction returns the input byte-for-byte. It performs no trim, lower/upper case conversion, NFC/NFKC or any other Unicode normalization, quote removal, or `@`/unit-address removal.
- The closed failure-reason union is exactly `not-string | empty | control-character | non-ascii | surrounding-whitespace | whitespace-forbidden | quoted-source-token | wildcard-forbidden | unit-address-present | length-out-of-range | invalid-syntax | structural-property`.
- A compatible selector must match `^[A-Za-z0-9][A-Za-z0-9+._/-]*(?:,[A-Za-z0-9][A-Za-z0-9+._/-]*)?$`. `*` returns `wildcard-forbidden`; every other grammar failure returns `invalid-syntax`.
- A node name accepts the exact root token `/`; every other value must match `^[A-Za-z][A-Za-z0-9,._+-]{0,30}$`. Any `@` returns `unit-address-present`; a string longer than 31 bytes returns `length-out-of-range`; every other grammar failure returns `invalid-syntax`.
- A property key must match `^[A-Za-z0-9,._+?#-]{1,31}$`. A string longer than 31 bytes returns `length-out-of-range`. The ASCII-case-insensitive structural deny-list is `compatible`, `device_type`, `gpio-controller`, `interrupt-controller`, `linux,phandle`, `phandle`, `ranges`, `reg`, `status`, `#address-cells`, `#gpio-cells`, `#interrupt-cells`, and `#size-cells`; every value beginning with `#` is also denied. Denial returns `structural-property`, while other grammar failures return `invalid-syntax`. Case-insensitive denial never changes the accepted/original bytes.
- Failure classification is deterministic in this exact order: `not-string`; `empty`; `control-character`; `non-ascii`; `surrounding-whitespace`; `whitespace-forbidden`; `quoted-source-token`; then kind-specific checks. Compatible checks `wildcard-forbidden` before `invalid-syntax`. Node checks `unit-address-present`, then `length-out-of-range`, then `invalid-syntax`. Property checks `length-out-of-range`, then `structural-property`, then `invalid-syntax`.
...
Owner/source derivation is also closed. Platform means exactly `(owner_scope_kind=platform, owner_scope_id=platform)`. Organization and Project use the source row's exact organization/project ID. A child without an owner inherits its parent; a missing or conflicting parent is R0. Composite `source_id` values use `serializeContract` over an ordered field list; delimiter concatenation and hash-only identity are forbidden. An `unresolved-protected-reference` source includes the consumer table, consumer primary key, column, and raw referenced ID. The registry maps each source-kind literal to its physical table, primary key, and owner extractor; callers cannot submit a table name or choose an extractor.

S2-SCH must support these exact internal storage/contract target literals at minimum: `parameter-observation`, `observation-match`, `review-item`, `review-resolution`, `publication-intent`, `policy`, `audit-event`, and `migration-history`, in addition to the core Catalog/Registration/Binding/Value targets. Display names such as Observation, ReviewResolution, Audit, or MigrationHistory never replace those literals. A public lookup response never exposes this internal source-kind registry or Archive candidates.
...
The checker also inventories all legacy relations, legacy identifier spellings, and retired route fragments, and applies default-deny to private module imports with an explicit public-seam allow-list. Any unresolved AST/dataflow receiver/target or unresolved dynamic SQL relation is itself a violation and fails closed; it is never skipped as unknown. Each violation occurrence is replacement-resistant: its record binds the mandatory `--trusted-base-sha` commit, trusted baseline blob OID, byte span, token/evidence, file, owner family, and rule. The checker compares the complete occurrence multiset, so deleting one identical occurrence and adding another cannot preserve the baseline set. Initialization requires a clean index/worktree and an exact caller-authorized HEAD, writes the fixture plus all eleven shards only into a private staging directory, validates the complete path set/schema/counts/checksums there, then atomically publishes every output or none. CI/Hosted must fetch and verify the named trusted base; `.github/workflows/ci.yml` and `package.json` are allowed production paths for S0-RAT solely for that wiring. Local mutable `origin/main` is not trusted evidence, and Hosted execution is required before S0-RAT completes.

The reviewed baseline contains exactly 3,350 violations. Its 548 duplicate base-ID groups contain 1,888 occurrences. An ordinal suffix distinguishes only the current ordering: deleting one occurrence and adding an equivalent occurrence elsewhere can reuse ordinals and evade a delta. Therefore the trusted blob/span/evidence occurrence record and complete replacement-resistant multiset comparison above are mandatory, not an optional hardening.
...
The PostgreSQL-aware lexer rejects transaction/session escape and explicitly denies `COMMIT WORK`, `END`, `PREPARE TRANSACTION`, `SAVEPOINT`, `RELEASE SAVEPOINT`, `COPY ... FROM STDIN`, `\i`, `\ir`, `\gexec`, `\gset`, `\copy`, `\connect`, `\!`, and `\q`, including alternate spellings, comments used to conceal controls, autocommit changes, role/search-path/session mutation, and dynamically executed control SQL. `synthetic-fixture-verify.sql` runs exactly three times: before candidate mutation; after candidate application plus validation and immediately before rollback; and after rollback. Cleanup is fail closed: every created database/object/process must be removed, cleanup failure fails the run, and success emits exactly `CLEANUP_OK`. A closed-world secret scan covers every source and generated artifact, including schema/output/log files. Only the exact registered path set may contain regular files; every symlink, unknown path, directory, device, socket, FIFO, or other non-regular entry fails closed before execution or hashing. Required gates include focused safety tests, `npm run test:scripts`, and real PostgreSQL execution; synthetic rehearsal remains synthetic and never becomes target, release, or production evidence.
...
## G0.3 efficiency amendment (2026-09-03)

The G0.2 protocol kept evidence honest but scheduled development too serially. The first post-G0.2 hours spent more wall-clock on Hosted-as-debugger, a second fingerprint of the same trusted-base inventory, a shared `5432/wiseeff` database, and idle CI waits than on remaining launch nodes. G0.3 amends execution only. All 55 accepted nodes, 53 launch Issues, seams, ownership, requirements, and `CD=90`, `CF=54`, `ID=27`, `RE=18` edges remain unchanged. Local and Hosted output still cannot be labeled T/R.

G0.3 supersedes the G0.2 run-profile rows listed below. Unlisted G0.2 rows remain in force. Catalog launch Issues follow [Catalog Launch Operating Rules](../../agents/catalog-launch-operating-rules.md) without editing frozen Issue bodies.

| Control | G0.3 setting |
| --- | --- |
| Capacity | Path-disjoint Scratch WIP 4. Lanes that share migrations, generated schema, OpenAPI, or a fingerprint stay at WIP 2. When review begins, finished implementation agents still release their slots so Standards and Spec run in parallel |
| Merge versus develop | One feature Hosted PR. Later lanes continue through `SEALED` while the merge lane is `HOSTED`, then refresh immediately before their own Hosted |
| Independent merge wave | Process-only PRs (protocol, operating rules, bilingual docs, `scripts/catalog-lane-env.ts`) may merge beside the current feature Hosted PR |
| Local acceptance | Dedicated lane DB via `npm run catalog:lane:env -- provision --issue <n>`. Issue-named command via `npm run catalog:lane:accept`. Role canary as `catalog_migration_owner` for RBAC/migration nodes. Zero collected tests is a hard failure. Default compose `postgres://wiseeff:wiseeff@127.0.0.1:5432/wiseeff` is forbidden catalog evidence |
| Inventory lock | Complete trusted-base scan at `THREAT-READY`; one fingerprint. Same-invariant second patch is a circuit breaker. If only one option can pass Hosted, the parent records the choice and continues |
| Hosted overlap | Parent dispatches or continues at least one other disjoint lane during Hosted, or records that none exists |
| Unrelated flake | One `gh run rerun --failed`. Do not patch unrelated tests |
| Models | G0.2 model slugs remain preferred. If a spawn allowlist cannot provide them, the parent records the substitute and continues; it does not idle |
...
| --- | --- | --- | --- | --- |
| Repository maps | Update | `AGENTS.md`; `ARCHITECTURE.md`; `docs/agents/agent-delivery-protocol.md`; `docs/agents/catalog-launch-operating-rules.md`; `docs/agents/fleet-coordination.md`; `scripts/bilingual-docs.ts` | `docs/zh-CN/root/AGENTS.md`; `docs/zh-CN/root/ARCHITECTURE.md`; `docs/zh-CN/agents/agent-delivery-protocol.md`; `docs/zh-CN/agents/catalog-launch-operating-rules.md` | G0.2/G0.3 own delivery lifecycle/budgets; first module merge records four modules/dependencies/readiness |
...
- G0.3 is developed on `codex/wayfinder-g03-delivery-efficiency` from accepted `origin/main@bd9648d2e6706bd81230bfd5be73229ba44e4212`; it is documentation/process plus the catalog-lane helper, and makes no launch-node product or T/R evidence claim.
- Every future ticket agent starts from then-current `origin/main` in an isolated worktree with branch template `codex/pcat-<issue-number>-<slug>`, reads this Spec plus the owning decisions/module docs, and receives the G0.3 task packet, [Catalog Launch Operating Rules](../../agents/catalog-launch-operating-rules.md), and stop boundary.
...
- The parent/session owner follows the G0.3 run profile and repository delivery protocol, integrates in deterministic merge order, and exclusively owns final PR creation, Hosted triage, merge, attestation, and main synchronization.
