...
1. **Implement B1** (decided by ADR-0046): schedule the ConfigurationSchema source-identity implementation. B6 fail-closed completion is done; registration permission remains the contract-defined `trusted-system` + `automatic` + `use-default` path.
