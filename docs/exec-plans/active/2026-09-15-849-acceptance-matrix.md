...
| 4 | Rebuild by seed: preserve non-parameter data; archive legacy parameter data, drafts, history and source versions offline; initialize only Atlas, Aurora and Nebula; archived notice on old links; retire acme but keep release history | **Partial** | **Offline archive delivered (capture)**: `0149_project_parameter_plane_archives.sql` + `seedInitialization/archive.ts` capture the legacy parameter plane (14 declared relations, child rows reached through their parents) to the object store with per-relation counts and digests, and `materializeSeedSources` archives each project *before* rebuilding it, with a guard that refuses a missing or truncated archive. `archive.integration.test.ts` (4 tests) covers counts, child-scope capture, cross-project isolation, idempotent reuse and both guard refusals. **Disposal is deliberately not implemented** - the ledger has no column that could imply deletion happened, and removing the archived rows needs its own reviewed decision. **Archived old-link notice delivered** in the parameter workbench (banner with parameter id, diagnostic and migration evidence, plus a dismiss control; drafting and submission are not offered for the archived record). **Acme retirement delivered**: `completeSuccessor.test.ts` (2 new tests: retired subject+alias carried with tombstones; mint under the retired subject refused), real-PostgreSQL lifecycle assertion in `vendorSuccessor.integration.test.ts`, digest pin moved to `sha256:5f0e7bcd…` in both `ops/self-hosted/upgrade.md` runbooks, historical R-F4 value preserved with a forward note. Plus the seed initialization target plan, blocking guard and run journal: `seedInitialization/plan.test.ts` (6); DTS seed-source materialization for the three projects: `seedInitialization/materialize.test.ts` (3); archived-diagnostic normalization: `parameterClient.test.ts` (3) | JSON seed materialization (B1), the acme *definition* lifecycle, archived-plane **disposal**, the remaining canonical bindings (B2/B6), archived-notice UI |
| 5 | S1/S2 acceptance: real authentication, PostgreSQL, source store, publication manager, archive rebuild, interruption/resume, whole-state recovery; frontend real-browser verification at three sizes | **Partial** | Real PostgreSQL across every integration suite; real authentication established under `AUTH_MODE=production AUTH_PROVIDER=local`; real browser evidence for the import wizard and the archived old-link notice at 1440×900 / 768×1024 / 390×844; the S2 operator path (interruption, resume, whole-state restore) executed against a real database | The Docker-based archive-rehearsal artifact path and quiescence; the publication manager exercised only through its own suites; the three-viewport matrix for draft/review/export |
...
B2 is therefore delivered for the example slice: registration is no longer a manual prerequisite (B6), and the whole 113-input slice now binds. What is not done is the rest of the objective's target: the counts the plan expected (124 per project) were an assumption about unconverted inputs and have never been measured against a complete seed.
...
**B6 - MOSTLY RESOLVED: seed-time automatic registration is implemented and works.** An earlier revision of this
...
**The modules were already there.** DTS ingest provisions a module per logical node, and those are exactly the
`node-type`/`driver-group` modules the placement guard requires, so no seed-time module provisioning is needed for
subjects the source declares. `nodeTypeSubjectBinding.integration.test.ts` previously had to hand-build
attribution subjects, modules, registrations and placements; that entire fixture is now deleted and the test still
passes.
...
| 9 Initialization and preservation matrix | **Partial** | The target plan covers the three allowed projects, custom projects, missing/ambiguous ownership, blocking and repeated-run idempotency. Materialization, the initialization lock and non-parameter preservation snapshots are not done |
...
- `catalogRoles.integration.test.ts > application, agent, and verifier logins are not members of Catalog writer roles` fails on a pristine `origin/main` checkout in this environment too. It is a pre-existing baseline failure, not attributable to this branch.
...
render no banner. Screenshots: `work/ui-checks/849/desktop-archived-notice.png`,
...
| Quality/testing | Updated | Status per testing decision 1–13 recorded above; no gate definition changed |
...
This matrix and its Chinese pair, the two round reports and their Chinese pairs, both `ops/self-hosted/upgrade.md` runbooks, and the baseline-verification reference pairs are all updated in this change. `npm run docs:check` and `git diff --check` pass. Every status above is backed by a command in section 4.1 run on the tree at this revision; nothing here claims target execution, deployment, Hosted CI or Issue completion.
