...
- TDD Red: `npm run test:server -- --run server/modules/parameter-kernel/sensitiveNode.test.ts` failed 1 test (11 passed) because `assertTrustedSensitiveNodeWriteAllowed` did not exist. Green introduced the shared guard, then the exact affected PostgreSQL command passed 11 files / 162 tests.
