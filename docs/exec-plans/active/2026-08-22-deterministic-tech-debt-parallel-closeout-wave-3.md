...
7. Prove deterministic/offline startup, health, Xiaoze perception, and Xiaoze action behavior through the two existing acceptance specs. Run with `XIAOZE_DETERMINISTIC=true`; do not provide or contact a live provider, and do not convert missing target evidence into a local success claim.
...
| --- | --- | --- |
| Repository maps | Review | `AGENTS.md`; `docs/zh-CN/root/AGENTS.md`; `ARCHITECTURE.md`; `docs/zh-CN/root/ARCHITECTURE.md`; `docs/README.md`. Record unchanged unless navigation or runtime boundaries actually move. |
| Architecture entry points | Update | `README.md`; `docs/zh-CN/README.md`; `docs/design-docs/full-stack-architecture.md`; `docs/zh-CN/design-docs/full-stack-architecture.md`. Align the Xiaoze canonical runtime/configuration entry points and retain the local-deterministic versus live-provider evidence boundary. |
