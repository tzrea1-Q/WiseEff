...
- Rewriting completed historical plans, `docs/design-docs/2026-06-26-xiaoze-sole-agent-cleanup-design.md`, its Chinese historical companion, or `server/modules/agent/xiaoze/SPIKE.md` merely to modernize old variable names. Current normative docs change; history remains history.
...
| TD-031 | One pure `resolveXiaozeLlmConfig(env)` module exporting canonical/legacy key metadata and a redacted resolution result | Operator configuration is externally supplied; parsing/health/setup are locally substitutable; a live model is external and excluded | Runtime, health, docs checks, and self-hosted tools consume the resolver/constants instead of re-implementing precedence. Canonical output, legacy input only. |
...
| --- | --- | --- | --- | --- |
| Worker 1 | TD-072 | `test/td-072-parameters-service-pg` | `WiseEff-worktrees/wave3-td072` | Review/support only after merge |
| Worker 2 | TD-110 | `refactor/td-110-api-users-empty-boot` | `WiseEff-worktrees/wave3-td110` | Reuse this slot for TD-112 after TD-110 is reviewed, merged, and `main` is refreshed |
| Worker 3 | TD-031 | `chore/td-031-xiaoze-llm-env` | `WiseEff-worktrees/wave3-td031` | Review/support only after merge |
| Worker 2, second pass | TD-112 | `refactor/td-112-project-admin-datatable` | `WiseEff-worktrees/wave3-td112` | Starts from refreshed `main`, not from the TD-110 branch |
| Parent/shared | closeout | `docs/deterministic-td-wave3-closeout` | `WiseEff-worktrees/wave3-closeout` | Tracker, plan archive, stale-plan hygiene, final gates |
...
| --- | --- | --- | --- |
| TD-072 | `server/modules/parameters/service.test.ts`; `server/modules/parameters/service.ts`; a narrowly named pure review-policy module/test under `server/modules/parameters/` only if the seam is required | None with other wave tracks | No frontend, env, tracker, or broad database refactor. |
| TD-110 | `src/application/state/apiInitialState.ts`; `src/infrastructure/mock/prototypeState.ts`; `src/mockData.ts`; `src/application/state/appState.ts`; `src/mockData.apiInitialState.test.ts`; focused reducer/App/user-directory tests | `docs/FRONTEND.md`, `docs/zh-CN/frontend.md` if architecture text changes | Merge TD-110 before creating TD-112. Preserve mock exports and unrelated state slices. |
| TD-031 | canonical env resolver/constants; `server/config/env.ts`; Xiaoze endpoint/health config consumers and tests; `.env.example`; `ops/self-hosted/.env*.example`; scoped self-hosted setup/profile/check scripts and tests; exact EN/ZH env/runbook/security/reliability docs | `docs/FRONTEND.md`, `docs/zh-CN/frontend.md`; active self-hosted files may move on `main` | Rebase immediately before review and again before merge. Touch only Xiaoze LLM keys in shared operator files; preserve upgrade/setup behavior. |
| TD-112 | `src/components/admin/ProjectAdminTable.tsx`; new focused behavior tests; `src/components/admin/DataTable.tsx` and tests; `src/components/HorizontalDragScroll.tsx` and tests; scoped ProjectAdmin CSS; route-level tests; new focused acceptance spec/registry/map entries | `docs/FRONTEND.md`, `docs/zh-CN/frontend.md`, acceptance registries/generated maps | Start after TD-110 merge. Rebase after TD-031 if it changed FRONTEND. No edits under `project-configuration-workbench/`. |
| Shared closeout | EN/ZH tracker; this plan; EN/ZH PLANS indexes; launch closeout plan current-state note; Xiaoze persistence metadata and stale plan archive | All TD numbers and plan locations | Start only after every implementation PR intended for closure is on `main`; re-check TD/ADR/migration numbers. |
...
5. **Red/green: impossible project-less merge.** Pin the semantic review subject/precondition as a pure policy table used by `reviewChange`, or prove through review that the defensive branch is unreachable and delete the redundant test/branch. Do not insert an invalid database row.
...
2. **Green: deep resolver.** Add `server/config/xiaozeLlmConfig.ts` with one dependency-light `resolveXiaozeLlmConfig(env)` and canonical key constants; adopt it from model construction, health/readiness, routes, config validation, docs governance, and self-hosted readers instead of duplicating precedence. Production direct reads of the legacy keys must reach zero outside this resolver.
3. **Red: generated output.** Assert root/self-hosted env templates and setup/profile writers emit only the three canonical names, while an existing legacy `.env` is read and rewritten/migrated without losing non-Xiaoze settings.
4. **Green: migration path.** Keep legacy aliases accepted for one compatibility window. Any present canonical raw key makes the canonical group authoritative, including explicit blank/unset; only total canonical absence allows legacy fallback. Health/error messages use canonical semantic names and never include secret values.
5. Update current normative EN/ZH environment, security, reliability, provider, setup, acceptance, and deployment docs. In the two active target-evidence plans, update future commands/examples and add a supersession note; do not rewrite completed historical plans, the sole-agent historical design, or the Xiaoze spike.
6. Prove deterministic/offline startup and health behavior. Do not call a live LLM and do not convert missing target evidence into a local success claim.

### Track D — TD-112, after its scope gate

1. **Red: public ProjectAdmin behavior.** Cover status filtering/clear, controlled header sort with `aria-sort`, >10-row pagination, empty states, Enter/click row entry, and edit/delete action isolation (actions must not trigger row entry).
...
5. Add dedicated `PARAM-ADMIN-003` automation and update its requirement/operation wording from the stale ≤960px claim. Run `PROJ-CONFIG-READ-001` to prove the row still enters the canonical workbench.

## Verification gates by track

| Track | Focused/component gate | Full/static gate | Browser gate | Acceptance/evidence gate |
| --- | --- | --- | --- | --- |
| TD-072 | `npx vitest run --config vitest.server.config.ts server/modules/parameters/service.test.ts server/modules/parameters/serviceReviewWorkflow.integration.test.ts server/modules/parameter-topology/postCutoverWorkflow.integration.test.ts` | `npm run test:server`; `npx tsc -b`; `npm run build`; `npm run docs:check`; `git diff --check` | Not applicable: no visible UI | Not applicable: server behavior is proven through real DB integration; no target DB claim |
| TD-110 | `npx vitest run src/mockData.apiInitialState.test.ts src/reducer.userPermissions.test.ts src/UserPermissionsPage.test.tsx src/App.test.tsx` | `npm test`; `npx tsc -b`; `npm run build`; `npm run docs:check`; `git diff --check` | API mode `/organization/members`, reload at 1440×900, 768×1024, 390×844; snapshot+screenshot each; auth probe has no demo-name flash; `/api/v1/me` and user directory are 200; zero console errors | `PERM-USER-MGMT-001` in `e2e/acceptance/permissions.acceptance.spec.ts`; preserve non-Admin denial, API, DB, and audit evidence |
| TD-031 | Focused server config/health/Xiaoze tests plus `ops/self-hosted/scripts/{check-self-hosted-config,selfhost-profile,selfhost-answers,ip-lab-profile,setup-selfhost}.test.ts` | `npm run test:server`; `npm run test:scripts`; `npm run selfhost:check`; `npm run contract:check`; `npm run docs:check`; `npx tsc -b`; `npm run build`; `git diff --check` | Not applicable: no user-visible UI or route behavior changes | Offline/deterministic health and self-hosted config gates only; live provider, target readiness, and secret provisioning are explicitly excluded |
| TD-112 | DataTable, HorizontalDragScroll, ProjectAdminTable behavior/layout, and `ParameterAdminNextPage` focused tests | `npm test`; `npm run lint`; `npm run ui:check`; `npx tsc -b`; `npm run build`; `npm run docs:check`; `git diff --check` | API mode `/parameter-admin/projects` at 1440×900, 768×1024, 390×844; snapshot+screenshot each; search/filter/sort/pagination/Enter row/edit/delete; 390 card completeness, 768 rail, 1440 no page overflow; zero console errors and relevant API requests 200 | Automate `PARAM-ADMIN-003` in a dedicated acceptance spec and run `PROJ-CONFIG-READ-001`; regenerate/verify coverage and operation evidence |
...
## Review, PR, CI, and merge workflow

1. Merge this planning-only change first. No implementation branch may predate the merged plan baseline.
2. Start TD-072, TD-110, and TD-031 in parallel from refreshed `main`.
3. For each implementation branch:
   - worker records red evidence before production edits and focused/full evidence after green;
   ...
   - parent alone pushes/opens the PR, waits for every required CI check, merges, then synchronizes local `main`.
...
4. Replace the misleading active plan `docs/exec-plans/active/td-031-xiaoze-run-timeline-streaming.md` with an archived, correctly titled completed plan at `docs/exec-plans/completed/2026-08-22-xiaoze-run-timeline-streaming-metadata-closeout.md`, and add its Chinese companion under `docs/zh-CN/exec-plans/completed/`.
5. Update current-state references in both PLANS indexes and the launch actionable closeout plan; preserve historical evidence rather than rewriting old implementation records.
6. Move this Wave 3 plan from `active/` to `completed/` in both languages only after every Update/Review documentation row is resolved and the combined gates pass. The filename must not remain in both trees.
...
- Repository `QueuedResult` stock is zero, all three former residual behaviors/policies remain covered, and no SQL-text/call-order assertion replaces behavior evidence.
...
- All current templates and setup output use `XIAOZE_LLM_*`; legacy names are isolated to one documented resolver fallback; config/health errors stay redacted; no live-provider readiness is claimed.
- `ProjectAdminTable` composes `DataTable`, no longer owns a handwritten `<table>` or scroll math, and preserves 390/768/1440 behavior plus entry/actions/filters/sort/pagination.
...
Also re-run the TD-110 and TD-112 API-mode browser gates from their final merged routes. Record exact test counts, skips, warnings, screenshots, network/console results, CI check names, PR numbers, and merge SHAs in the completed plan and trackers.

## Documentation Impact Matrix

| Area | Status | Exact files / required evidence |
| --- | --- | --- |
| Repository maps | Review | `AGENTS.md`; `docs/zh-CN/root/AGENTS.md`; `ARCHITECTURE.md`; `docs/zh-CN/root/ARCHITECTURE.md`; `docs/README.md`; `docs/zh-CN/README.md`. Record unchanged unless navigation or runtime boundaries actually move. |
| Planning | Update | This EN/ZH plan; `docs/PLANS.md`; `docs/zh-CN/PLANS.md`; `docs/exec-plans/active/2026-08-17-launch-actionable-tech-debt-closeout.md`; `docs/zh-CN/exec-plans/active/2026-08-17-launch-actionable-tech-debt-closeout.md`; stale timeline-plan source/destination named in Shared closeout. |
...
| Domain context | Review | `CONTEXT.md`; `docs/adr/`. Preserve the everyday Parameter workbench vs Parameter admin/configuration-workbench distinction; add an ADR only if implementation changes a durable decision. |
...
| Browser acceptance | Update | `e2e/acceptance/requirements.ts`; `e2e/acceptance/operationMatrix.ts`; new TD-112 acceptance spec; `docs/developer/browser-acceptance-coverage-map.md`; `docs/zh-CN/developer/browser-acceptance-coverage-map.md`; generated `docs/developer/user-operation-coverage-matrix.md`; generated `docs/zh-CN/developer/user-operation-coverage-matrix.md`. Make `PARAM-ADMIN-003` automated with accurate breakpoints and keep `PROJ-CONFIG-READ-001`. |
| Environment/runtime | Update | `.env.example`; `ops/self-hosted/.env.example`; `ops/self-hosted/.env.ip-lab.example`; `docs/developer/environment-variables.md`; `docs/zh-CN/developer/environment-variables.md`; `docs/developer/local-development.md`; `docs/zh-CN/developer/local-development.md`; `docs/zh-CN/backend-runtime.md`. Canonical output is `XIAOZE_LLM_*`; legacy input is migration-only. |
...
| Security/governance | Update | `docs/SECURITY.md`; `docs/zh-CN/SECURITY.md`; `docs/security/secrets-management.md`; `docs/zh-CN/security/secrets-management.md`; `scripts/check-doc-governance.ts` and tests. Retain secret-redaction and no-committed-key rules under canonical names. |
...
- all canonical/legacy Xiaoze env mentions are inventoried with `rg`; current templates and operator instructions use canonical keys, while legacy mentions remain only in migration/history contexts;
- `PARAM-ADMIN-003` requirement, operation registry, generated maps, automated spec, and recorded three-viewport evidence agree on the 390/768/1440 contract;
...
- `npm run docs:check`, `npm run acceptance:coverage`, `npm run contract:check`, and `git diff --check` pass before moving the plan to completed;
