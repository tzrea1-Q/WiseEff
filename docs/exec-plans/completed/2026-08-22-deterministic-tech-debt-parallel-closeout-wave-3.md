...
For TD-031 focused acceptance, reserve frontend `5193` and API `8931`; both were free during implementation. Port `5193` stays inside the repository's existing localhost CORS allowlist (`5173–5199`); do not expand that allowlist for this gate. Immediately before the run, confirm both ports are still free with read-only probes:
...
- Two repository-wide local commands exposed out-of-scope baseline debt and are not presented as green. On Darwin, `acceptance:visual` used the task-specific isolated database, passed 9/20, and failed 4 stale plus 7 missing snapshots; the TD-112 relevant `/parameter-admin`, configuration-workbench, and focused visual cases passed, while repository CI Acceptance quality passed 97/97. `acceptance:browser -- --mode local-non-hdc` ran from the TD-112 implementation worktree while it contained test-generated changes and used the existing shared acceptance database; it passed 109, skipped 29 planned cases, and failed 18 broad-suite cases (including toolchain, permissions, knowledge, and Xiaoze baseline state). That failed full run was not publishable, so the unscoped latest-run `acceptance:evidence` check correctly had no valid full manifest; it is diagnostic inventory, not fresh-database reproduction evidence. All 29 failures are assigned to open TD-122 and `docs/exec-plans/active/2026-08-22-acceptance-baseline-integrity.md`; its first gate is a clean merged-main plus fresh-database rerun. No snapshots, permissions, hardware/toolchain, or unrelated workflows were rewritten in this documentation closeout. Required per-track browser/evidence gates and all four implementation PR CI runs are green.
...
| Active target-evidence plans | Update | `docs/exec-plans/active/2026-05-29-wiseeff-m5-2-staging-pilot-evidence-execution.md`; `docs/exec-plans/active/2026-05-29-wiseeff-m5-2-non-hdc-target-evidence-closure.md`. These legacy evidence-execution plans have no direct Chinese companions; update their future-facing examples and supersession notes in English without fabricating target evidence. Current operator instructions remain bilingual through the environment, runbook, reliability, and security rows above. |
...
- `npm run docs:check`, `npm run acceptance:coverage`, `npm run acceptance:operations`, `npm run acceptance:quality`, `npm run acceptance:a11y`, `npm run acceptance:responsive`, `npm run contract:check`, and `git diff --check` pass before moving the plan to completed; TD-110/112 focused browser operations, focused visual cases, and their required evidence records must also pass;
- repository-wide `npm run acceptance:visual`, `npm run acceptance:browser -- --mode local-non-hdc`, and unscoped latest-run `npm run acceptance:evidence` are diagnostic for this four-row wave rather than substitutes for its scoped gates. Any non-green result must be recorded with exact counts, must not be presented as passed, and must be assigned to an Open tracker row plus active plan before Wave 3 can be archived. The 11 visual and 18 browser failures found here are assigned to TD-122; closing TD-122 still requires those repository-wide commands to pass with a valid full evidence manifest;
