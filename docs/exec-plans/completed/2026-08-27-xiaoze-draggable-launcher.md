...
- Follow-up release-path defect: some browser input paths end an otherwise valid launcher drag with `pointercancel` or `lostpointercapture`. The launcher previously treated that as cancellation and rendered the gesture start position. Effective movement now commits the last clamped position on those events, while no-movement cancellation and component-unmount cleanup retain rollback semantics. Component and CDP touch-cancel coverage lock the distinction down.
