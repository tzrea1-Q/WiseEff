...
| TD-019 | Release Operations | M5 now has a pilot readiness route and smoke command. On 2026-05-29 the local `.env` proved database/object-store/worker/live-Agent health, standard local PostgreSQL-backed API-mode E2E, and a local backup/restore drill; strict M5 smoke is still blocked by simulator `deviceGateway`. | The code can report blocked gates correctly, but commercial pilot readiness should not be claimed from local checks alone, and local smoke skips must be explicit. | Capture target-environment staging evidence, HDC device-lab smoke, deployment rollback rehearsal, and cloud/staging object-store evidence; keep `docs/generated/m5-pilot-acceptance.md` current. |
| TD-020 | Production Auth | Production-auth API mode correctly rejects unauthenticated business requests, but frontend HTTP clients do not yet inject a bearer token, and the local smoke token subject in `.env.example` is not a seeded database user for FK-backed writes. | UI E2E passes in development-auth API mode, but cannot close a production-auth full-chain workflow; signed smoke tokens can pass `/me` while later writes fail if the subject is not present in the database. | Add a frontend auth-token provider for API clients, generate local/staging smoke tokens for seeded or provisioned users, and add production-auth API-mode E2E coverage. |
