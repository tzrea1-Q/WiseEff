...
| TD-019 | Release Operations | M5 now has a pilot readiness route and smoke command, and M5.2 local repository plus local PostgreSQL-backed API-mode gates pass, but real staging evidence is still missing for live API, staging PostgreSQL-backed E2E, HDC device-lab, backup/restore, rollback, and live Agent provider signoff. | The code can report blocked gates correctly, but commercial pilot readiness should not be claimed from local checks alone, and local smoke skips must be explicit. | Provide staging secrets/URLs and device-lab inputs, run the M5.2 external evidence plan, record results in `docs/generated/m5-pilot-acceptance.md`, and keep the rollback drill current. |
