...
On `/parameters`, API mode keeps the mature `ParametersPage`/`WorkbenchLayout` hierarchy and renders `DtsParameterWorkbench` inside `ApiProjectTopologyWorkspace`. The coordinator remains responsible for API loading while semantic rows, a collapsible DTS navigator, current-edits tray, typed detail dialog, and binding submission panel are integrated into the familiar workbench. This is not a topology-only replacement: the navigator scopes the list and preserves the existing search, focus, detail, draft, and review handoff mental model. The `amba → i2c@FDF5E000 → sc8562@6E → gpio_int` path is represented by real parent relationships. Browse mode is effective-topology only; source file/node/line and source-chain provenance stay on each list row and in the detail dialog so a separate source/effective page toggle is not required. Desktop rows expose property, driver/instance, full DTS location with source provenance, raw/effective value, shape, and schema/policy/mapping governance; tablet/mobile rows stack these fields and expose provenance in the detail sheet. Search covers property, compatible/driver, address, full topology path, source file/node path, and raw value; clear-all preserves pending drafts.
