...
- `/knowledge-admin` (`KnowledgeAdminPage.tsx`): the agent-draft publish queue (Phase 3) — `list({ status: "draft", sourceType: "agent" })` rows with creator, session origin, source deep link (`/logs?logId=…` for log-sourced drafts, `/dts-reload?runId=…` for reload-sourced drafts), and created time; per-row review (`/knowledge?entryId=…`), publish, and archive-reject behind a `ConfirmDialog` (`rejectAgentDraft`) — plus archived-entry management (restore), manage-gated hard delete behind an acknowledged `ConfirmDialog`, and the retrieval index health section — honest retrieval-mode banner (pgvector/embedding availability), per-entry index status with failure reasons, per-entry retry, and rebuild-all. The entry detail dialog shows the same source links.
...
- Distil-to-knowledge from reload runs (deferred roadmap item 3): the `/dts-reload` run result section shows the distil-to-knowledge action on TERMINAL runs only (verified / unverifiable / contradicted / failed) when the user holds `knowledge:edit`; it calls `KnowledgeRepository.distillFromReloadRun(runId)` and hands off into the `/knowledge?entryId=…` draft detail. `/dts-reload?runId=…` deep links open a history run's detail, so knowledge-entry source links point back at the evidence. Mock mode builds the same pre-filled draft via `src/domain/knowledge/distillReload.ts` from the runtime's shared mock `DtsReloadRepository` instance; both prefills couple only to the stored run/snapshot DTO and state the outcome honestly.
