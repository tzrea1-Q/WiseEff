...
**Project operations are a page, not a modal (ADR-0001, POD-D1).** `ProjectOperationsView` renders the four views in the page body: a back link to the project list, one authoritative `<h2>` per view, the shared view navigation (`.project-operations-nav`, `role="navigation"` + `aria-current="page"` with left/right/Home/End roving focus), the latest governance audit hint with a timestamp and a dismiss action, and the view slot itself as a labelled `role="region"`. Every panel keeps one heading level (`<h3>`). Visited views stay mounted so filters, selected nodes, and unsubmitted structure drafts survive view switches; leaving the project with unsubmitted drafts asks for confirmation first. An unknown project id renders a not-found screen with a way back instead of titling the page with the raw id. Search hits in `DtsSearchPanel` navigate to the structure view and select that node, and say so when the node lives in another file.
