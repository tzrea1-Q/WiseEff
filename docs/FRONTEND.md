...
Notification feedback (`state.notifications`, pushed by `ADD_NOTIFICATION`) is rendered by `AppToastLayer` (`src/components/common/AppToastLayer.tsx`), mounted once in the AppShell for **both runtime modes** and visible on every route. It shows the newest queue entry inside a `role="status"` / `aria-live="polite"` region, auto-dismisses after ~4s, and has a labelled manual close button; dismissal dispatches `DISMISS_NOTIFICATION`, which removes the entry from the queue rather than only hiding it. The toast uses the `--z-toast` layer so it stays above dialogs and the Xiaoze popup. This replaces the earlier mock-mode-only toast inside `LogsPage`, which left every API-mode success/failure notice invisible. Server failures surfacing through runtime notifications go through `toUserErrorMessage` (`src/infrastructure/http/userErrorMessage.ts`), which maps `WiseEffApiError.code` (and known `details.reason` values) to Chinese copy with a short request-id suffix; unknown codes keep the server text but stay reportable, and fetch-level network failures get connectivity copy.
