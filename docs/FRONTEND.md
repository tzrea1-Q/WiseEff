...
The TopBar project picker is visible only on parameter-management routes. `pageUsesProjectScope()` in `src/appConfig.ts` returns `true` for `parameters`, `parameter-submissions`, `parameter-review`, `parameter-admin`, `parameter-admin-projects`, and `parameter-home`. It is hidden on the temporary construction routes `/parameter-admin-next` and `/parameter-admin-next/projects` (organization-scoped governance must not sit under a project selector; see ADR-0001), and on log analysis (`logs`, `log-admin`) and debugging (`node-debugging`, `debugging-admin`) routes because M2/M3 data loads from authenticated organization context without `projectId`. Parameter pages may still read `?project=` from the URL; log and debug pages ignore project query params.

### Parameter admin next (construction)

`/parameter-admin-next` hosts the redesigned admin shell while `/parameter-admin` stays on the legacy tree. Organization governance and project operations are peer destinations (`/parameter-admin-next` vs `/parameter-admin-next/projects`). Project file views are deep-linked at `/parameter-admin-next/projects/:projectId/files`. Panels consume a single `createParameterAdminApplication` facade over `ParameterTopologyRepository`, `ParameterModuleRegistryRepository`, and import actions from `parameterActions` (resolved for both mock and api modes). Admin cross-panel state lives in `ParameterAdminProvider` — not global `PrototypeState`. URL query params (`q`, `lifecycle`, `driver`, `sort`, `spec`, …) are the source of truth for filters, sort, and selection. Organization area includes bulk import, identity mapping governance, spec governance, and module tree / driver mapping. Operation `PARAM-IDENTITY-MAP-ADMIN-001` tracks admin-side identity mapping coverage (browser acceptance retarget in #198).
...
**Workbench attention:** Module-navigator "Pending" counts and row badges use `governanceState`. `attention` is only for open identity-mapping tasks; `blocked` is for `schemaState=invalid` or `policyState=fail`. Provisional-surface `schemaState=unreviewed` (typical after overlay-only ingest without `compatible`) does not elevate to attention — Admin SpecReviewQueue owns that backlog.
