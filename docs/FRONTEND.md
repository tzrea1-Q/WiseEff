...
`OrganizationModuleGovernancePanel` composes `ParameterModuleMappingPanel`, which in turn wires `UnclassifiedCompatibleQueue`, `ClassifyCompatibleDialog`, and `ModuleAttributionTree`. Seed and ingest build a three-tier tree: **business category → driver group → instance module**; bus/scaffolding nodes stay out of the product tree. Org mappings match on `compatible` or `instance` only (no `driver` match kind).

- **Placement helpers:** `src/domain/parameter-topology/modulePlacement.ts` (server mirror under `server/modules/parameter-modules/`).
- **Binding writes:** ingest uses `resolveBindingInstanceModuleId` to ensure/create instance modules and assign `module_id` to the instance. Unmapped `compatible` values land in provisional `Unclassified · {driver}` buckets without blocking ingest.
- **Unclassified queue:** `GET /api/v2/parameter-modules/discovery-hints` lists non-dismissed compatibles with parameter/project counts; dismiss/restore via v2 dismissal routes (audited). Bulk classify opens `ClassifyCompatibleDialog`, calls `POST /api/v2/parameter-modules/mappings/preview`, then applies scoped `POST /api/v2/parameter-modules/mappings` on confirm.
- **Kind-scoped tree:** `ModuleAttributionTree` shows kind badges, parameter counts, and business-only importance (`effectiveImportance` from registry). Actions follow server kind guards (no delete on `instance`; driver-group delete = disband; unclassified root read-only).
