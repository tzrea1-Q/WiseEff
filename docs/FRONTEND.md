...
`/parameter-admin-next` hosts the redesigned admin shell while `/parameter-admin` stays on the legacy tree. Organization governance and project operations are peer destinations (`/parameter-admin-next` vs `/parameter-admin-next/projects`). Panels consume a single `createParameterAdminApplication` facade over `ParameterTopologyRepository`, `ParameterModuleRegistryRepository`, and import actions from `parameterActions` (resolved for both mock and api modes). Admin cross-panel state lives in `ParameterAdminProvider` — not global `PrototypeState`. URL query params (`q`, `lifecycle`, `driver`, `sort`, `spec`, …) are the source of truth for filters, sort, and selection. Organization area includes bulk import (existing `ParameterImportWizard`), identity mapping governance (`IdentityMappingReview`), spec governance, and the module tree / driver-mapping panel. Operation `PARAM-IDENTITY-MAP-ADMIN-001` tracks admin-side identity mapping coverage (browser acceptance retarget in #198).
