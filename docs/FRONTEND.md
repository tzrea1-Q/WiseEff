...
- Parameter-definition references: entry detail renders the entry's structural definition references as chips (`src/features/knowledge/KnowledgeParameterReferenceChips.tsx` — display name, attribution module, and an honest lifecycle badge; deprecated definitions keep their chip per ADR-0011) deep-linking to `/parameter-admin?spec=…`. The entry editor gains a definition-reference picker for existing entries: searching runs through the injected `searchParameterSpecs` seam (`ParameterTopologyRepository.listSpecs`, wired in `src/app/routes.tsx` only when the role holds `parameter.view`), and add/remove call `KnowledgeRepository.addParameterReference` / `removeParameterReference` immediately (audited server-side, independent of the markdown save; the editor keeps its own reference state so an add never resets unsaved text). The definition detail dialog (`ParameterSpecDetail`) gains a related-knowledge section fed by `KnowledgeRepository.relatedToSpec(specId)` (published-only server-side), injected through `ParameterAdminProvider.relatedKnowledge` only when the caller holds `knowledge:view` — the section is hidden entirely otherwise.
