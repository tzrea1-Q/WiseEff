...
Developers can browse project parameters, inspect current/recommended values, create drafts, submit rounds, route reviews, merge approved changes, import/export governed data, and inspect history/audit evidence. **New projects** go through governed parameter initialization (semantic binding snapshot or empty library → Admin approve/reject) before normal typed binding submits unlock. Parameter and debugging **module taxonomies** are multi-level trees per domain; parent module filters include descendant assignments. Organization Parameter definitions live on one `/parameter-admin/specs` page: list, opaque deep link, detail, timeline, same-page Review Queue, Registration/Placement, and Proposal. There is no Effective definitions or Governance history peer view and no second catalog route. Conflict cases preserve input and require reconfirmation; legacy bookmarks produce exact mapped, gone, conflict, or not-found outcomes without Archive inference; Agent access is read-only; API and mock adapters share the same states; the page must remain usable at 1440x900, 768x1024, and 390x844. Browser IDs are `PCAT-UI-01` through `PCAT-UI-15`.
