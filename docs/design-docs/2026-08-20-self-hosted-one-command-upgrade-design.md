...
Restricted-network inputs form a separate deep module behind `build-network.sh`. A mode-`0600`, non-symlink data file accepts only proxy pairs, one npm registry URL, one approved PEM path, an enum build-TLS policy, and an explicit runtime-proxy boolean. Shell proxy variables remain an input for interactive operators; ambiguous upper/lower pairs fail closed. The controller exports Docker predefined proxy build arguments, uses an npm registry host-replacement policy for lockfile-resolved tarballs, and mounts the CA through a BuildKit secret into the shared build-transport stage. The external interface remains one `verify|insecure` policy while internal adapters translate the authorized insecure value into build-scoped npm, Git, pip-host, and apk endpoint-certificate behavior.

`verify` is the default. `insecure` is a two-key break-glass path: the private file selects it, while the setup/upgrade invocation must independently provide `--allow-insecure-build`. The module clears any inherited acknowledgement, rejects mismatched policy/authorization pairs before Docker, and exports a fixed acknowledgement only to that build process. A non-secret fingerprint covering policy, CA content, and package hosts makes trust changes invalidate BuildKit cache. Journals and the candidate image record policy provenance without proxy values. No disabling variable enters runtime `ENV`; host Git, Docker daemon trust, npm integrity, Alpine package signatures, and runtime TLS stay outside this adapter.
...
- URI userinfo redaction accepts only ASCII `scheme://userinfo@host` forms, including HTTP(S), SOCKS, SOCKS4/SOCKS4A, SOCKS5/SOCKS5H, and extension schemes, and it never scans past `/`, `?`, or `#` for an authority delimiter; ordinary email text, query/fragment email values, Unicode pseudo-schemes, and URLs without userinfo are not treated as credentials.
