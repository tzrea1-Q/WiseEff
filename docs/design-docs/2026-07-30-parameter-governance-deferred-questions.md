...
The current round adds non-repudiation only: before/after `value_shape` and `constraints` go into the governance audit, and the frontend shows a diff plus the reference count before saving.
