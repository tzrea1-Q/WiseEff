...
| `GET` | `/api/v2/projects/:projectId/bindings/:bindingId/history` | Phase-2 binding revision history. Returns `{ items: [{ id, changedAt, actor?, fromRawValue?, toRawValue?, reason? }] }` ordered oldest→newest, derived from `project_parameter_binding_revisions` plus binding-tied merged change requests. Never reads the legacy flat `parameter_history_entries`. |
...
| `POST` | `/api/v2/parameter-modules/recompute-bindings` | Admin re-resolves `project_parameter_bindings.module_id` for existing bindings (optional `{ projectId }`). Returns `{ updated, conflicts }`; unique-key conflicts → `409`. Backs the mapping panel "recompute module assignment" action. |

**Binding module identity (Phase 2):** every `project_parameter_bindings` row persists a required `module_id` referencing `parameter_modules(id)` (migration `0067`), with browse unique key `(project_id, logical_node_id, parameter_spec_id, module_id)`. Writes resolve the module (instance > compatible > driver, unclassified fallback — never null); the bindings DTO exposes `moduleId` and the workbench treats it as the browse source of truth (no derive-on-read). This is a clean cutover with no dual-read compatibility layer.
