...
| `GET` | `/api/v2/parameter-modules/driver-registry` | Org driver-registry view: `{ items: DriverRegistryEntry[], total }`. Each entry is a driver-group module with exact compatibles, `origin`, business category, parameter/observed coverage, read-only `driverNature` / `instanceCardinality` (when linked to `driver_registrations`), and per-compatible parse coverage (pinned schema pattern **or** active org overlay; `source`/`driverId` identify which). Scaffolding labels excluded. |
