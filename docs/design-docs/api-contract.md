...
| `PATCH` | `/api/v2/parameter-specs/:specId` | Admin in-place content update on **active** (or platform-global) definitions — not for drafts (`409`; use activate). Body: `documentation`, `reason`, optional `valueShape`, `constraints`, `displayName`, `description`, `units`, `exampleValue`. `constraints` **replaces** the stored object (omitted keys are removed). PATCH runs activation completeness on `valueShape` **only when it changed**; incomplete legacy shapes remain editable for documentation. Updates the current `parameter_spec_version` row. Does **not** accept `policyTarget` (product-scoped; see TD-055). Audit: `spec-updated`. |
