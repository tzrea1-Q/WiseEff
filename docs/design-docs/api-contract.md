...
| `GET` | `/api/v2/parameter-specs/:specId/property-key-cutover` | Admin reads the **open** property-key cutover run (`{ item }`). Items include `fileId` / `fileName` / `nodePath` for workbench handoff. `stagedRewrite.status` is the **live** file-candidate status (`ready`, `active`, `abandoned`, `missing`, …), not a stale prepare snapshot. Does not activate candidates or write live source. No open run → `404`. |
