...
| `POST` | `/api/v2/parameter-specs/:specId/activate` | Admin activates a **draft** org-owned spec (`valueShape`, `constraints`, `documentation`, `reason`). Only `lifecycle=draft` specs; incomplete or unsupported shapes → `409`. Audit: `parameter-topology-governance` / `spec-activated`. |
...
Cutover/rollback procedure: `docs/runbooks/parameter-identity-cutover.md`. **TD-042 remains a BLOCKER** until a clean non-customer snapshot rehearsal completes — round 4 and round 5 fixes do not clear production cutover readiness.
