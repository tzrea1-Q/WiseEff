...
`POST /api/v1/parameter-submission-rounds` has two non-overlapping item shapes. Legacy flat submissions use `{ parameterId, targetValue, reason }` **only before semantic cutover**. Post-cutover topology submissions use `{ draftId, projectParameterBindingId, parameterSpecId, targetValue, reason }` and must not also send `parameterId`; the legacy draft-save and legacy submission paths return `409`. The server locks the exact user-owned draft row within the caller organization/project, verifies binding/spec/value/reason, the draft's persisted candidate revision (`0059`), the candidate binding revision's exact raw value, and its binding/file/occurrence write lock before creating the round. Cross-project or missing drafts return `404`; mismatched identities, candidate values, non-draft candidates, or stale locks return `409`. Failed validation creates no success audit. The `parameter-submit` audit records the accepted draft, binding, and spec IDs.
