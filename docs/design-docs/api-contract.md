...
| `POST` | `/api/v2/parameter-specs/:specId/property-key-cutover/prepare` | Admin reclassifies open-run items from **live** source locations. Body `{ reason? }`. Does **not** create file-candidate / change-request drafts and does **not** rewrite DTS or the catalog triple. Honest skip when the source already has the new key (or the binding is gone); `would-rewrite` stays `pending`; conflict / missing / no-occurrence stay `incompatible`. Audit: `spec-property-key-cutover-prepared`. |
