...
| `POST` | `/api/v1/projects/:projectId/baselines/:baselineId/release` | Re-evaluate release readiness (require matching `gateToken`), run the validation gate against current member contents, then mark the baseline `released` if the gate allows it. Body: `{ gateToken, acknowledgedWarningIds? }`. Returns `200 { item: baseline, gate }`. Stale/blocked readiness or validation failure → `409`. |
