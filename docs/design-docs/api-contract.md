...
| `GET` | `/api/v2/parameter-modules` | Org module registry: `{ item: { modules, mappings } }`. Module DTO fields include `kind`, `origin`, `sourceKey`, `effectiveImportance`, `parameterCount` (plus `name`, `parentId`, `sortOrder`, `importance`). Module CRUD stays on v1. |
...
| `POST` | `/api/v2/parameter-modules/mappings/preview` | Admin dry-run create mapping (`{ moduleId, matchKind, matchValue, priority? }`); `matchKind` is `compatible` \| `instance` only. Returns `{ item: MappingApplyPreview }` without persisting. |
