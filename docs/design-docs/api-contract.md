...
| Method | Path | Purpose |
| --- | --- | --- |
| `GET` | `/api/v2/parameter-specs` | List versioned parameter specs (property key, driver module, locator fields separated). Each item includes `attributionModules: Array<{ id, name, kind }>` — the distinct attribution units (driver groups and node-type units) its bindings resolve to, computed server-side from binding relationships. Empty array means not yet observed in any project. The spec layer does not re-run compatible or node-type matching (ADR-0010). |
