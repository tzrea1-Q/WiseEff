...
`POST /api/v1/parameter-submission-rounds` has two non-overlapping item shapes. Legacy flat submissions use `{ parameterId, targetValue, reason }` **only before semantic cutover**. Post-cutover topology submissions use `{ draftId, projectParameterBindingId, parameterSpecId, action, targetValue, reason }` and must not also send `parameterId`; `action` is `set|delete` (omitted legacy-exact action defaults to `set`), `set` requires a non-empty target, and `delete` requires `targetValue: ""`. The legacy draft-save and legacy submission paths return `409`. The server locks the exact user-owned draft row within the caller organization/project and verifies binding/spec/action/value/reason, the persisted candidate revision (`0059`), and binding/file/occurrence write locks. For `set`, the candidate binding revision must carry the exact raw value. For `delete`, that binding revision must be absent and the same candidate revision must carry the matching logical-node + property-spec delete occurrence effect; missing or contradictory tombstone proof returns `409`. Cross-project or missing drafts return `404`; mismatched identities, candidate values, non-draft candidates, or stale locks return `409`. Failed validation creates no success audit. The `parameter-submit` audit records the accepted draft, binding, spec, and action.
...
**Round 6 evidence (branch `fix/parameter-topology-round6-review-blockers`):** migration `0058` evidence-only review-task scope reconcile (including missing-evidence reopen); lossless manual entity IDs and persisted specification keys (`vendor,limit` ≠ `vendor-limit`); org Admin cannot activate global drafts; full valueShape activate path; tenant-scoped cleanup; API-runtime/fixture/query isolation under `test:all`; migration `0059` plus explicit binding-draft submission identity; `0060` manual invalidation followed by forward `0061` all-origin candidate-less invalidation; and `0062` durable `set|delete` workflow action. Exact submission rejects post-cutover legacy shapes, locks the draft row, and proves candidate value or delete tombstone consistency. Topology acceptance creates a marker-verified disposable post-cutover database and passes real set and delete submit→review→merge→writeback→reload role chains. This is implementation acceptance only; TD-042 still blocks production cutover readiness.
