...
| `PCAT-DB-P01` | Every production role fails direct INSERT/UPDATE/DELETE on immutable Catalog rows. No login may be a member of `catalog_migration_owner`, and no login member of `catalog_synchronizer_role` or `parameter_governance_writer_role` may inherit it (`rolinherit`); the documented `NOINHERIT` composition-root memberships remain the audited `SET LOCAL ROLE` path. Direct `pg_auth_members` reachability only — see the limit note below. | `PCAT-PRIV-CATALOG-IMMUTABILITY-BYPASS` | role/action negative matrix and SQLSTATE checksum        | Grant repair before startup / security                                        |
...
**P01 scope note (2026-09-15).** `docs/SECURITY.md` deliberately gives the API login a `NOINHERIT` membership in `parameter_governance_writer_role` and the publication manager one in `catalog_synchronizer_role`, so the composition roots may `SET LOCAL ROLE` on the audited path; `provisionRuntimeLogins` grants exactly that and revokes `catalog_migration_owner` from every login. An earlier P01 checked for *any* login membership and therefore failed on every correctly provisioned cluster — that check was corrected to the rule in the row above. Two caveats remain recorded rather than silently fixed: migration `0138`'s inline P01 comment still says "cannot `SET ROLE` to `catalog_synchronizer_role` or `catalog_migration_owner`", which predates this model and cannot be edited in an applied migration; and the gate reads direct `pg_auth_members` rows, so indirect reachability (for example a login that inherits a NOLOGIN role which is itself a member of a privileged role, which `pg_has_role(..., 'SET')` reveals and the current predicates do not) is a known, pre-existing gap that needs its own gate change.
