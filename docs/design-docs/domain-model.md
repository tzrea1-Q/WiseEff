...
| Binding-draft submission identity | A topology draft owns an exact `draftId`, binding, spec, candidate config revision (`parameter_drafts.candidate_config_revision_id`, migration `0059`), `set|delete` action, value/reason, and write lock. Submission locks that draft row. A `set` proves the candidate binding revision has the exact submitted raw value; a `delete` proves the same candidate/evidence chain has no replacement binding revision and contains the matching logical-node + property-spec delete occurrence effect. Migration `0060` first invalidated historical candidate-less manual drafts; forward migration `0061` closes the origin gap by recording and invalidating every remaining candidate-less draft, including `file_sync` and conflict-derived rows. Users must recreate invalidated edits through the typed editor. Post-cutover legacy save/submit is rejected, and a binding ID must not masquerade as legacy `parameterId`. |
