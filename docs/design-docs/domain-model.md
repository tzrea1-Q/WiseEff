...
### Parameter seed and archive vocabulary

- **ConfigurationSchema**: a formal Catalog subject owning a software configuration contract, selected by its governed model identifier rather than a device compatible. A **configuration instance** is one project-owned occurrence of that contract. It shares canonical definitions and values with Driver/NodeType subjects. [ADR-0045](../adr/0045-configuration-schema-subject-and-seed-rebuild.md) is an accepted destination extension, not a claim that the current two-kind runtime already supports it.
- **Seed definition set**: reviewed reusable parameter contracts supplied for explicit initialization; separate from configured project values.
- **Seed project configuration**: initial source configuration and parameter values for an explicitly selected example project. One definition may serve multiple projects or node occurrences with different values.
- **Offline parameter archive**: former parameter records and source versions preserved outside the current online parameter workflow. An **archival reference** points to the authorized historical retrieval procedure, not an online legacy detail or editable current parameter.

These terms describe the accepted rebuild/archive boundary in the [parameter unification plan](../exec-plans/active/2026-09-14-parameter-unification-and-seed-parity.md); they do not claim that the workflow switch has been implemented.

### Existing domain responsibilities

- Organization and users define the tenant boundary, identity-source bindings, home-organization membership, platform role bindings, and disabled-user behavior. Organization administration operates that record; hardware/software is Role discipline, never an Organization ([ADR-0037](../adr/0037-organization-administration-is-home-org-tenant-operations.md)).
- Projects group modules and workflow state. Organization administration does not provide a ProjectMember management product. Project review role bindings (`user_role_bindings` with `project_id`) explicitly bind active home-organization users to project workflow roles (`hardware-committer`, `software-committer`, `software-user`), granting parameter edit/review/merge permissions strictly within that project and populating candidate pools for review stages, without granting organization-wide permissions. Do not infer unrestricted Catalog usage from that UI scope: at source baseline `67d4a77325b6009b77c2373bd788298a6d022bcf`, `parameter-catalog-api/productionWire.ts` consumes actual database role bindings, including explicit project IDs or organization-wide grants, and `rootUsageScope.integration.test.ts` exercises that restriction. Project-list visibility and authorized Catalog usage projection are separate decisions.
- `ParameterModule` (org-scoped tree) is the source of truth for parameter taxonomy; `ProjectModule` mirrors per-project module metadata for governance counts. Each row carries `kind` (`business` | `driver-group` | `node-type` | `unclassified`), `origin` (`curated` | `auto`), and optional stable `source_key` (`compatible:{token}` for driver groups, `nodetype:{name}` for node-type units — not display name). **`curated → auto` is out of the domain**: adoption is a one-way stated fact when an Admin renames or moves an auto-discovered module (ADR-0004); reversing origin would let a human decision disguise itself as ingest. Product attribution is a **taxonomy**, not a topology copy: **business category → {driver group | node-type unit} → nested node-type\*** (bus/scaffolding nodes stay out of the tree). Bindings attach to **driver groups** and **node-type units** only; device instance identity is `logical_node_id` on the binding (ADR-0010). A **driver registration** is a stable `DriverRegistration` attribution subject (with `driverNature`, `instanceCardinality`, and authoritative `defaultBusinessCategoryModuleId` for auto placement — D-AG-04 / TD-046) whose placement/display node is a curated or auto driver-group module plus its exact `compatible` mappings (ADR-0013; partially supersedes ADR-0007). **Auto** driver-groups follow the registration default business category and replay when that default changes; **curated** placements stay frozen (Admin move still promotes auto→curated). Keyword `businessCategoryForNodePath` is seed/bootstrap-once only. An **organization driver schema overlay** is a separate org-scoped store (`organization_driver_schemas` + property links to `parameter_specs`) that supplies exact-compatible manual parsing as the lowest releasable matcher tier without editing `schemas/dts` (ADR-0008). Overlay rows claim which properties a driver covers; types/units/docs live on ParameterSpec. Activating an overlay upgrades matching provisional manual specs in place; historical binding-revision `schemaState` stays immutable. Admin **adoption**: renaming or moving an `origin=auto` module promotes it to `curated`. Registry reads expose `effectiveImportance` inherited from the nearest business ancestor (driver groups and node-type units do not store their own importance). Attribution-tree counts on the registry DTO split **definition count** (`definitionCount`, distinct specs in the subtree) from **measured-occurrence count** (`parameterCount`, bindings). Library **reference count** (`referenceCount`) is that same binding fact scoped to one definition — not a third tree metric. **Attribution levers** (ADR-0010): `compatible` resolves a device model to a driver group; `node-type` resolves a driverless config node to a node-type unit (match value = bare node name, unit address stripped, normalized). Precedence: compatible → node-type → unclassified root.
- Parameter management centers on definitions, project values, drafts, submission rounds, change requests, review decisions, imports, history, audit, per-project DTS/JSON parameter files with bidirectional sync, and **project parameter initialization** (durable draft/review + `projects.initialization_status`; approve materializes semantic bindings — see `docs/design-docs/2026-05-20-project-parameter-initialization-design.md`).
...
**Validation gate:** `DtcValidator.validate(files, { mode })` compiles a config set's DTS members with the system `dtc` binary in a restricted subprocess (isolated temp dir, minimal environment of `PATH` plus present `HOME`/`USER`/`LOGNAME`, hard timeout) and returns `{ ok, mode, diagnostics, compiler }`. `mode` is read from `DTS_VALIDATION_MODE` (`block` default, `warn`, or `off`; see `docs/developer/environment-variables.md`).
...
**Principal and invocation provenance (ADR-0038 / TD-068):** the authenticated principal owns identity, Organization, roles, permissions, and accountability. The operation initiator is a separate server-trusted `user` / `agent` / `system` fact. #610 introduced the shared context, strict constructors, and policy/audit projection. Current source constructs Agent invocation in `agent/orchestrator.ts` and user invocation on reload mutation routes in `dts-reload/routes.ts`; the [security document](../SECURITY.md) states the source-review boundary. This does not assert program closure or complete production-path verification. User and Agent invocations retain the authenticated principal; Agent also carries session/tool-call and applicable approval correlation. System names a service/job and has no synthetic user. Approval does not change Agent provenance. Human-required DTS reload and critical parameter policies accept only `user`; missing provenance cannot default to user. This is honest classification of known server paths, not physical-human detection for ordinary bearer credentials.
...
| `ParameterSpec` / `ParameterSpecVersion` | Stable specification identity is owner scope + `attribution_subject_id` + `property_key`. `parameter_specs.id` is a **surrogate** (ADR-0017): identity is correctable in place without rewriting the row id or cascading FKs. `example_value` is illustrative only and never drives DB constraints or release policy. An effective driver property additionally requires an active definition, an active current version, a subject-bearing driver schema (for DTS properties), and exactly one organization-scoped `driver_registration_placements` row. Organization active definitions override platform active definitions; draft/deprecated and shadowed rows remain governance history. |
...
| Effective driver-definition catalog | The default catalog projection is a server-selected effective view: one row per `(organization, canonical DriverRegistration source key, property_key)`, with organization active > platform active precedence. `view=governance` exposes drafts, deprecated rows, shadowed twins, and blockers for repair. Declared placement (`driver_registration_placements`) is authoritative; observed binding modules are evidence only. Unmatched or ambiguous ingest evidence creates review metadata but never a recognized spec, binding, or resolved occurrence. The pre-existing `0117_user_account_deletion.sql` remains unchanged; Issue #649 migrations `0118`–`0125` expand/correct/harden the graph, and the audited `parameter-definitions:reconcile` job performs data repair while `parameter-definitions:check` is the read-only release gate (ADR-0039). |
| Target Subject registration and placement | The replacement catalog uses one durable `active | retired` Subject registration per `(Organization, Catalog subject)`. It always retains exactly one Subject placement through non-null `current_placement_id`, `UNIQUE (registration_id)`, and ADR-0040's deferred composite ownership FK, including while retired. Only a subject whose membership in the current Catalog Release is active may be explicitly registered or uniquely auto-matched. Retirement blocks new registration/matching but preserves registration, placement, Binding, Project value, Parameter observation, and history; a later active membership restores the same Catalog subject and existing registration IDs. A new Organization starts with zero Subject registrations. Unknown/ambiguous Parameter observations enter the Review Queue only, and neither observations nor modules become Catalog subject or Parameter definition identity. Definitions dynamically inherit placement; module move preserves the placement ID (ADR-0040, ADR-0041, ADR-0042). |
...
| Global vendor specs | `ParameterSpec` rows with `organization_id IS NULL` are platform-global vendor definitions. Tenants may **read and bind** active global specs. Organization admins must **not** activate, update, delete, reattribute, or rename-property-key global drafts/specs — only org-owned rows (`organization_id === caller org`) are mutable via standard Admin APIs. Platform globals are maintained via bootstrap/migration/dedicated platform governance (`platform-admin`), which includes deprecate / restore / identity correction of platform-global definitions (ADR-0011 amended, ADR-0017 ID-R5). |
| Driver schema overlay tiers | `driver_schema_overlays` holds organization overlays (`organization_id` set) and platform overlays (`organization_id IS NULL`). Matcher order is linux → vendor → platform-manual → org-manual. Promotion materializes platform-owned, subject-scoped ParameterSpec copies (contributor definitions remain organization-owned), then supersedes contributors (`lifecycle=superseded`) with provenance in `driver_schema_overlay_promotions`; revert restores contributors to `active`. |
| Manual spec identity | Manual/org draft IDs hash **lossless** canonical parts (`field:length:rawValue`) **when minting a new row**. Lookups resolve by `(organization_id, attribution_subject_id, property_key)` rather than re-deriving the hash (ADR-0017). Display sanitize must never feed uniqueness hashes. Distinct DTS keys such as `vendor,limit` vs `vendor-limit` must yield distinct IDs. Legacy sanitize-hash collisions are audited fail-closed without silently rewriting referenced IDs. |
...
| Workbench governance attention | User-facing "Pending" / `attention` may still flag bindings tied to open identity-mapping tasks, but resolution UI lives in `/parameter-admin` only; the workbench surfaces publish blockers instead of an inline review panel. `blocked` covers invalid / policy fail. Provisional-surface `schemaState=unreviewed` (common for overlay-only / self-anchored nodes without `compatible`) stays visible on the row but does **not** elevate to attention — that signal belongs to Admin spec review, not the everyday parameter list. |
...
| Parameter execution identity | Governance rows distinguish the accountable principal from the execution initiator. Trusted User/Agent rows carry the validated principal user; Agent rows also retain non-empty session/tool-call/approval correlation. System rows carry no user attribution and persist an explicit service/job identity. Branch-only #614 migrations `0129`–`0134` introduce the additive projection and historical compatibility after the mainline `0128`; forward migration `0135` is the database-enforced discriminated union (`NOT VALID` for old rows, strict for every new/update row). Explicit `legacy` is metadata-free and reserved for pre-#614 writers until #615 ratifies the remaining rows. Binding revisions retain the initiator projection while their parent draft/change request owns the accountable principal. System high/no-match operations remain valid and are displayed with a coarse service/job label rather than an authenticated user's name. Full correlation stays in internal domain/audit projections; public DTOs and notifications expose only the stable display label. When a retained Agent row loses its live user FK during permanent account deletion, migration `0136_parameter_execution_principal_deleted_marker.sql` records only the server-owned boolean `initiator_principal_deleted = true`; it never stores, hashes, snapshots, or restores the deleted user id. The row remains `initiator_type = 'agent'` with its complete correlation, while every accountable-user FK and derived `userId` is `NULL`. The marker can only be set by the nested database FK deletion transition and cannot be selected or cleared by ordinary writes. `parameter_drafts` is different: it is transient account-owned state, keeps `user_id ON DELETE CASCADE`, receives no deleted-principal marker, and is removed with the user. User-history deletion continues to use explicit metadata-free `legacy`, and direct malformed updates remain rejected. |
...
| Manual spec lifecycle | Unmatched `createSpec` creates org-owned **draft** specs with typed shapes inferred from occurrence AST. Admin `activate` promotes draft→active with complete constraints; only active+complete specs can `resolve`. Soft retirement (`deprecate`) moves draft or active → **deprecated** (still releasable for parsing; hidden from the default library view; not selectable in review). `restore` returns to active when `activated_at` is set, otherwise to draft (ADR-0011). |
...
| Entity | Description |
| --- | --- |
| `LogDomain` | Org-scoped registration of one business's log intake: unique `name` per organization, optional description, `status` (`active` \| `archived`), an optional declarative `format_profile` JSON (timestamp pattern, multi-line merge rule, severity mapping), P3b webhook config columns (`webhook_url`, write-only `webhook_secret`, `webhook_enabled`), and an optional `model_override` that swaps only the model name for that domain's analyses (endpoint/key/budget stay global). |
...
| `LogWebhookDelivery` | P3b: one result-webhook delivery ATTEMPT (`log_webhook_deliveries`, 0107-style composite org FKs, cascade with domain/record): `kind` (`result` \| `test`), `attempt`, `status` (`delivered` \| `retrying` \| `failed`), `http_status?`, `error?`, timestamp. `log_record_id`/`run_id` are NULL for admin test deliveries. Best-effort channel — rows record honesty, never analysis state. |
| Analyzer provenance | Additive columns on `log_analysis_reports`: `analysis_source` (`agent` \| `rules-fallback`, NULL for legacy rule reports), `degraded_reason` (`provider-unavailable` \| `token-budget-exhausted`), `prompt_version`, and `model` (the effective model — the domain override when set). |
...
- Text log extensions are `.log`, `.txt`, `.csv`, and `.json` (UTF-8 line-oriented parse; `.json` is treated as text, typically JSON-lines). P3 also accepts `.gz` (single file) and single-entry `.zip` archives, unpacked at upload time with zip-bomb bounds (100MB absolute / 200× compressed) — the stored file object always holds plain UTF-8 text, so run/evidence semantics are unchanged; unpack failures become failed records on the unsupported-format path with explicit reasons.
- Domain governance (`create`/`update`/`archive`, knowledge-link replace, webhook config, model override) requires `logs:admin-domains`, validates the format profile JSON on save, and writes `log-domain-*` audit events. Archiving a domain stops new bindings but keeps existing log records readable.
...
- `log_feedback` is append-only. Each row is linked to the log record and audit event, and — when known — to the analysis run (`run_id`, stamped from `log_records.current_run_id` at insert). Quality insights join the report for that run; a null `run_id` falls back to the log's current run.
- P3b result webhooks: after a domain-bound analysis reaches a terminal state (complete — degraded included — or dead-lettered failed), the worker fires a best-effort, signed, SSRF-guarded delivery of a compact summary (never raw log content) and records each attempt in `log_webhook_deliveries`. Delivery never blocks, fails, or retries the analysis; the uncategorized domain has no webhook. Config saves are SSRF-validated with explicit error codes and audited (`log-domain-webhook-config`); admin test deliveries are audited (`log-domain-webhook-test`). See `docs/SECURITY.md` and `docs/api/log-analysis-integration.md`.
...
Permanent node deletion is an explicit destructive catalog-governance operation, distinct from disabling. It is allowed for enabled or disabled nodes after Admin confirmation. One transaction locks the organization-scoped node, deletes every `node_operations` row referencing that node, removes their operation events and unshared snapshots, and then deletes the node; `debug_node_bindings` cascade from the node delete. A snapshot still referenced by another operation is retained and detached from the deleted operation. The row lock and restrictive operation foreign key prevent a concurrent operation reference from committing across the deletion. Shared debugging sessions, shared snapshots, unrelated operations, and audit evidence remain. The High-severity audit records only node identity/state plus binding and operation counts, never paths, values, descriptions, or notes.

Debug-module deletion remains empty-leaf-only. Reference checks count both exact `debug_node_module_id` assignments and legacy nodes whose module id is null but whose trimmed module name matches the registry module. This conservative compatibility guard prevents deleting a registry row while a legacy node would recreate the same category in runtime filters.
Runtime separation:
