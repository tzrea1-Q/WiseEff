...
- `ParameterModule` (org-scoped tree) is the source of truth for parameter taxonomy; `ProjectModule` mirrors per-project module metadata for governance counts. Each row carries `kind` (`business` | `driver-group` | `instance` | `unclassified`), `origin` (`curated` | `auto`), and optional stable `source_key` (instance identity; not display name). Product attribution is a three-layer tree: **business category → driver group → instance module** (bus/scaffolding nodes stay out of the tree). Admin **adoption**: renaming or moving an `origin=auto` module promotes it to `curated`. Registry reads expose `effectiveImportance` inherited from the nearest business ancestor (driver groups and instances do not store their own importance).
