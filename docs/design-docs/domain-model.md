...
| Binding-draft submission identity | A topology draft owns an exact `draftId`, binding, spec, candidate config revision (`parameter_drafts.candidate_config_revision_id`, migration `0059`), value/reason, and write lock. Submission locks that draft row and proves the candidate binding revision has the exact submitted raw value. Migration `0060` records and invalidates historical manual drafts without candidate identity; users must recreate them through the typed editor. Post-cutover legacy save/submit is rejected, and a binding ID must not masquerade as legacy `parameterId`. |
