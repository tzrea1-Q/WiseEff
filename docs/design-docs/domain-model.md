...
| Target Organization subject registration | The replacement catalog uses one durable `active | retired` registration and exactly one retained placement per `(Organization, formal Platform Driver or NodeType subject)`. Organization creation registers no subjects. An Organization Admin may pre-register; one uniquely proven catalog match registers idempotently under the reserved unclassified root, while unknown/ambiguous evidence enters review. All subject definitions inherit the current placement dynamically. Modules and observations never create subject or definition identity. This is the target decision, not current production behaviour (ADR-0042). |
