...
| Parameter execution identity | Governance rows distinguish the accountable principal from the execution initiator. Trusted User/Agent rows carry the validated principal user; Agent rows also retain session/tool-call/approval correlation. System rows carry no user attribution and persist an explicit service/job identity. Migrations `0116`–`0120` establish the strict projection and `0121` preserves only completely unprojected pre-#614 legacy `user` rows; any provenance-bearing row is checked bidirectionally at the database boundary. Binding revisions retain the initiator projection while their parent draft/change request owns the accountable principal. System high/no-match operations remain valid and are displayed with the service/job label rather than an authenticated user's name. |
