...
| Driver schema overlay tiers | `driver_schema_overlays` holds organization overlays (`organization_id` set) and platform overlays (`organization_id IS NULL`). Matcher order is linux → vendor → platform-manual → org-manual. Promotion supersedes contributors (`lifecycle=superseded`) with provenance in `driver_schema_overlay_promotions`; revert restores contributors to `active`. |
