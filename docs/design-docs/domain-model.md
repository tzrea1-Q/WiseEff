...
| Parameter execution identity | Governance rows distinguish the accountable principal from the execution initiator. Trusted User/Agent rows carry the validated principal user; Agent rows also retain non-empty session/tool-call/approval correlation. System rows carry no user attribution and persist an explicit service/job identity. Branch-only #614 migrations `0128`–`0133` introduce the additive projection and historical compatibility after the mainline `0127`; forward migration `0134` is the database-enforced discriminated union (`NOT VALID` for old rows, strict for every new/update row). Explicit `legacy` is metadata-free and reserved for pre-#614 writers until #615 ratifies the remaining rows. Binding revisions retain the initiator projection while their parent draft/change request owns the accountable principal. System high/no-match operations remain valid and are displayed with a coarse service/job label rather than an authenticated user's name. Full correlation stays in internal domain/audit projections; public DTOs and notifications expose only the stable display label. When a retained Agent row loses its live user FK during permanent account deletion, migration `0135` records an opaque principal tombstone and preserves `initiator_type = 'agent'` plus its full correlation; only the nested FK transition can create that tombstone-backed projection. User-history deletion continues to use explicit metadata-free `legacy`, and direct malformed updates remain rejected. |
