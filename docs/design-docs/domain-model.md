...
| `ParameterSpec` / `ParameterSpecVersion` | Stable specification identity is owner scope + `attribution_subject_id` + `property_key`. `parameter_specs.id` is a **surrogate** (ADR-0017): identity is correctable in place without rewriting the row id or cascading FKs. `example_value` is illustrative only and never drives DB constraints or release policy. |
