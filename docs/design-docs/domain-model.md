...
| Workbench governance attention | User-facing「待处理」/`attention` is reserved for open identity-mapping tasks (and `blocked` for invalid / policy fail). Provisional-surface `schemaState=unreviewed` (common for overlay-only / self-anchored nodes without `compatible`) stays visible on the row but does **not** elevate to attention — that signal belongs to Admin spec review, not the everyday parameter list. |
