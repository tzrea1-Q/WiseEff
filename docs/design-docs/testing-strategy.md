...
| Binding submission identity | `routes.test.ts`, `postCutoverWorkflow.integration.test.ts`, migrations `0059`/`0060` | HTTP keeps `draftId`/binding/spec; post-cutover legacy save/submit is rejected; server locks the draft and proves candidate raw value/write lock. Two real PG connections prove a concurrent typed edit waits and survives cleanup. Upgrade tests prove old candidate-less drafts are recorded/invalidated with rollback and idempotency. |
