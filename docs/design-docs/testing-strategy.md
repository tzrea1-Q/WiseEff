...
| Binding submission identity | `routes.test.ts`, `postCutoverWorkflow.integration.test.ts`, migrations `0059`–`0062` | HTTP keeps `draftId`/binding/spec/action; post-cutover legacy save/submit is rejected; server locks the draft and proves candidate set value or delete tombstone/write lock. Two real PG connections prove a concurrent typed edit waits and survives cleanup. Upgrade tests prove 0061 invalidates manual, `file_sync`, and conflict-derived candidate-less drafts while preserving candidate-backed drafts, with rollback and idempotency. |
