...
## CI execution integrity (EFF-02)

The fixed L1 split preserves the original validation set; selective validation remains disabled. Static/build checks, frontend tests, scripts/bridge tests and backend tests run in separate jobs. Scripts and backend each retain PostgreSQL/vector and DTS prerequisites; the sole L1 `docs:check` runs in `l1-server` after vector setup and before backend tests; only seed compilation remains advisory. `Build and test` and `Merge bar` keep their names and always run, including docs-only changes. Existing main, nightly, labeled and manual acceptance semantics remain unchanged.

The CI wrapper discovers files through Vitest `list --filesOnly`, then runs the unchanged npm test command with default and JSON reporters in a new private directory. Native file/assertion counts must reconcile with the discovery set; zero execution, missing reports, required skipped assertions and mismatched invocation identity fail. Nested Vitest suite counts are not file counts. Known conditional skips are limited to the exact vendor DTS suite when PATH tools are missing, the exact rehearsal suite when its named container is absent, and four named macOS bridge cases on other platforms. Historical skip totals are not an allowance. Reports retain skips separately from passes.

The stdlib-only `scripts/ci-required-results.ts` validates fresh reports, complete fixed-command receipts and expected job results. Aggregators use Node 22 without reinstalling dependencies. Receipts bind actual checkout SHA/tree, PR base/head where applicable, run and attempt; report digests associate bytes and do not prove success. Raw reports stay on their runner; job outputs contain bounded identities and counters without messages, logs, environment values or credentials. This does not replace full browser evidence or authorize reuse of PG/Hosted results. Regression coverage exercises malformed/stale/missing reports, skipped/cancelled required jobs, unknown flags/events/modes, lost prerequisites and altered trusted-base commands. See the [verification matrix](../developer/verification-matrix.md) for commands; performance effects remain `observation-pending` until comparable samples exist.
...
| Canonical source edit/delete | Canonical source PostgreSQL suites, `canonical-value-workflow.acceptance.spec.ts` | Single software approval, real DTS/JSON writeback, explicit deleted ProjectValue state, terminal historical pins, unchanged non-target values, no resurrection during later sibling edits, exact historical export and API-process restart. This has its own operation ID and does not replace the older topology/compiler/identity/publish gate. |
