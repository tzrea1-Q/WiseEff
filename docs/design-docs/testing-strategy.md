...
The fixed L1 split preserves the original validation set; selective validation remains disabled. Static/build checks, frontend tests, scripts/bridge tests and backend tests run in separate jobs. Scripts and backend each retain PostgreSQL/vector and DTS prerequisites; only seed compilation remains advisory. `Build and test` and `Merge bar` keep their names and always run, including docs-only changes. Existing main, nightly, labeled and manual acceptance semantics remain unchanged.
