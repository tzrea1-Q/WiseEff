...
2. Structural parameter-to-knowledge references with integrity rules (behavior on spec deprecation and entry archive). — **Shipped 2026-08-13** via [`2026-08-13-knowledge-parameter-references.md`](../exec-plans/completed/2026-08-13-knowledge-parameter-references.md): `knowledge_parameter_references` bound to the `parameter_specs.id` surrogate (survives ADR-0017 identity corrections; ADR-0011 deprecation keeps the reference with an honest deprecated badge), edit-gated authoring from the entry editor, a published-only related-knowledge list on the definition detail, and `knowledge.getDocument` reporting referenced definitions.
