...
In the local development profile (`NODE_ENV=development`, `AUTH_MODE=production`, `AUTH_PROVIDER=local`), self-registered accounts are intentionally attached to the seeded `org-chargelab` / `ChargeLab` demo organization so they can see the seeded parameter, log, and debugging data. The same development profile also upserts fixed demo usernames and a shared password for the seeded ChargeLab personas during `db:seed:m0` (see [local development](../developer/local-development.md#development-demo-logins-api-mode)); non-development seeds do not write those credentials. Non-development local-account deployments keep the selected department organization ids (`org-hardware-department` or `org-software-department`) for tenant isolation.
