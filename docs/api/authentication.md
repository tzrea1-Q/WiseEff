...
Passwords are stored as salted `scrypt` hashes in `user_password_credentials`. Session tokens are returned once to the caller as opaque `we_local_*` bearer tokens only for successful login or non-committer registration; pending Committer registration and Admin-created local accounts never return a session token from their creation response. Only SHA-256 token hashes are persisted in `auth_sessions`. Sessions expire after the service TTL and logout sets `revoked_at`. Every register, login, logout, profile update, home-organization rename, Admin user creation, role replacement, and activation change writes an audit event.
