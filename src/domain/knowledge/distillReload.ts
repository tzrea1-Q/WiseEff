import type { DtsReloadParameterVerification, DtsReloadRun, DtsReloadRunStatus } from "@/domain/dtsReload/types";

/**
 * Mock-runtime mirror of the server-side reload distillation prefill
 * (`server/modules/knowledge/reloadDistillation.ts`): same title/tags/markdown
 * shape, built from the frontend reload run DTO so mock mode keeps the same
 * port behavior. Couples ONLY to the stored run/snapshot DTO on both sides.
 */
...
export function isReloadRunDistillable(status: DtsReloadRunStatus): status is ReloadDistillableStatus {
  return (RELOAD_DISTILLABLE_STATUSES as readonly DtsReloadRunStatus[]).includes(status);
}
...
const VERIFICATION_OUTCOME_LABELS: Record<DtsReloadParameterVerification["outcome"], string> = {
  verified: "已验证",
  contradicted: "矛盾",
  unbound: "缺少调试节点绑定",
  "read-failed": "读取失败"
...
const PURPOSE_LABELS: Record<DtsReloadRun["purpose"], string> = {
  ordinary: "参数调试重载",
...
function deviceContext(run: DtsReloadRun): string {
...
function propertySummary(run: DtsReloadRun): string {
...
export function buildReloadDistillationDraft(run: DtsReloadRun): ReloadDistillationDraft {
  ...
  const snapshot = run.reloadSnapshot ?? null;
  const verificationByBinding = new Map<string, DtsReloadParameterVerification>(
