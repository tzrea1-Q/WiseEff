...
export function ParameterDefinitionForm({
  ...
  const updateRange = (patch: { min?: string; max?: string }) => {
    ...
    onMetadataChange({ range: min || max ? `${min} - ${max}` : parameter.range });
  };
  ...
  return (
    <section className="shared-definition-panel" aria-label="共享参数定义">
      ...
      <form className="config-form-grid param-def-form" onSubmit={(event) => event.preventDefault()}>
        <label>
          参数名
          <input
        ...
        <label className="wide">
          展示描述
          <textarea value={parameter.description} onChange={(event) => onMetadataChange({ description: event.target.value })} rows={3} />
        </label>
        <label className="wide">
          参数解释
          <textarea value={parameter.explanation} onChange={(event) => onMetadataChange({ explanation: event.target.value })} rows={4} />
        </label>
        <label className="wide">
          配置格式
          <textarea value={parameter.configFormat} onChange={(event) => onMetadataChange({ configFormat: event.target.value })} rows={3} />
        </label>
      </form>
    </section>
  );
...
function getNameError(parameter: PowerManagementParameterTemplate, allParameters: readonly PowerManagementParameterTemplate[]) {
  ...
  if (!NAME_RE.test(parameter.name)) {
    return "只允许小写字母、数字、下划线，且首字符为字母";
  }
  if (allParameters.some((item) => item.id !== parameter.id && item.name === parameter.name)) {
    return "已存在同名参数";
  }
  return null;
}
