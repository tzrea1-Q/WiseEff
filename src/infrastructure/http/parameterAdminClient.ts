...
export type ProjectAdminSummaryDto = {
  ...
  status: string;
  /** Init workflow status from `projects.initialization_status`. */
...
export function createParameterAdminClient(client: ApiClient = createDefaultApiClient()) {
  ...
  return {
    ...
    listCatalogRegistrations: (organizationId: string, query?: CatalogListQuery) =>
      catalogClient.listRegistrations(organizationId, query),
    listCatalogReviewItems: (organizationId: string, query?: CatalogListQuery) =>
      catalogClient.listReviewItems(organizationId, query),
