...
- `server/modules/logs/`: M2 log upload, analysis records, object storage, and worker boundary. The analysis kernel behind `LogAnalysisAdapter` is a single-shot LLM analyzer (`analyzer/llmAnalyzer.ts`, ADR-0022: plain `ChatOpenAI` on the `LOG_ANALYSIS_*` env family — no LangGraph, no ToolRegistry, no write path): the deterministic prefilter (`prefilter.ts`, extracted from the legacy rule analyzer) bounds a token-budgeted excerpt, output is strict JSON with a grounding check on cited line numbers, and provider failure degrades honestly to the rule engine with explicit provenance (`analysis_source` / `degraded_reason`). Org-scoped log domains (`log_domains`) carry declarative format profiles applied at parse time; behavior-layer eval lives in `server/modules/logs/eval/` (`npm run logs:eval`, CI-gated).
