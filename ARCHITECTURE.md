...
- `server/modules/parameter-modules/`: v2 module registry, discovery hints, compatible dismissals, mapping preview/scoped apply, binding recompute (`dryRun`), and driver-group disband helpers used by workbench attribution and `/parameter-admin/modules`.
...
- `server/modules/logs/`: M2 log upload, analysis records, object storage, and worker boundary. The analysis kernel behind `LogAnalysisAdapter` (ADR-0022: plain `ChatOpenAI` on the `LOG_ANALYSIS_*` env family — no LangGraph, no ToolRegistry, no write path) defaults to the P2 bounded agent loop (`analyzer/agentLoop.ts`, `LOG_ANALYSIS_KERNEL=loop`): at most `LOG_ANALYSIS_MAX_STEPS` strict-JSON model steps over five read-only org-scoped tools in `analyzer/tools/` (`search_log_lines`, `read_line_range`, `get_prefilter_findings`, `read_domain_knowledge` — published knowledge entries linked via `log_domain_knowledge_links`, `get_related_parameter_context`), running inside the `rootcause` stage with per-step progress. The P1 single-shot analyzer (`analyzer/llmAnalyzer.ts`) stays as the `single-shot` config fallback. Both share the deterministic prefilter (`prefilter.ts`), the grounding check on cited line numbers, and honest degradation to the rule engine with explicit provenance (`analysis_source` / `degraded_reason`; exhausted loop budgets converge early with capped confidence, still marked). Org-scoped log domains (`log_domains`) carry declarative format profiles applied at parse time. Two eval layers live in `server/modules/logs/eval/`: behavior (`npm run logs:eval`, CI-gated, scripted models over both kernels) and quality (`npm run logs:eval:quality`, golden case set under `eval-cases/logs/`, rubric-judge seam, baseline gate).
- `server/modules/product-feedback/`: Internal Beta feedback submission, image attachment metadata, and Admin triage routes.
- `server/modules/knowledge/`: organization-scoped knowledge entries with immutable revisions, file uploads with text extraction (seam over pdf-parse/mammoth), published-only hybrid retrieval, and `knowledge:view|edit|manage` enforcement. `knowledge/indexing/` holds the async chunk-index worker seam (polling default, mirrors the logs worker): publish/edit/archive enqueue per-entry refreshes into `knowledge_index_status`, chunks carry optional pgvector embeddings via the `EMBEDDING_API_*` seam, and search fuses vector + FTS/trigram rankings (RRF) with an honest FTS-only degradation when the extension or endpoint is missing (ADR-0025). Xiaoze gains the read tools `knowledge.search` / `knowledge.getDocument` (org-scoped, `knowledge:view`, published-only, citation payloads). UI at `/knowledge` (search, ask-Xiaoze entry in API mode) plus archived governance and index health/rebuild on `/knowledge-admin`; agent drafts arrive with the distillation phase.
...
Xiaoze is the sole Agent surface. API mode mounts CopilotKit against `POST /api/v1/agent/xiaoze`; mock mode has no Agent UI. The backend reuses `ToolRegistry` authorization: read tools run automatically, while mutating tools (`action.submitParameterChange`) pause on AG-UI interrupts and execute only through the orchestrator-owned Agent approval chain (`beginApproval` / `resolveApproval`) with audit `actorType=agent`; approval state lives in `agent_tool_calls` + `agent_approvals` rows, never process memory, and `editedArgs` replaces the payload before approval-time re-authorization (ADR-0024). P2 uses a LangGraph `StateGraph` planning loop (intent → perceive → plan → act → observe) with a checkpointer keyed by `threadId`; production and self-hosted deployments use `XIAOZE_CHECKPOINTER=postgres` so HITL resume survives API restarts and multi-replica routing, while local dev/tests default to in-memory checkpointing unless configured otherwise. Approval resume re-enters the graph via `Command({ resume })` carrying only the approval decision; per-request auth context and the run event sink flow through per-invoke `config.configurable`, so the planning agent holds no per-request module state. Opt-in proactive suggestions use read-only `POST /api/v1/agent/xiaoze/suggest`, gated by `XIAOZE_PROACTIVE_ENABLED` / `VITE_XIAOZE_PROACTIVE_ENABLED` (default off). Live runs use LangChain `ChatOpenAI` configured by `XIAOZE_LLM_API_BASE_URL`, `XIAOZE_LLM_MODEL`, and `XIAOZE_LLM_API_KEY` unless `XIAOZE_DETERMINISTIC` is set.
...
Prometheus, alert rules, Alertmanager, exporters, and automatically provisioned Grafana dashboards live in the optional self-hosted `observability` Compose profile under `ops/self-hosted/observability/`; `ops/self-hosted/scripts/observability` owns its lifecycle. The API and dedicated worker expose private Prometheus targets, while blackbox probes cover every runtime service. `/metrics` and the loopback-bound monitoring UIs are internal operations data and must be reached only through a private network, SSH tunnel, VPN, reverse-proxy allowlist, mTLS, or a stronger equivalent control.
